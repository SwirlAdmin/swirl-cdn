      ...
      if (json.status_code == '1') {
        if (json.data != '') {
          ...
          db.collection("live_streams").doc(stream_id).update({
            ...
            'product_status_updated': firebase.firestore.FieldValue.increment(1)
          });
    ...
    snapshot.docChanges().forEach((change) => {
      ...
      if (change.type === "added") {
        if (change.doc.data().from == des_id) {
          ...
          var pin = `<img src="https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/gif/gift-line.png" class="gift-icon" onclick="gift_send(${param}, '${customer_name}', '${customer_phone}')">`;
        }
        i++;
...
if ((end_time_stream == '0000-00-00 00:00:00')) {
  setInterval(function () {
    $.ajax({
      ...
      url: "https://livevideoshopping.in/index.php/FirestoretestIvs/getConcurrentUsers",
      data: "stream_id=" + encodeURIComponent(stream_id),
      beforeSend: function () { },
      success: function (data) {
        //multiple views factor
        if (data.ivs_viewer_count > 0) {
          console.log(data.ivs_viewer_count);
          $('#view-count').text(data.ivs_viewer_count);
          db.collection("live_streams").doc(stream_id).update({
            'ivs_viewer_count': data.ivs_viewer_count,
            'unique_viewers': data.unique_viewers,
            'total_watch_time': data.total_watch_time,
            'total_playing_time': data.total_playing_time
          });
        }
      }
    });
...
function gift_send_swal(id, customer_name, customer_phone) {
  ...
  swal({
    ...
    closeOnConfirm: false,
    closeOnCancel: false,
    imageUrl: "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/gif/gift-icon-bg.png",
    imageAlt: "gift icon",
