...
if(doc.exists == true)
{
  ...
  if(livestream == true) {
      //$('.live-status').html('<img src="<?php echo base_url(); ?>assets/live/recorded-status.png">');
      $('.live-status').html('<span class="recording-icon">RECORDED</span>');
      $('.mux_views_div').hide();
  } else {
      //$('.live-status').html('<img class="live-sign" src="<?php echo base_url(); ?>assets/live/live_sign.png">');
    ...
    success: function (json) {
      ...
      if (json.status_code == '1') {
        ...
        for (var k = 0; k < json['data'].length; k++) {
          ...
          $('.dynamicproductlist').append('<div class="d-flex align-items-start SLS-product-tile productblock-' + json['data'][k]['product_id'] + '"><div class="SLS-product-image"><img src="' + json['data'][k]['images'] + '" alt="' + json['data'][k]['product_title'] + '" title="' + json['data'][k]['product_title'] + '" /></div><div class="SLS-product-content"><p class="m-0" title="' + json['data'][k]['product_title_full'] + '">' + json['data'][k]['product_title'] + '</p><label class="m-0 mb-2 d-block">' + currency + json['data'][k]['product_price'] + '</label></div><label class="actions mx-2 starproducts '+ setproduct +'" data-productid="' + json['data'][k]['product_id'] + '"><img src="'+ base_url +'assets/images/star_black.png" class="unsetproduct" style="display:'+ unsetproduct_css +'" value="0"><img src="'+ base_url +'assets/images/star_fill.png" class="setproduct" style="display:'+ setproduct_css +'" value="1"></label><label class="actions mx-2"><img src="https://shop.goswirl.live/assets/live/delete-button.png" onclick="deleteproduct(' + json['data'][k]['product_id'] + ')"></label></div>');
        }
      }
      else {
        $('.SLS-add-pro-btn').toggleClass("main");
  ...
  }).then(function(isConfirm) 
  {
    if (isConfirm.value == true) 
    {
      ...
      $.ajax({
          ...
          data: "stream_id=" + encodeURIComponent(stream_id) + "&datetime=" + dateTime + "&multiple_views_factor=" + encodeURIComponent(multiple_views_factor),
