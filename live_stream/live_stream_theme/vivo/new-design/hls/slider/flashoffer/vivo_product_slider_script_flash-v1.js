...
function listenToFlashOffer(streamId)
{
  db.collection("features").doc(streamId).onSnapshot((doc) => {
    if (doc.exists) {
        ...
        var fieldName = ActiveCouponId + "_remainingCoupon";
        var currentRemainingCouponValue = doc.data()[fieldName] || 0;
        console.log('active ' + currentRemainingCouponValue);
            ...
            if (localStorage.getItem("couponId") != ActiveCouponId) {
                ...
                bookRef.get().then((doc) => {
                    if (doc.exists) {
                        ...
                        var couponCard = `
                              <div id="flashoffer-container-inner" class="${doc.id}">
                                <div class="leftcoupon-div" >
                                  <p><img src="https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/svg/vivo/flashvector.svg" alt="flashvector">${currentRemainingCouponValue} coupons left</p>
                                  <div class="timer flash-timer" id="timer-${doc.id}">00 : ${data.duration}</div>
                                </div>
                                <div class="flash-div">
                                  <div class="left">
                                      <div class="icon">
                        ...
                        startTimerFlashOffer(doc.id, data.duration);
  ...
  if (localcouponId != couponId) 
  {
    if (user_id != '') {
      ...
      db.collection('features').doc(streamId).collection('Userdata').doc(couponIdNew).set(couponData)
          .then(() => {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(couponCode).then(function() {
                    ...
                    alert('Unable to copy. Please try manually: ' + couponCode);
                });
                $(this).text('Copied');
            } else {
        ...
        // Update the remainingCoupon field
            var fieldName = couponId + "_remainingCoupon";
            var couponDocRef = db.collection("features").doc(streamId);
            
            var updateObject = {};
            updateObject[fieldName] = firebase.firestore.FieldValue.increment(-1)

            couponDocRef.update(updateObject)
