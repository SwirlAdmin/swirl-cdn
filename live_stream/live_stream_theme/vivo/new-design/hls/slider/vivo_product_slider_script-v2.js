$(document).ready(function () {
  if ($(window).width() > 768) {
    ...
    if(end_time != '0000-00-00 00:00:00')
    {
      ...
      $('.swiper-button-next, .swiper-button-prev').css('bottom','120px');
    }
  }
  SLSsetHeight();
});
...
/* $(".SLS-show-products").click(function () 
{
  if ($(window).width() < 768) 
  {
    if ($("#bottom-showcase-products").is(":visible")) {
      $('#bottom-showcase-products').hide();
      $('.swiper-button-next, .swiper-button-prev').hide();
    } else {
      $(".SLS-comments-panel-close, .SLS-share-panel-close, .SLS-ask-panel-close, .SLS-offer-panel-close, .SLS-poll-panel-close").click();
      let pinheight = actionbar_size + recordingcontrol_size;
      $('#bottom-showcase-products').css('bottom', pinheight);
      $('#bottom-showcase-products').show();
  ...
  else
  {
    if ($(".SLS-products-panel").is(":visible")) {
      ...
      $(".SLS-products-panel").show();
      setTimeout(() => {
        $(".SLS-products-panel").css("transform", "translateX(0%)");
      }, 10);
      $('.SLS-product-tile').css('border-bottom', '1px solid #c7c7c7');
      $('.SLS-added-products .SLS-product-tile').removeClass('hide');
    }
...
/* $(".SLS-show-products").click(function () {
  ...
  const bottomShowcaseProducts = $("#bottom-showcase-products");
  const productsPanel = $(".SLS-products-panel");

  if (windowWidth < 768) {
    if (bottomShowcaseProducts.is(":visible")) {
      bottomShowcaseProducts.hide();
      $('.swiper-button-next, .swiper-button-prev').hide();
      if ($(".SLS-comments-panel").is(":visible")) {
        ...
        var pinheightbottom = actionbar_size + recordingcontrol_size;
        $('.messages-pin-mobile').css('bottom', pinheightbottom);
      }
    } else {
      $(".SLS-comments-panel-close, .SLS-share-panel-close, .SLS-ask-panel-close, .SLS-offer-panel-close, .SLS-poll-panel-close").click();
      const pinHeight = actionbar_size + recordingcontrol_size;
      bottomShowcaseProducts.css('bottom', pinHeight).show();
      $('.swiper-button-next, .swiper-button-prev').show();
      var pinheightbottom = actionbar_size + recordingcontrol_size + $("#bottom-showcase-products").height();
      $('.messages-pin-mobile').css('bottom', pinheightbottom);
    }
  } else {
    if (productsPanel.is(":visible")) {
      $(".SLS-products-panel-close").click();
    } else {
      productsPanel.show();
      setTimeout(() => {
        productsPanel.css("transform", "translateX(0%)");
      }, 10);
      $('.SLS-product-tile').css('border-bottom', '1px solid #c7c7c7');
      $('.SLS-added-products .SLS-product-tile').removeClass('hide');
    }
...
$(".SLS-show-comments").click(function () {
  if ($(".SLS-comments-panel").is(":visible")) {
    ...
    if ($(window).width() < 768) {
      if ($("#bottom-showcase-products").is(":visible")) {
        var pinheight = $(".messages-pin-mobile").height() + actionbar_size + recordingcontrol_size + $("#bottom-showcase-products").height();
      } else {
        var pinheight = $(".messages-pin-mobile").height() + actionbar_size + recordingcontrol_size;
      }
      ...
      if(is_defer == 2)
      {
        var pinheight = $(".SL-schedule-timer").height() + actionbar_size + 30;
      }
      $('.SLS-comments-panel').css('bottom', pinheight);
      $(".SLS-products-panel-close").click();
      $(".SLS-share-panel-close").click();
...
/* $(".SLS-show-comments").click(function () {
  if ($(".SLS-comments-panel").is(":visible")) {
    ...
    if ($(window).width() < 768) {
      let pinheight = $(".messages-pin-mobile").height() + actionbar_size + recordingcontrol_size + 94;
  ...
  if (doc.exists) {
    ...
    if (pin != '' && pin != undefined) {
      ...
      docRef.get().then((doc) => {
        ...
        let pinheight2 = $(".messages-pin-mobile").height() + actionbar_size + recordingcontrol_size + $("#bottom-showcase-products").height();
...
function reloadOnceLive() {
  db.collection("live_streams").doc(streamId).onSnapshot((doc) => {
    ...
    if(is_defer == 2)
    {
      ...
      $('.live-status').show();
     // $('.view-count-live').show();
     /*  var pinheight = $("#bottom-showcase-products").height() + 70;
      $('.SL-schedule-timer').css('bottom', pinheight); */
