...
db.collection("live_streams").doc(streamId)
  .onSnapshot({
    ...
    if (doc.exists == true) {
      ...
      if(is_defer == 2)
      {
        $('.live-status').html('<span class="live-icon" style="background: gray;"> Upcoming</span>');
      }
    }
  });
      ...
      if (change.doc.data().flag === '1') {
        if (change.type === "removed") {
          ...
          if ((change.doc.data().message).match(regex)) {
            ...
            $('<div id="message-' + change.doc.id + '"><p class="' + fillcolor + '"><span>' + change.doc.data().name + ' </span>: ' + string_link + '<span class="time ' + brand_time + '">' + time + '</p></div>').appendTo($('.SLS-comments-all')).addClass('new');
          }
          else {
            $('<div id="message-' + change.doc.id + '"><p class="' + fillcolor + '"><span>' + change.doc.data().name + ' </span>: ' + change.doc.data().message + '<span class="time ' + brand_time + '">' + time + '</p></div>').appendTo($('.SLS-comments-all')).addClass('new');
          }
        }
      }
      setTimeout(function () {
