...
$(window).on("orientationchange", function () {
  ...
  if (window.matchMedia("(orientation: portrait)").matches) {
      ...
      console.log("Portrait mode - Window Height: " + windowHeight);
  }
  setTimeout(function() {
      if (window.orientation === 0 || window.orientation === 180) {
          // Portrait mode
          const currentHeightDataBox = window.innerHeight - $(".video-box").height() - 17;
          console.log(currentHeightDataBox);
          $(".data-box").height(currentHeightDataBox);
      }
  }, 200);
...
const videoSrc = $('#live_stream_url').val(); // Replace with your M3U8 URL
const player = videojs('livestreambro');

// Validate video source
if (!player || !videojs.players.livestreambro) {
  alert("Player is not supported or could not be initialized.");
} else {
  if (!videoSrc || !videoSrc.endsWith('.m3u8')) {
      console.log("Invalid video source. Please provide a valid M3U8 URL.");
      player.dispose();
  } else {
      ...
      player.on('error', function() {
          ...
          if (error) {
              switch (error.code) {
                  case 2:
                      errorMessage = "The media is corrupted or cannot be played.";
                      break;
                  case 3:
                      errorMessage = "The media is unsupported.";
                      break;
                  default:
          ...
          player.play().catch((playError) => {
              console.error("Failed to start playback after reload:", playError);
          });
      });

      // Optional: Handle playback end
...
$('.message-submit').click(function () {
  ...
  if (user_id != '') {
    ...
    $('#demo-modal-1').removeAttr("data-product-id");
...
function otpcheck_without_sp(e) {
  if (e == 0) {
    ...
    var relocation = document.getElementById("re-location").value;
    ...
    if (name == '') {
      ...
      $('.error_msg').html("This is a required fields.").fadeIn().delay(3000).fadeOut();
      $('#re-email').focus();
      return false;
    }
    else if (!emailPattern.test(reemail)) {
      $('.error_msg').html("Please enter a valid email address.").fadeIn().delay(3000).fadeOut();
      $('#re-email').focus();
      return false;
    }
    else if (mobile == '') {
      ...
      $('.error_msg').html("Please enter a valid mobile number between 8 and 11 digits.").fadeIn().delay(3000).fadeOut();
      $('#mobile').focus();
      return false;
    }
   
    else if (relocation == '') {
      $('.error_msg').html("Please select a location.").fadeIn().delay(3000).fadeOut();
      $('#re-location').focus();
      return false;
    }
    else {
      ...
      $('#user_email').val(reemail);
      $('#user_location').val(relocation);
      $('#country_code').val(code);
      // Set Item
      try {
        localStorage.setItem("user_id", user_code);
        ...
        localStorage.setItem("user_email", reemail);
        localStorage.setItem("user_location", relocation);
      } catch (e) {
      }
      insertMessage();
      if ($('#demo-modal-1').attr('data-product-id')) {
          var proid = $('#demo-modal-1').attr('data-product-id');
          customBuynowpopup(proid);
      }
    }
  }
}
function customBuynowpopup(proid)
{
  var productData = $('[data-proid='+proid+']').data();
  console.log(productData);

  var product_id = $('[data-proid='+proid+']').attr("data-proid");
  var designer_id = $('[data-proid='+proid+']').attr("data-desid");
  var pro_title = $('[data-proid='+proid+']').attr("data-protitle");
  var pro_img = $('[data-proid='+proid+']').attr("data-proimg");
  var pro_redirection = $('[data-proid='+proid+']').attr("data-redirection");
      ...
      if (result.isConfirmed) {
       ...
       // localStorage.setItem(localStorageKey, true);
        Swal.fire({
          position: "bottom",
          title: "Submitted!",
          text: "Thanks for your test drive request! A "+brand_name+" representative will reach out to you soon.",
...
function rsvp_submit() {
  ...
  var location = document.getElementById("rsvp-location").value;
  ...
  if (name == '') {
    ...
    $('.form-validation-error').html("Please enter a valid email address.").fadeIn().delay(3000).fadeOut();
    $('#rsvp-email').focus();
    return false;
  }else if (phone == '') {
    $('.form-validation-error').html("This is a required fields.").fadeIn().delay(3000).fadeOut();
    ...
    $('.form-validation-error').html("Please enter a valid mobile number between 8 and 11 digits.").fadeIn().delay(3000).fadeOut();
    $('#rsvp-phone').focus();
    return false;
  }
  else if (location == '') {
    $('.form-validation-error').html("Please select a location.").fadeIn().delay(3000).fadeOut();
    $('#rsvp-location').focus();
    return false;
  }
    ...
    $('.error_msg_rsvp').css('color', '#11ff11').html("RSVP form submitted successfully").fadeIn().delay(3000).fadeOut().promise()
    .done(function() {
...
try {
  ...
  if (localStorage.getItem("user_location") != null) {
    res3 = localStorage.getItem("user_location");
    $('#user_location').val(res3);
  }
...
function topFunction() {
  ...
  }, 400); // Duration of 400 milliseconds for a smoother scroll
}
...
$('body').on('click', '.SLS-product-buynow-btn', function () 
{
  ...
  if (user_id == '') {
    $('#demo-modal-1').attr('data-product-id', product_id).modal('show');
    $('.close1').css('display', 'none');
  }
  else {
  ...
  if (doc.exists) {
    ...
    if (end_time == '0000-00-00 00:00:00') {
      ...
      if (is_defer != 2) {
        ...
        $('.top-bar .muteicon').removeClass('d-inline').hide();
        $('.top-bar .share-icon').removeClass('d-inline').hide();
      ...
      if (!querySnapshot.empty) {
        ...
        querySnapshot.forEach(function (doc) {
          ...
          if (doc.exists) {
            if(doc.data().content != '')
            {
              $('li.tab-link[data-tab="tab-4"]').show();
              var productsid = $('#product_ids').val();
              if(productsid == '')
              {
                $('li.tab-link[data-tab="tab-4"]').addClass('onlyoffer');
              }
         ...
         if (hasContent) {
          if ($(window).width() > 768){
            ...
            $('li.tab-link[data-tab="tab-4"]').css('display', 'block').addClass('current');
        ...
        $('li.tab-link[data-tab="tab-4"]').hide();
        $('li.tab-link.product-tab-title.current[data-tab="tab-2"]').addClass('only');
      }
    }
  );
}
...
$(document).ready(function () {
  ...
  $('body').on('click', '.SLS-poll-panel-close', function () {
    ...
    if ($(window).width() < 768) {
      $(".poll-tab").css("visibility", "hidden");
      $('.tab-link.current').removeClass('current');
      $('li.tab-link[data-tab="tab-1"]').css('display', 'block').addClass('current');
      $('#tab-1').addClass('current');
    }
  });
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      if (json['status_code'] == '1') {
        $('li.tab-link[data-tab="tab-4"]').removeClass('onlyoffer');
        ...
        $('.all-tabs').addClass('h-100');
        $('.tab-link.current').removeClass('current');
        $('li.tab-link[data-tab="tab-2"]').css('display', 'block').addClass('current');
