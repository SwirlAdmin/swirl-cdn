  ...
  .onSnapshot({
    ...
    if(doc.exists == true)
    {
      ...
      if (livestream == true) {
        ...
        var reLink = "https://goswirl.shop/swirl-embed/short-videos-carousel/v12M/";
        var mobileOS = navigator.userAgent || navigator.vendor || window.opera;
        if (/iPad|iPhone|iPod/.test(mobileOS) && !window.MSStream) {
          $('.btn-buy').click(function () {
            var redirectWindow = window.open(reLink, '_self');
            $.ajax({
              type: 'POST',
              url: '/echo/json/',
              success: function (data) {
...
if (storedBlueId !== blueid || storedStreamId !== streamId) 
{
  ...
  blueRewardDataRef.collection("blue_ids").doc(blueid).set(blueData);
  
  blueRewardDataRef.update({
      u_users: firebase.firestore.FieldValue.increment(1)
  });
