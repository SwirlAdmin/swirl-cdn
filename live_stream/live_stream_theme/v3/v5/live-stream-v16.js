$(document).ready(function () {
  if ($(window).width() > 768) {
    ...
    setTimeout(() => {
      ...
      $('.SLS-comments-all').html($('.SLS-comments-all').find('div').get().reverse());
      pinheightweb(); 
    }, 5000);
  }else{
...
db.collection("live_streams").doc(streamId)
  .onSnapshot({
    ...
    if(doc.exists == true)
    {
      ...
      if (livestream == true) {
        $('#livestatus').text('TALLENNE');
        $('#livestatus').addClass('recordingflag');
        //$('.live-status').html('<span class="recording-icon">TALLENNE</span>');
      } else {
        $('#livestatus').text('LÄHETYS');
        $('#livestatus').addClass('liveflag');
        //$('.live-status').html('<span class="live-icon">LÄHETYS</span>');
      }
    }else{
      if(is_defer == 2)
      {
        $('#livestatus').text('TULEVAAN');
        $('#livestatus').addClass('upcoming-flag');
        $('#viewcount').hide();
        //$('.live-status').html('<span class="live-icon upcoming-flag" style="background: gray;"> TULEVAAN</span>');
      }
    }
  });
     ...
     if (doc.exists) {
      let docInterval = setInterval(function () {
        if (live_stream_status == 'idle') {
          $.ajax({
            url: 'https://api.goswirl.live/index.php/api/Mogicon/muxLiveStreamStatus',
            method: 'POST',
            dataType: 'JSON',
  ...
  if (msg == '') {
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        alert('Kysymyksesi on lähetetty onnistuneesti.');
        ask_que_count();
      }
    });
  }
