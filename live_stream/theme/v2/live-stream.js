...
db.collection("messages").doc(streamId).collection("messages").orderBy("created_time", "asc")
  .onSnapshot((snapshot) => {
      ...
      if (change.doc.data().flag === '1') {
        if (change.type === "removed") {
          ...
          if ((change.doc.data().message).match(regex)) {
            ...
            $('<div id="message-' + change.doc.id + '"><p class="' + fillcolor + '"><span>' + change.doc.data().name + ' </span>: ' + string_link + '</p></div>').appendTo($('.SLS-comments-all')).addClass('new');
          }
          else {
            $('<div id="message-' + change.doc.id + '"><p class="' + fillcolor + '"><span>' + change.doc.data().name + ' </span>: ' + change.doc.data().message + '</p></div>').appendTo($('.SLS-comments-all')).addClass('new');
          }
        }
      }
      setTimeout(function () {
    ...
    $("#comments_count").text(comment_count);
    updateScrollbar();
  });
if ($(window).width() < 768) {
