...
function generatessv(videos) {
                ...
                <div class="video-modal-video-container-ssv">
                    ...
                    <div class="video-modal-actions-ssv">                                           
                        ...
                        <button class="video-modal-like-ssv" title="Like" onclick="likeVideossv(this, ${video.video_id}, ${video.designer_id});" style="background: rgb(0, 0, 0, .6) !important; display: none !important;">
                            <img src="${getCookie(`ssv_vl_${video.video_id}`) ? 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/heart-fill-2.webp' : 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/heart-outline.webp'}"
                                alt="Heart icon">
                    ...
                    <div class="user-registration-popup-ssv">
                        ...
                        <div class="urp-form-ssv">
                            <form onsubmit="return userRegistrationssv(this, ${video.video_id});">
                                ...
                                <input name="code" id="codeUrp${video.video_id}" type="hidden" value="91" />
                                <input name="phone" id="phoneUrp${video.video_id}" type="text" style="margin: 0px !important;" placeholder="Enter your phone" pattern=".{10,10}" onkeypress='return event.charCode >= 48 && event.charCode <= 57' title="Minimum 10 and maximim 10 digits." required />                                
    ...
    videos.forEach(video => {
        ...
        // User registration         
        jQuery(`#phoneUrp${video.video_id}`).intlTelInput({
            initialCountry: "in",
            separateDialCode: true,
            // utilsScript: "https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/11.0.4/js/utils.js"
        }).on('countrychange', function () {
            jQuery(`#codeUrp${video.video_id}`).val(jQuery(`#phoneUrp${video.video_id}`).intlTelInput("getSelectedCountryData").dialCode);
        });
...
function playWithPromisessv(player) {
    ...
    let src = ssv_globalMute ? 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/volume-mute-fill.webp' : 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/volume-up-fill.webp';
...
function volumessv(btn) {
    if (jQuery(btn).closest('.video-modal-video-container-ssv').find('video').prop('muted')) {
        jQuery(btn).children('img').attr('src', 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/volume-up-fill.webp');
        ...
        jQuery(btn).children('img').attr('src', 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/volume-mute-fill.webp');
...
function openUrpssv(btn) {
    jQuery(btn).closest('.video-modal-video-container-ssv').find('.user-registration-popup-ssv').fadeIn(500);
    jQuery('.video-modal-fade-ssv').show();
}

function closeUrpssv(btn) {
    jQuery(btn).closest('.video-modal-video-container-ssv').find('.user-registration-popup-ssv').fadeOut(500);
    jQuery('.video-modal-fade-ssv').hide();
}
...
function volumepipssv(btn) {
    if (jQuery('.video-pip-ssv video').prop('muted')) {
        jQuery('.video-pip-volume-ssv img').attr('src', 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/volume-up-fill.webp');
        jQuery('.video-pip-ssv video').prop('muted', false);

        ssv_globalMute = false;
    } else {
        jQuery('.video-pip-volume-ssv img').attr('src', 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/volume-mute-fill.webp');
...
function likeVideossv(btn, videoId, designerId) {
    ...
    jQuery(btn).find('img').attr('src', 'https://apigoswirl.com/short_video/assets/images/goswirl-webp/heart-fill-2.webp');
    SWIRLHeartFlawsssv(elm);
    SWIRLHeartFlawsssv(elm);
