...
let ssv_baseURL =
  ssv_mode === "Live"
    ? "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v12/swirl-style-v86.min.css"
    : "";
...
if (
  ...
  ["keune", "file:"].some((el) => window.location.origin.includes(el))
) {
  jqTag = document.createElement("script");
  jqTag.rel = "text/javascript";
...
// Shredskinz
let shredskinzReady = false;
let shredskinzCount = 0;
if (window.location.origin.includes("shredskinz") && window.location.href.includes("/products")) {
  console.log('Shredskinz PDP Fix');
  let shredskinzWait = setInterval(() => {
    shredskinzCount++;
    if (
      document.querySelector(
        'script[src="https://cdn.elements.amazon/elements/amazon-product-rating/v1.0/web-component.shopify.js"]'
      ) != null
    ) {
      setTimeout(() => { shredskinzReady = true; }, 500);
      clearInterval(shredskinzWait);
    } else {
      if (shredskinzCount > 10) { shredskinzReady = true; }
    }
  }, 500);
} else {
  shredskinzReady = true;
}

// check all loaded or not - then execute ssv
let SSVScriptsLOADED = setInterval(() => {
  if (JSLOAD1 && JSLOAD2 && shredskinzReady) {
    executessv();
...
function executessv() {
  ...
  if (
    ...
    currentURL = ["itsherway", "file:"].some((el) =>
      window.location.origin.includes(el)
    )
      ? window.location.href
      : currentURL;
    currentURL =
      currentURL == "https://scarters.com/products/terminal"
        ? "https://scarters.com/products/terminalXXXXX"
        : currentURL;
          ...
          if (
            ...
            if (getCookie("_ssv_tooltip")) {
              if (window.innerWidth < 768)
                setCookie(
                  "_ssv_tooltip",
                  parseInt(getCookie("_ssv_tooltip")) + 1,
                  365
                );
            } else {
...
function imagekitDynamicssv(img, w) {
  w = ["lg.com", "file:"].some((el) => window.location.origin.includes(el))
    ? 120
    : w;
...
function generatessv(videos) {
  ...
  let insReview = ssv_brandCustomizations.brand_id == "236633479" ? `
  ...
  videos.forEach((video) => {
    ...
    if (!ssv_pip && !ssv_pdppipHideOnscreen) {
      ...
      let outerProduct = outerProductCheck
        ? `
          ...
          : "display: none !important;"
                    ...
                    <label class="dynamic-price-ssv-${outerProductFinal.product_id
        }" style="${ssv_brandCustomizations.product_price_status == "0"
          ? "display: none !important;"
          : ""
        } color:${ssv_brandCustomizations.dis_fk_color} !important;">${outerProductFinal.currencysymbols
        } ${formatCurrencyssv(
      ...
      onpageSlides += `
          ...
          : ""
        } class="carousel-video-ssv ${video.is_landscape == "0"
          ? "landscape-video-c-ssv"
          : "potrait-video-c-ssv"
        }" poster="${video.cover_image}" ${ssv_brandCustomizations["auto_play"] === "1" && i < 5 ? "autoplay" : ""
        } loop onplay="jQuery(this).next().hide();" onpause="jQuery(this).next().show();" playsinline="" preload="metadata" data-setup="{}" muted>
                        <source src="${autoplayVideo}" data-src="${video.cover_video}" type="video/mp4">
                    </video>
          ...
          : "00:00"
                    ...
                    ${outerProduct}
                    <p class="video-title-ssv" style="${ssv_brandCustomizations.video_title == "0" ||
          ssv_brandCustomizations.product_blog_img == "1" ||
          !outerProductCheck
          ? "display: none !important;"
          : ""
    ...
    if (typeof video.product[0] != "undefined") {
      if (Object.keys(video.product).length > 1) {
        ...
        video.product.forEach((product) => {
          ...
          let moredetails =
            ssv_storeType == "6"
              ? `
                        <a class="ontile-addtocart-ssv ga-tracking-size-ssv" data-ssv-product-id="${product.sku_code
              }" onclick="jQuery(this).parents('.frommoredetail').click();" style="background: none !important; border: 2px solid ${ssv_brandCustomizations.front_color_add_to_cart_btn
                ...
                : ""
              } font-size: 12px!important; font-weight: 800!important;">Select Size</a>
                    `
              : `
                        <a class="ontile-addtocart-ssv ga-tracking-cart-ssv" data-ssv-product-id="${product.sku_code
              }" onclick="addtocartssv(${video.video_id}, ${product.product_id
              }, '${product.sku_code}', this, 1); CTAClicksssv(${product.product_id
              }, '${product.sku_code}', '${product.title}', '${product.image
              }', '${product.url}', ${video.designer_id}, ${video.video_id
            ...
            ssv_brandCustomizations.cta_on_tile == "1"
              ? `
                        <div class="ontile-cta-ssv">
                            ${moredetails}
                            <a class="ontile-buynow-ssv ga-tracking-buy-ssv" data-ssv-product-id="${product.sku_code
              }" href="${product.url}" onclick="CTAClicksssv(${product.product_id
              }, '${product.sku_code}', '${product.title}', '${product.image
              }', '${product.url}', ${video.designer_id}, ${video.video_id
          ...
          productTile += `
              ...
              : ""
                                        ...
                                        <p>${product.title}</p>
                                        ${desc}
                                        <label class="dynamic-price-ssv-${product.product_id
            }" style="${ssv_brandCustomizations.product_price_status == "0"
              ? "display: none !important;"
              : ""
            } color:${ssv_brandCustomizations.dis_fk_color} !important;">${product.currencysymbols
            } ${formatCurrencyssv(product.discount_price)} <strike style="${parseFloat(product.price) > parseFloat(product.discount_price)
              ? ""
              : "display: none;"
            } color:${ssv_brandCustomizations.mrp_fk_color} !important;">${product.currencysymbols
            } ${formatCurrencyssv(product.price)}</strike>
          ...
          let ptClick =
            ssv_brandCustomizations.buy_now == "1"
              ? `onclick="window.location='${product.url}'; CTAClicksssv(${product.product_id}, '${product.sku_code}', '${product.title}', '${product.image}', '${product.url}', ${video.designer_id}, ${video.video_id}, '1');"`
              : "";
          let customDetail =
            ssv_brandCustomizations.brand_id == "236633588"
              ? `
                        <div class="product-custom-details-ssv">
                            <div class="product-cd-ssv">
                                <p>Gender</p>
                                <label>
                                    <img src="${product.product_gender == "Male"
                ? "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/webp/men-line.webp"
                : "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/webp/women-line.webp"
              }" alt="Clock">
          ...
          pb += `
                        <div class="video-modal-product-block-product-ssv video-modal-product-block-product-multi-ssv" style="transform: translateX(110%);">
                            ...
                            <div class="product-middle-pb-ssv">                            
                                <div class="product-detail-pb-ssv" style="${ssv_brandCustomizations.buy_now == "1"
              ? "cursor: pointer;"
              : ""
            }" ${ptClick}>
              ...
              : product.image
                                    ...
                                    <div class="product-detail-block-pb-ssv">
                                        <p>${product.title}</p>
                                        <section style="display: none !important;">${product.desription
            }</section>
                                        <label class="dynamic-price-up-ssv dynamic-price-ssv-${product.product_id
            }" style="${ssv_brandCustomizations.product_price_status == "0"
              ? "display: none !important;"
              : ""
            } color:${ssv_brandCustomizations.dis_fk_color} !important;">${product.currencysymbols
              ...
              : ""
                                ...
                                <div class="product-desc-pb-ssv">
                                    ${insReview}
                                    <p class="product-desc-title-pb-ssv">${ssv_brandCustomizations.product_desc_title
            }</p>
                                    <section>${product.desription}</section>
                                    ${customDetail}                                                                       
                                </div>                                    
              ...
              : ""
                                ...
                                <div class="product-options-pb-ssv possv-${video.video_id
            }-${product.product_id
            }">                                    
                                </div>
                            ...
                            <div class="product-bottom-pb-ssv">
                                <button class="product-addtocart-pb-ssv ga-tracking-cart-ssv" data-ssv-product-id="${product.sku_code
            }" onclick="addtocartssv(${video.video_id}, ${product.product_id
            }, '${product.sku_code}', this); CTAClicksssv(${product.product_id
            }, '${product.sku_code}', '${product.title}', '${product.image}', '${product.url
              ...
              : ""
                                ...
                                <a href="${product.url
            }" class="ga-tracking-buy-ssv" data-ssv-product-id="${product.sku_code
            }" onclick="CTAClicksssv(${product.product_id}, '${product.sku_code
        ...
        let moredetails =
          ssv_storeType == "6"
            ? `
                    <a class="ontile-addtocart-ssv ga-tracking-size-ssv" data-ssv-product-id="${video.product[0].sku_code
            }" onclick="jQuery(this).parents('.frommoredetail').click();" style="background: none !important; border: 2px solid ${ssv_brandCustomizations.front_color_add_to_cart_btn
            } !important; color: ${ssv_brandCustomizations.front_color_add_to_cart_btn
            } !important; ${ssv_brandCustomizations.add_to_cart == "0"
              ? "display: none !important;"
              ...
              : ""
                ...
                `
            : `
                    <a class="ontile-addtocart-ssv ga-tracking-cart-ssv" data-ssv-product-id="${video.product[0].sku_code
            }" onclick="addtocartssv(${video.video_id}, ${video.product[0].product_id
            }, '${video.product[0].sku_code}', this, 1); CTAClicksssv(${video.product[0].product_id
            }, '${video.product[0].sku_code}', '${video.product[0].title
        ...
        let cta =
          ssv_brandCustomizations.cta_on_tile == "1"
            ? `
                    <div class="ontile-cta-ssv">
                        ${moredetails}
                        <a class="ontile-buynow-ssv ga-tracking-buy-ssv" data-ssv-product-id="${video.product[0].sku_code
            }" href="${video.product[0].url
            }" onclick="CTAClicksssv(${video.product[0].product_id}, '${video.product[0].sku_code
        ...
        productTile = `
            ...
            : ""
                                ...
                                ${desc}
                                <label class="dynamic-price-ssv-${video.product[0].product_id
          }" style="${ssv_brandCustomizations.product_price_status == "0"
            ? "display: none !important;"
            : ""
          } color:${ssv_brandCustomizations.dis_fk_color} !important;">${video.product[0].currencysymbols
          } ${formatCurrencyssv(
          ...
          } ${formatCurrencyssv(video.product[0].price)}</strike>
        ...
        let ptClick2 =
          ssv_brandCustomizations.buy_now == "1"
            ? `onclick="window.location='${video.product[0].url}'; CTAClicksssv(${video.product[0].product_id}, '${video.product[0].sku_code}', '${video.product[0].title}', '${video.product[0].image}', '${video.product[0].url}', ${video.designer_id}, ${video.video_id}, '1');"`
            : "";
        let customDetail =
          ssv_brandCustomizations.brand_id == "236633588"
            ? `
                    <div class="product-custom-details-ssv">
                        <div class="product-cd-ssv">
                            <p>Gender</p>
                            <label>
                                <img src="${video.product[0].product_gender == "Male"
              ? "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/webp/men-line.webp"
              : "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/assets/images/webp/women-line.webp"
            }" alt="Clock">
                                ${video.product[0].product_gender}
                            </label>
                        </div>
                        <div class="product-cd-ssv">
                            <p>Duration</p>
        ...
        productList = `
            ...
            : video.product[0].image
                                ...
                                <div class="product-detail-block-pb-ssv">
                                    <p>${video.product[0].title}</p>
                                    <section style="display: none !important;">${video.product[0].desription
          }</section>
                                    <label class="dynamic-price-up-ssv dynamic-price-ssv-${video.product[0].product_id
          }" style="${ssv_brandCustomizations.product_price_status == "0"
            ? "display: none !important;"
            : ""
          } color:${ssv_brandCustomizations.dis_fk_color} !important;">${video.product[0].currencysymbols
            ...
            : ""
                        ...
                        <div class="product-bottom-pb-ssv">
                            <button class="product-addtocart-pb-ssv ga-tracking-cart-ssv" data-ssv-product-id="${video.product[0].sku_code
          }" onclick="addtocartssv(${video.video_id}, ${video.product[0].product_id
          }, '${video.product[0].sku_code}', this); CTAClicksssv(${video.product[0].product_id
          }, '${video.product[0].sku_code}', '${video.product[0].title}', '${video.product[0].image
          }', '${video.product[0].url}', ${video.designer_id}, ${video.video_id
          }, '2');" style="background: none !important; border: 2px solid ${ssv_brandCustomizations.front_color_add_to_cart_btn
          } !important; color: ${ssv_brandCustomizations.front_color_add_to_cart_btn
          } !important; ${ssv_brandCustomizations.add_to_cart == "0"
            ? "display: none !important;"
          ...
          }" class="ga-tracking-buy-ssv" data-ssv-product-id="${video.product[0].sku_code
          }" onclick="CTAClicksssv(${video.product[0].product_id}, '${video.product[0].sku_code
          }', '${video.product[0].title}', '${video.product[0].image}', '${video.product[0].url
      ...
      buynowBtn =
        video.product_link && ssv_brandCustomizations.buy_now == "1"
          ...
          }" onclick="CTAClicksssv(0, '', '', '', '', ${video.designer_id
          }, ${video.video_id}, '1');">
    ...
    let shareURLssv =
      window.location.href.includes("?") &&
        !window.location.href.includes("ssv=")
        ? window.location.href + "&ssv=" + window.btoa(video.video_id)
        : ssv_store + "?ssv=" + window.btoa(video.video_id);
    modalSlides += `
            <div class="swiper-slide ${typeof video.product[0] == "undefined" ? "no-product-ssv" : ""
                ...
                <div class="video-modal-video-container-ssv">
                    ...
                    <video class="${video.is_landscape == "0"
        ? "landscape-video-ssv"
        : "potrait-video-ssv"
      }" id="modalVideossv-${video.video_id
                    ...
                    <p class="video-title-modal-ssv" style="${ssv_brandCustomizations.video_title == "0"
        ? "display: none !important;"
        : ""
      }" title="${video.video_title}">
                        ${video.video_title}
        ...
        : ""
                    ...
                    <div class="video-model-marquee-ssv" style="${ssv_brandCustomizations.marque_txt == "0"
        ? "display: none !important;"
        : ""
      }" onanimationend="hideMarqueessv(this)">
                    ...
                    <div class="video-modal-share-modal-ssv">
                        ...
                        <a onclick="copyEmailEmbedssv('${shareURLssv}', '${video.cover_image
      ...
      }</b>, Coustomer executive <span onclick="jQuery(this).closest('.chat-history-ssv').hide();">&#10006;</span></p>
  ...
  let pipPositionCSS = ["swtantra.com", "file:"].some((el) =>
    window.location.origin.includes(el)
  )
    ? `
        .video-pip-ssv {
  ...
  let ssvSlideCSS = ["shopmulmul.com", "file:"].some((el) =>
    window.location.origin.includes(el)
  )
    ? `
  ...
  videos.forEach((video) => {
    ...
    if (
      typeof video.product[0] != "undefined" &&
      ssv_storeType == "6" &&
      ssv_brandCustomizations.add_to_cart == "1"
    ) {
      video.product.forEach((product) => {
        var settings = {
          url:
            "https://api.omuni.com/d/api/productSearch/albl/" +
            product.sku_code,
          method: "GET",
          timeout: 0,
          headers: {
          ...
          if (typeof response.data != "undefined") {
            if (Object.keys(response.data.skus).length) {
              ...
              response.data.skus.forEach(function (sku, s) {
                optionsAppend += `                                    
                                    <option value="${sku.skuId}" ${sku.quantity == 0 ? "disabled" : ""
                  } data-p-mrp="${sku.mrp}" data-p-selling="${sku.sellingPrice
                  }">${sku.size}</option>                                    
                                `;
              ...
              jQuery(`.possv-${video.video_id}-${product.product_id}`).append(
                optionsAppend
              );
              jQuery(`.possv-${video.video_id}-${product.product_id}`).css(
                "display",
                "flex"
              );
              ...
              $(`.swiper-ssv-c .dynamic-price-ssv-${product.product_id}`).html(`
                                ${product.currencysymbols} ${formatCurrencyssv(
                response.data.skus[i].sellingPrice
              )} 
                                <strike style="${parseFloat(response.data.skus[i].mrp) >
                  parseFloat(response.data.skus[i].sellingPrice)
                  ? ""
                  : "display: none;"
                } color:${ssv_brandCustomizations.mrp_fk_color
                } !important;">${product.currencysymbols} ${formatCurrencyssv(
                  response.data.skus[i].mrp
                )}</strike>
                                <br>
                                <span style="${parseFloat(response.data.skus[i].mrp) >
                  parseFloat(response.data.skus[i].sellingPrice)
                  ? ""
                  : "display: none;"
                } color:${ssv_brandCustomizations.off_fk_color
                } !important; background:${ssv_brandCustomizations.off_bk_color
                } !important;">${parseFloat(response.data.skus[i].mrp) >
                  parseFloat(response.data.skus[i].sellingPrice)
                  ? Math.round(
                    ((response.data.skus[i].mrp -
                      response.data.skus[i].sellingPrice) *
      ...
      video.product.forEach((product) => {
        if (typeof product.pReviews != "undefined") {
          if (typeof product.pReviews[0] != "undefined") {
            ...
            totalRating = totalRating
              ? Math.round(totalRating / Object.keys(product.pReviews).length)
              : totalRating;

            let rappend =
              Object.keys(product.pReviews).length > 4
                ? `
                            <p class="product-rating-title-ssv">${totalRating} <i class="starssv"></i> (${Object.keys(product.pReviews).length
                } reviews)
                                <a class="product-rating-all-btn-ssv" onclick="openRatingssv(this);">See all</a>
                            </p>
                            <div class="product-rating-comments-ssv">
            ...
            let cappend = `
                            <div class="product-rating-all-title-ssv">
                                <p>
                                    ...
                                    (${Object.keys(product.pReviews).length
              } reviews)                                    
                                </p>
                            </div>
                            <div class="product-rating-all-comments-ssv">
            ...
            jQuery(`.prassv-${video.video_id}-${product.product_id}`).append(
              cappend
            );
          }
        }
    ...
    if (ssv_brandCustomizations.brand_id == "236633479") {
      video.product.forEach((product) => {
        if (
          ...
          fetch(
            `https://stg.api.bazaarvoice.com/data/reviews.json?apiversion=5.4&passkey=ca0JdCvzHhKIZyVfUykPxWVxDv5xwei9IwB4QOo0gdoGI&Filter=ProductId%3A${product.brand_product_id}&Limit=25`,
            options
          )
            ...
            .then((response) => {
              if (response.TotalResults > 0) {
                ...
                totalRating = totalRating
                  ? Math.round(totalRating / Object.keys(reviews).length)
                  : totalRating;

                let rappend =
                  Object.keys(reviews).length > 4
                    ? `
                                    <p class="product-rating-title-ssv">${totalRating} <i class="starssv"></i> (${Object.keys(reviews).length
                    } reviews)
                                        <a class="product-rating-all-btn-ssv" onclick="openRatingssv(this);">See all</a>
                                    </p>
                                    <div class="product-rating-comments-ssv">
    ...
    ssv_swiper = new Swiper(".swiper-ssv-c", {
      ...
      on: {
        init: function () {
          ...
          // landscape video height
          setTimeout(() => {
            carouselVideoHeightssv();
          }, 500);
        },
        slideChange: function () {
          if (ssv_brandCustomizations.auto_play == '1') {
            this.slides.forEach(s => {
              let vdo = s.querySelector('video');
              let sourc = vdo.querySelector('source');
              if (sourc.getAttribute('src') == "") {
                sourc.setAttribute('src', sourc.dataset.src);
                vdo.load();
                vdo.play();
              }
            });
  ...
  ssv_swiper_modal.on("slideChange", function () {
    ...
    playWithPromisessv(videoPlayer);
  });
...
function carouselVideoHeightssv() {
  ...
  $(".landscape-video-c-ssv").height(maxHeight);
}

function modalVideoHeightssv() {
  if (window.innerWidth > 768) {
    ...
    $(".landscape-video-ssv").css(
      "cssText",
      "width:" + maxWidth + "px !important"
    );
  } else {
    $(".landscape-video-ssv").css("width", "auto");
  }
}
...
function updatePriceBySKUssv(drp, cSymbol) {
  ...
  $(drp)
    .parents(".video-modal-product-block-product-ssv")
    .find(".dynamic-price-up-ssv").html(`
        ${cSymbol}${selling} 
        <strike style="${parseFloat(mrp) > parseFloat(selling) ? "" : "display: none;"
      } color:${ssv_brandCustomizations.mrp_fk_color
      } !important;">${cSymbol}${mrp}</strike>        
        <span style="${parseFloat(mrp) > parseFloat(selling) ? "" : "display: none;"
      } color:${ssv_brandCustomizations.off_fk_color
      } !important; background:${ssv_brandCustomizations.off_bk_color
      } !important;">${parseFloat(mrp) > parseFloat(selling)
        ? Math.round(((mrp - selling) * 100) / mrp)
        : ""
      }% OFF</span>
    `);
}
  ...
  jQuery(swpr + " .swiper-slide").each(function () {
    ...
    if (jQuery(this).prev().length)
      lpp = jQuery(this)
        ...
        .find("video")
        .attr("class")
        .includes("landscape-video-ssv")
        ? "landscape-v-image-ssv"
        : "potrait-v-image-ssv";
    ...
    jQuery(this).prepend(
      jQuery(this).prev().length
        ...
        '" class="video-modal-np-img-ssv p-img-ssv ' +
        lpp +
        '" onclick="jQuery(`.video-modal-ssv:visible`).find(`.swiper-button-prev-ssv`).click();">'
        : ""
    );
    jQuery(this).prepend(
      jQuery(this).next().length
        ...
        '" class="video-modal-np-img-ssv n-img-ssv ' +
        lpn +
        '" onclick="jQuery(`.video-modal-ssv:visible`).find(`.swiper-button-next-ssv`).click();">'
        : ""
    );
  });
}
...
function playssv(slideno) {
  ...
  jQuery(".swiper-ssv-m .swiper-slide video").each(function () {
    if (jQuery(this).find("source").attr("src") == "") {
      jQuery(this)
        .find("source")
        .attr("src", jQuery(this).find("source").attr("data-src"));
      jQuery(this)[0].load();
    }
  });
...
function showMarqueessv(player) {
  setTimeout(() => {
    if (
      !jQuery(player)
        .parent(".video-modal-video-container-ssv")
        .find(".video-model-marquee-ssv")
        .is(":visible")
    ) {
      var marquee = jQuery(player)
        .parent(".video-modal-video-container-ssv")
        .find(".video-model-marquee-ssv span")[0];
      var mContainer = jQuery(player)
        .parent(".video-modal-video-container-ssv")
        .find(".video-model-marquee-ssv")[0];
...
function closessv() {
  ...
  const updatedData = analyticsData?.length
    ? analyticsData
      .filter(
        (i) =>
          i.total_views !== 0 && i.unique_views !== 0 && i.watch_time !== 0
      )
      .map((i) => {
        i.video_id = i.id.replace(substring_to_remove, "");
        i.id = i.id.replace(substring_to_remove, "");
        return i;
      })
    : [];
  if (
...
function openProductssv(e, btn) {
  if (
    e.target.className.includes("ontile-addtocart-ssv") ||
    e.target.className.includes("ontile-buynow-ssv")
  )
    return;
...
function updateProgressbarssv(player) {
  ...
  if (ssv_brandCustomizations.marque_txt == "1") {
    ...
    if (
      (player.currentTime >= halfwayPoint &&
        player.currentTime <= halfwayBeetween) ||
...
function playpipssv(v = "", p = "") {
  ...
  if (video) {
    ...
    videoPlayer.onloadstart = function () {
      ...
      if (playPromise !== undefined) {
        playPromise.then((_) => {
          let vId = jQuery(`source[data-src='${video}']`).parent('video').attr('id').split('-')[1];
          videoPlayer.id = vId;
          initializeVideoData(vId, video);
        }).catch((error) => { });
      }
    };
...
function closepipssv(analytics = false) {
  // Pause video
  ...
  jQuery
    ...
    .done(function (response) {
      ...
      if (ssv_brandCustomizations.cart_count_class && updateWeb) {
        jQuery(`.${ssv_brandCustomizations.cart_count_class}`).html(
          response.item_count
        );
      }
    });
...
function addtocartssv(videoId, productId, skuCode, btn, qty = 0) {
  if (ssv_storeType == "1" || ssv_storeType == "2")
  ...
  if (ssv_storeType == "6") {
    ...
    skuCode = qty
      ? jQuery(
        `.possv-${videoId}-${productId} select[name="posssv-1"] option:not([disabled]):first`
      ).attr("value")
      : jQuery(`.possv-${videoId}-${productId} select[name="posssv-1"]`).val();
...
window.addEventListener("beforeunload", () => {
  ...
  const substring_to_remove = "modalVideossv-"; // Define the substring to remove
