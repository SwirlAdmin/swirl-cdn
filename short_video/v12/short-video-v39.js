...
let ssv_baseURL = ssv_mode === 'Live' ? 'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v12/swirl-style-v29.min.css' : '';
...
function generatessv(videos) {
    ...
    ssv_brandCustomizations.view_cart = '1';
                    ...
                    pb += `
                        <div class="video-modal-product-block-product-ssv video-modal-product-block-product-multi-ssv" style="transform: translateX(110%);">
                            ...
                            <div class="product-middle-pb-ssv">                            
                                ...
                                <div class="product-options-pb-ssv possv-${product.product_id}">                                    
                                </div>
                ...
                productList = `
                    <div class="video-modal-product-block-product-ssv">
                        ...
                        <div class="product-middle-pb-ssv">                            
                            ...
                            <div class="product-options-pb-ssv possv-${video.product[0].product_id}">
                            </div>
                ...
                $.ajax(settings).done(function (response) {
                    ...
                    if (typeof response.data != 'undefined') {
                        if (Object.keys(response.data.skus).length) {
                            ...
                            jQuery(`.possv-${product.product_id}`).append(optionsAppend);
                            jQuery(`.possv-${product.product_id}`).show();
                        }
                    }
                });
            });
...
function addtocartssv(videoId, productId, skuCode, btn, qty = 0) {
    ...
    if (ssv_storeType == '1') {  // Shopify        
        ...
        skuCode = qty ? jQuery(`.possv-${productId} select[name="posssv-1"] option:not([disabled]):first`).attr('value') : jQuery(`.possv-${productId} select[name="posssv-1"]`).val();
        ...
        if (getCookie('accessToken')) { // logged in              
            ...
            var settings = {
                ...
                "headers": {
                    ...
                    "Content-Type": "application/json",
                    "Bbversion": "v2",
                    "Correlationid": "4d0cc81b-eafa-49a9-a3b9-ef4754e2a297",
            ...
            jQuery.ajax(settings).done(function (response) {
                if (response.status == true) {
                    ...
                    // website Count
                    if (ssv_brandCustomizations.cart_count_class) {
                        $(`.${ssv_brandCustomizations.cart_count_class}`).html(response.data.bagTotalItemCount);
                    }
                } else {
            ...
            // website Count
            if (ssv_brandCustomizations.cart_count_class) {
                $(`.${ssv_brandCustomizations.cart_count_class}`).html(bagC);
            }
        }
