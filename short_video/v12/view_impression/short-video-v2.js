...
function generatessv(videos) {
  ...
  videos.forEach((video) => {
    ...
    if (!ssv_pip && !ssv_pdppipHideOnscreen) {
      ...
      onpageSlides += `
          ...
          : ""
        } class="carousel-video-ssv ${video.is_landscape == '0' ? 'landscape-video-c-ssv' : 'potrait-video-c-ssv'}" poster="${video.cover_image}" ${ssv_brandCustomizations["auto_play"] === "1" && i < 5 ? "autoplay" : ""
    ...
    modalSlides += `
            <div class="swiper-slide ${typeof video.product[0] == "undefined" ? "no-product-ssv" : ""
                ...
                <div class="video-modal-video-container-ssv">
                    ...
                    <video class="${video.is_landscape == '0' ? 'landscape-video-ssv' : 'potrait-video-ssv'}" id="modalVideossv-${video.video_id
  ...
  let pipPositionCSS = ["swtantra.com", "justhuman.co.in", "file:"].some((el) =>
    window.location.origin.includes(el)
  )
    ? `
        .video-pip-ssv {
...
function carouselVideoHeightssv() {
  var maxHeight = Math.max.apply(null, $(".potrait-video-c-ssv").map(function () {
    return $(this).height();
  }).get());
...
function modalVideoHeightssv() {
  if (window.innerWidth > 768) {
    var maxWidth = Math.max.apply(null, $(".potrait-video-ssv").map(function () {
      return $(this).width();
    }).get());
    $(".landscape-video-ssv").css('cssText', 'width:' + maxWidth + 'px !important');
  } else {
...
function updatePreviewThumbsssv(swpr) {
  jQuery(swpr + ' .swiper-slide').each(function () {
    var lpn = lpp = '';
    if (jQuery(this).prev().length) lpp = jQuery(this).prev().find('video').attr('class').includes('landscape-video-ssv') ? 'landscape-v-image-ssv' : 'potrait-v-image-ssv';
    if (jQuery(this).next().length) lpn = jQuery(this).next().find('video').attr('class').includes('landscape-video-ssv') ? 'landscape-v-image-ssv' : 'potrait-v-image-ssv';

    jQuery(this).prepend(jQuery(this).prev().length ? '<img src="' + jQuery(this).prev().find('video').attr('poster') + '" class="video-modal-np-img-ssv p-img-ssv ' + lpp + '" onclick="jQuery(`.video-modal-ssv:visible`).find(`.swiper-button-prev-ssv`).click();">' : '');
    jQuery(this).prepend(jQuery(this).next().length ? '<img src="' + jQuery(this).next().find('video').attr('poster') + '" class="video-modal-np-img-ssv n-img-ssv ' + lpn + '" onclick="jQuery(`.video-modal-ssv:visible`).find(`.swiper-button-next-ssv`).click();">' : '');
  })
}
