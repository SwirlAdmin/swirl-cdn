...
let ssv_pdpapiURL = "https://goswirl.info:4000/api/getMultipleProductVideoWithSearchAndFilter"; // bigleap.live  ,  api.goswirl.live
...
function executessv() {
    ...
    if (document.querySelector("#swirl-short-videos") != null || localStorage.getItem("_ssv_pip")) {
        ...
        let finalAPICall = ssv_pdppip ? ssv_pdpapiURL : localStorage.getItem("filtered") == "true" ? "https://goswirl.info:4000/api/videolistingV4WithSearchAndFilter" : ssv_apiURL;
...
async function generatessv(videos) {
    ...
    //     checkbox.addEventListener("change", function () {
    //         if (checkbox.checked) {
    //             trackMixPanel("filter_ski_level", {
    //                 App_Data: {
    //                     filterName: `${checkbox.value}`,
    //                 },
    //             });
    ...
    jQuery.ajax({
        url: `https://goswirl.info:4000/api/getFilterInfoByBrandId?userCode=${ssv_storeCode}`,
        type: "GET",
        dataType: "json",
        success: function (response) {
            // Handle success response
            // filterDataFromAPi = response.filterInfo
            // console.log(filterDataFromAPi);
            ...
            if (multiselectData.length > 0) {
                ...
                function updateSelectedValues(filterKey, selectedValues) {
                    globalData[filterKey] = selectedValues;
    ...
    //         "filterValues": ["Beginner", "Intermediate", "Advanced"],
    //         "status": true
    //     }
    // ];
    ...
    // const container3 = document.getElementById("multiSelectContainer3");
    // const options3 = ["R43", "Eclipse"];
    // let selectedValues3 = [];
    ...
    // // Check if selected values are stored in local storage
    // const storedValues3 = localStorage.getItem("productFilter");
    // if (storedValues3) {
    //     selectedValues3 = JSON.parse(storedValues3);
    // }

    // options3.forEach((option, index) => {
    //     const checkbox = document.createElement("input");
    //     checkbox.type = "checkbox";
    //     checkbox.id = `checkbox3${index}`;
    //     checkbox.value = option;
    //     checkbox.className = "checkbox_with_btn"
    ...
    //     checkbox.addEventListener("change", function () {
    //         if (checkbox.checked) {
    //             trackMixPanel("filter_productName", {
    //                 App_Data: {
    //                     filterName: `${checkbox.value}`,
    //                 },
    //             });
    ...
    //         } else {
    //             selectedValues3 = selectedValues3.filter(value => value !== checkbox.value);
    //         }

    //         // Update local storage
    //         localStorage.setItem("productFilter", JSON.stringify(selectedValues3));

    //         updateButtonStyle(checkbox, label);
    //         updateSelectedProductValues();
    //     });
    //     // Set initial checkbox state based on stored values
    //     checkbox.checked = selectedValues3.includes(checkbox.value);
    //     updateButtonStyle(checkbox, label);
    // });

    // async function updateSelectedProductValues() {
    //     globalData.productFilter = selectedValues3;
    //     if (globalData.ageGroups.length > 0 || globalData.skieLevel.length > 0 || globalData.address.state !== undefined || globalData.sortBy.length > 0 || globalData.productFilter.length > 0) {
    //         filtered = true;
    //         localStorage.setItem("filtered", true)
    //         await executessv();
