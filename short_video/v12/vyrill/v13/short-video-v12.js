...
(() => {
    ...
    jqTag.href = window.location.href.includes('short-videos-carousel/v13') ?
        'style.css' :
        document.querySelector('script[src="https://livevideoshopping.in/swirl-embed/short-videos-carousel/v13.2/swirl-embed.js"]') ?
            'https://livevideoshopping.in/swirl-embed/short-videos-carousel/v13.2/style.css' :
            'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v12/vyrill/v13/short-video-v10.min.css';
...
function initssv13() {
    // check PIP 
    if (localStorage.getItem('ssv13_pip') && !document.querySelector('#swirl-short-videos')) {
        ssv13_pipMode = true;
        let cDiv = document.createElement('div');
        cDiv.style.display = 'none';
        cDiv.innerHTML = `
            <div id="swirl-short-videos" data-code="${localStorage.getItem('ssv13_storeCode')}" data-wt="" data-vlimit="${localStorage.getItem('ssv13_videoPerPage')}"></div>
        `;
        document.body.appendChild(cDiv);
    }

    if (!document.querySelector('#swirl-short-videos')) {
        ...
        `;
        document.querySelector('#swirl-short-videos').insertAdjacentElement('beforebegin', lDiv);
    }

    ssv13_masterDiv = document.querySelector('#swirl-short-videos');
    ...
    ssv13_menuTheme = typeof ssv13_masterDiv.dataset.menu != 'undefined' ? ssv13_masterDiv.dataset.menu : '0';
    ...
    let playlistcode = ssv13_masterDiv.dataset.playlist
    ...
    if (ssv13_storeCode || ssv13_pipMode) {
        fetch(`https://liveshopping.asia/getVideosWIthplaylistfilters?user=${ssv13_storeCode}&playlist=${playlistcode} `, requestOptions)
            ...
            .then(result => {
                ...
                localStorage.setItem("videoDataForFilters", JSON.stringify(data.data.video))
                ssv13_brandCustomizations = data.data.customization;
                ssv13_responseData = data;
                localStorage.setItem('ssv13_responseData', result);

                // testing
                ssv13_brandCustomizations.pageView = 1; // 1 = Carousel, 2 = Grid                
...
function getVideoIDsFromVideoArray(videos) {
    const videoIds = videos.map((el) => {
        return el.video_id
    })
    return videoIds
}
...
async function handleSearchVideos(el) {
    clearTimeout(debounceTimer);

    debounceTimer = setTimeout(async () => {
        sQuery = el.value.trim()
        ...
        fetch(`https://liveshopping.asia/getV13VideosWIthCustomSearchQueryByVideoId?brandId=${ssv13_brandCustomizations.brand_id}&searchQuery=${str}&videoIds=${getVideoIDsFromVideoArray(JSON.parse(localStorage.getItem('videoDataForFilters')))}`, requestOptions)
            .then(response => response.text())
            .then((res) => {
                const videos = JSON.parse(res).data.video;
                if (videos.length > 0) {
                    ssv13_swiper = [];
                    ssv13_swiper_modal = [];
                    ...
                    generatessv13(videos, videos.length, 'F', ssv13_masterDiv, 0);
                    // localStorage.setItem("videoDataForFilters", JSON.stringify(videos));
                } else {
                    localStorage.setItem("videoDataForFilters", JSON.stringify([]));
                    ...
                    ssv13_swiper_product = [];
                    ssv13_swiper_review = [];
                    // localStorage.setItem("videoDataForFilters", localStorage.getItem('ssv13_responseData'))
                    document.querySelectorAll('.JS-video-modal-div-ssv13').forEach(d => {
                        d.remove();
                    });

                    generatessv13(ssv13_responseData.data.video, ssv13_responseData.data.video_total, 'D', ssv13_masterDiv, 0);
                    ssv13_masterDiv.innerHTML = `<p style="text-align: center;color: red; margin-top:-20px;padding:10px;font-size: 16px;">No matching videos for applied filters</p>` + ssv13_masterDiv.innerHTML
                }
                setTimeout(() => {
    ...
    }, 600); // Adjust debounce delay time (in milliseconds) as needed
}
function generateFilterssv13(filter, product, target, no) {
    ...
    // Left
    filterHTML += `<div class="filters-left-ssv13" style='visibility:${Object.keys(filter).length ? 'visible' : 'hidden'}'>        
       
    `;
    if (Object.keys(filter).length) {
        let fi = 0;
        filterHTML += `<div class="filters-ssv13" onscroll="checkFilterssv13();">`;
        filter.forEach(f => {
            fi++;
            if (Object.keys(f.sub).length) {
                filterHTML += `
                    <div class="dropdown-ssv13">
                        <button type="button" onclick="toggleFilterDropssv13('dropdown-ssv13-${fi}')" class="dropbtn-ssv13">${f.f_name}
                            <span class="arrow-ssv13"></span>
                        </button>
                        <div id="dropdown-ssv13-${fi}" class="dropdown-content-ssv13">
                        <label class="custom-checkbox-ssv13" style="display: none !important;">
                            <input onchange="document.querySelector('#filters-master-form-submit-ssv13').click();" type="checkbox" class="select-all" onclick="toggleSelectAllFilterssv13('dropdown-ssv13-${fi}')">
                            Select all
                ...
                f.sub.forEach(sf => {
                    filterHTML += `
                        <label class="custom-checkbox-ssv13">
                            <input name="menuFilter[]" onchange="document.querySelector('#filters-master-form-submit-ssv13').click();" type="checkbox" class="option" value="${sf.video_id}" onclick="updateSelectAllFilterssv13('dropdown-ssv13-${fi}')">
                            ${sf.f_name}
                            <span class="checkmark-ssv13"></span>
                        </label>
                    `;
                ...
                filterHTML += `
                    <div class="dropdown-ssv13">
                        <button type="button" onclick="" class="dropbtn-ssv13">
                            <input name="${ssv13_menuTheme == '1' ? 'menuFilter' : 'menuFilter[]'}" onchange="document.querySelector('#filters-master-form-submit-ssv13').click();" style="position: absolute; opacity: 0; cursor: pointer; left: 0; width: 100%; height: 100%; margin: 0;" type="${ssv13_menuTheme == '1' ? 'radio' : 'checkbox'}" class="option" value="${f.video_id}">
                            ${f.f_name}
                        </button>                        
                    </div>                        
                `;
    ...
    filterHTML += `    <div class="custom-search-container" style="display: ${ssv13_brandCustomizations.search == 1 ? 'flex' : 'none'}">
                        ...
                        <input 
    type="text" 
    class="swirl_ssv_filter_input" 
    value="" 
    placeholder="Search" 
    onkeydown="if (event.key === 'Enter') { even
        t.preventDefault(); }" 
    oninput="handleSearchVideos(this)" 
  />
                        <i class="clear-search-icon" onclick="clearFilterForSearch()" >
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 9L12 12M12 12L15 15M12 12L9 15M12 12L15 9M12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12C21 16.9706 16.9706 21 12 21Z" stroke="#000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                            </svg>
                        </i>
                    </div>`

    // if (Object.keys(product).length) {
    //     filterHTML += `<div class="filters-select-product-ssv13"><select id="ssvProductsFilter" name="ssvProductsFilter" data-placeholder="Select Products" data-max="3" data-select-all="false" data-list-all="false" multiple data-ssvmulti-select>`;
    //     product.forEach(p => {
    //         filterHTML += `<option value="${p.product_id}">${p.product_title}${p.product_attr ? ' - ' + p.product_attr : ''}</option>`;
    //     });
    ...
    if (Object.keys(product).length) {
        new SSVMultiSelect('#ssvProductsFilter', {
            onChange: function () {
                ...
                document.querySelector('.ssvmulti-select-header').click();
                // expandFilterSearchssv13();

                document.querySelector('.ssvmulti-select-options').scrollTop = 0;
    ...
    if (Object.keys(filter).length) {
        window.onclick = function (event) {
            if (!event.target.matches('.dropbtn-ssv13') && !event.target.matches('#filters-master-form-submit-ssv13') && !event.target.closest('.dropdown-content-ssv13')) {
...
function expandFilterSearchssv13() {
    let fLBtn = document.querySelector('.filters-filter-btn-ssv13');
    let fRBtn = document.querySelector('.filters-search-btn-ssv13');
    let fL = document.querySelector('.filters-left-ssv13');
    let fR = document.querySelector('.filters-right-ssv13');
    ...
    if (fRBtn.style.display == 'block') {
        ...
        setTimeout(() => {
            fR.querySelector('.filters-select-product-ssv13').style.display = 'block';
        ...
        fR.querySelector('.filters-select-product-ssv13').style.display = 'none';
    ...
    setTimeout(() => {
        const ele = document.querySelector('.swirl_ssv_filter_input');
...
document.addEventListener('keydown', function (event) {
    ...
    // if (event.ctrlKey && event.key === 'f') {
    //     event.preventDefault(); // Prevent the default browser behavior (if desired)
...
let fVissible = [];
function checkFilterssv13() {
    let fHidden = 0;
    document.querySelectorAll('.dropbtn-ssv13').forEach(e => {
        if (!isFilterVisiblessv13(e, document.querySelector('.filters-ssv13'))) {
    ...
    let fCount = document.querySelector('.filter-plus-count-ssv13');
    if (fHidden > 0) {
...
function filterVideossv13() {
    ...
    let vIds = '';
    if (ssv13_menuTheme == '1') {
        vIds = document.querySelector('input[name="menuFilter"]:checked').value;
    } else {
        let filters = document.getElementsByName('menuFilter[]');
        if (filters.length) {
    ...
    if (vIds || pIds) {
        ...
        fetch(`https://liveshopping.asia/getV13VideosWIthCustomSearchQueryByVideoId?brandId=${ssv13_brandCustomizations.brand_id}&searchQuery=${sQuery}&videoIds=${vIds}`, requestOptions).then(response => response.text()).then(result => {
            let data = JSON.parse(result);
...
function pageChangessv13(btn, page, vType) {
    ...
    if (vType == 'D') {
        if (page == '0') {
            ...
            fetch(`https://api.goswirl.live/index.php/ShortVideo/getNextDefaultVideosv13_2?brandId=${ssv13_brandCustomizations.brand_id}&limit=${ssv13_videoPerPage}&page=${ssv13_videoCurrentPage}`, requestOptions).then(response => response.text()).then(result => {
                let data = JSON.parse(result);
                if (Object.keys(data.data.video).length) {
                    ssv13_swiper = [];
                    ssv13_swiper_modal = [];
...
function scrollToDiv(ch, id) {
    // const targetDiv = document.getElementById(divId);
    // if (targetDiv) {
    //     setTimeout(() => {
    //         targetDiv.scrollIntoView({ behavior: 'smooth', inline: 'center' });
    //     }, 100);
    // } else {
    //     console.error(`Div with ID '${divId}' not found.`);
    // }

    let div = document.querySelector(`.swirl-short-videos-m-ssv13-${ssv13_currentModal} .video-modal-sl-thumbs-block-ssv13`);
    ...
    if (div && ch) {
        ...
        // console.error(`Container with ID '${containerId}' or child with ID '${childId}' not found.`);
        console.log('ERR')
    }
    // div.scrollLeft += (((n - i) * 35));
}
    ...
    videos.forEach((video, index) => {
        ...
        cSlides += `
            <div class="swiper-slide" onclick="openVideoModalssv13(${i}, ${no});">
                <video data-src="${video.cover_video}" data-srcf="${video.server_url}" data-id="${video.video_id}" class="carousel-video-ssv13 lazy-poster ${video.is_landscape == '0' ? 'JS-cvideo-L-ssv13' : 'JS-cvideo-P-ssv13'}" onplay="this.closest('.swiper-slide').querySelector('.carousel-video-play-ssv13').style.display='none';"
                    data-poster="${video.cover_image}" loop playsinline muted>
                ...
                ${vcBadge[Object.keys(video.product).length ? (video.product[0].product_tags ? video.product[0].product_tags : 0) : 0]}
                ${cProduct}
                ${cTitle}
            </div>
        `;
        ...
        mThumbs += i == 0 ? `
            <div class="video-modal-sl-thumb-toggler-ssv13" style="display: none !important;" onclick="toggleVideoThumbssv13(this)">
                <img class="lazy-load" data-src="${video.cover_image}" 
                    alt="Video thumbnail">
            </div>
            <div class="video-modal-sl-thumb-container-ssv13" id="galary_card_${index}" >
                <img  data-src="${video.cover_image}"  class="video-modal-sl-thumb-ssv13 CSS-img-thumb-ssv13 active-ssv13 lazy-load" onclick="changeModalVideossv13(${i})"
                     alt="Video thumbnail">
                <div class="CSS-img-thumb-ssv13" onclick="this.previousElementSibling.dispatchEvent(new Event('click'));"></div>
            </div>
        ` : `
            <div class="video-modal-sl-thumb-container-ssv13" id="galary_card_${index}">
                <img data-src="${video.cover_image}"  class="video-modal-sl-thumb-ssv13 CSS-img-thumb-ssv13 lazy-load" onclick="changeModalVideossv13(${i})"
                     alt="Video thumbnail">
                <div class="CSS-img-thumb-ssv13" onclick="this.previousElementSibling.dispatchEvent(new Event('click'));"></div>
            </div>
        `;
        ...
        mSlides += `
            <div class="swiper-slide swiper-slide-m">
                <div class="video-modal-sl-video-container-ssv ${!Object.keys(video.product).length ? 'no-products-slide-ssv13' : ''}">
                    <video id="ssv13-video-m-${video.video_id}" data-src="${video.server_url}" data-products="${Object.keys(video.product).length}" data-id="${video.video_id}" class="video-modal-sl-video-ssv13 lazy-poster ${video.is_landscape == '0' ? 'JS-mvideo-L-ssv13' : 'JS-mvideo-P-ssv13'}"
                    ...
                    <div class="video-modal-sl-thumb-toggler-ssv13" onclick="toggleVideoThumbssv13(this)">
                        <img data-src="${video.cover_image}" class="lazy-load"
                            alt="Video thumbnail">
                    </div>
    ...
    if (videoTotal > ssv13_videoPerPage) {
        ...
        if (totalPages < 5) {
            ...
            if (ssv13_videoCurrentPage < 4) {
                ...
                pagination += `<span>...</span>`;
                for (let pages = totalPages - 5; pages < totalPages; pages++) {
                    pagination += `<p onclick="pageChangessv13(this, '${pages}', '${videoType}')" class="${pages == ssv13_videoCurrentPage ? 'activePage' : ''}">${(pages + 1)}</p>`;
                }
            } else {
                pagination += `<p onclick="pageChangessv13(this, '${0}', '${videoType}')" class="${0 == ssv13_videoCurrentPage ? 'activePage' : ''}">1</p>`;
    ...
    ssv13_swiper_modal[no] = new Swiper(`.swiper-ssv13-m${no}`, {
        ...
        on: {
            ...
            slideChange: function () {
                ...
                scrollToDiv(document.querySelectorAll(`.swirl-short-videos-m-ssv13-${no} .video-modal-sl-thumb-ssv13`)[activeInd], activeInd);
...
function playModalVideossv13(slideNo) {
    ...
    if (ssv13_brandCustomizations.pip_mode == '1') {
        ...
        let cPlaylist = document.querySelector(`video[data-srcf="${vdo.src}"]`).closest('#swirl-short-videos').dataset.playlist;
...
const lazyLoadPosters = () => {
    const videos = document.querySelectorAll(".lazy-poster");
    ...
    videos.forEach((video) => {
        const observer = new IntersectionObserver(
            (entries, observer) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        // Set the poster image from data-poster
                        video.poster = video.dataset.poster;

                        observer.unobserve(video); // Stop observing once poster is set
                    }
                });
            },
