...
let ssv_baseURL =
  ssv_mode === "Live"
    ? "https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v12/vyrill/swirl-style-v76.min.css"
    : "";
let ssv_responseData = [];
let ssv_productIds = [];
let ssv_apiURL = window.location.href.includes("olliecraft")
  ? "https://api.goswirl.live/index.php/ShortVideo/videolistingV4"
  : "https://liveshopping.asia/videolistingV4";
let ssv_pdpapiURL =
  "https://liveshopping.asia/getMultipleProductVideoWithSearchAndFilter"; // bigleap.live  ,  api.goswirl.live
...
const mixPanelInit = (code) => {
  if (
    !(
      window.location.href.includes("olliecraft") ||
...
jqTag.rel = "stylesheet";
jqTag.href =
  "https://apigoswirl.com/short_video/v12/vyrill/swirl-filter.min.css";
document.body.insertBefore(jqTag, document.body.lastChild);
...
function executessv() {
  ...
  if (
    ...
    let finalAPICall = ssv_pdppip
      ? ssv_pdpapiURL
      : localStorage.getItem("filtered") == "true"
      ? window.location.href.includes("odrskis")
        ? "https://liveshopping.asia/videolistingV4WithSearchAndFilter"
        : "https://liveshopping.asia/videolistingV4WithSearchAndCustomFilter"
      : ssv_apiURL;
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          if (
            ...
            // if (!ssv_pdppip) {
            if (
              localStorage.getItem("filtered") === null ||
              localStorage.getItem("filtered") == "false"
            ) {
            ...
            if (
              ...
              generatessv(
                JSON.parse(localStorage.getItem("ssv_first_time_data")).swilrs
                  .video
              );
            }
          } else {
...
async function generatessv(videos) {
    ...
    videos.forEach(async (video, index) => {
      ...
      if (!ssv_pip && !ssv_pdppipHideOnscreen) {
        let outerProductCheck =
          (Array.isArray(video.product) && video.product.length > 0) ||
        ...
        let outerProduct = outerProductCheck
          ? `
                    <div class="product-on-carousel-ssv" ${
                      ssv_brandCustomizations.product_blog_img == "0"
                        ? 'style="display: none !important;"'
                        : ""
                    }>                                    
                        <img src="${
                          outerProductFinal.image.includes("imagekit")
                            ? imagekitDynamicssv(outerProductFinal.image, 80)
                            : outerProductFinal.image
                        }" alt="Product Image">                                        
                        
                        <h6 style="${
                          video.product.length > 1
                            ? ""
                            : "display: none !important;"
                        } background: ${
              ssv_brandCustomizations.bk_color_buy_btn
            } !important; color: ${
              ssv_brandCustomizations.front_color_buy_btn
            } !important;">${video.product.length}</h6>
                        <p>${outerProductFinal.title}</p>
                        <label style="${
                          ssv_brandCustomizations.product_price_status == "0"
                            ? "display: none !important;"
                            : ""
                        } color:${
              ssv_brandCustomizations.dis_fk_color
            } !important;">${outerProductFinal.currencysymbols}${
              outerProductFinal.discount_price
            } <strike style="${
              parseFloat(outerProductFinal.price) >
              parseFloat(outerProductFinal.discount_price)
                ? ""
                : "display: none;"
            } color:${ssv_brandCustomizations.mrp_fk_color} !important;">${
              outerProductFinal.currencysymbols
              ...
              ssv_brandCustomizations.off_fk_color
            } !important; background:${
              ssv_brandCustomizations.off_bk_color
            } !important;">${
              parseFloat(outerProductFinal.price) >
              parseFloat(outerProductFinal.discount_price)
                ? Math.round(
        ...
        onpageSlides += `
                    <div class="swiper-slide ms-${
                      video.video_id
                        ...
                        <video id="onpageVideossv-${video.video_id}" ${
          ssv_brandCustomizations.product_blog_img == "0" || !outerProductCheck
            ? 'style="margin-bottom: 0 !important;"'
            : ""
        } class="carousel-video-ssv" poster="${video.cover_image}" ${
          ssv_brandCustomizations["auto_play"] === "1" && i < 5
            ? "autoplay"
            : ""
                        ...
                        <img ${
                          ...
                          !outerProductCheck
                            ...
                            alt="Play icon" />  
                        <div class="video-views-count-top-ssv" ${
                          ssv_brandCustomizations.views === "0"
                            ? 'style="display: none !important;"'
                            : ""
                        }>
                        ...
                        <div class="video-timer-top-ssv" ${
                          ssv_brandCustomizations.time_sec === "0"
                            ? 'style="display: none !important;"'
                            : ""
                        }>
                            <p>${
        ...
        if (Object.keys(video.product).length > 1) {
            ...
            let cta =
              ssv_brandCustomizations.cta_on_tile == "1"
                ? `
                    ...
                    product.sku_code
                  }', this, 1); CTAClicksssv(${product.product_id}, '', '${
                    product.image
                  }', '${product.url}', ${video.designer_id}, ${
                    video.video_id
                  }, '2');" style="background: none !important; border: 2px solid ${
                    ssv_brandCustomizations.front_color_add_to_cart_btn
                  } !important; color: ${
                    ssv_brandCustomizations.front_color_add_to_cart_btn
                  } !important; ${
                    ssv_brandCustomizations.add_to_cart == "0" ||
                    ssv_brandCustomizations.brand_id != "236633565"
                      ? "display: none !important;"
                      : ""
                  } ${
                    ...
                    ssv_brandCustomizations.buy_now == "0"
                      ...
                      : ""
                            ...
                            <a class="ontile-buynow-ssv" href="${
                              product.url
                            }" onclick="CTAClicksssv(${
                    product.product_id
                  }, '', '${product.image}', '${product.url}', ${
                    video.designer_id
                  }, ${video.video_id}, '1');" style="background: ${
                    ssv_brandCustomizations.bk_color_buy_btn
                  } !important; color: ${
                    ssv_brandCustomizations.front_color_buy_btn
                  } !important; ${
            ...
            productTile += `
                        <div class="swiper-slide-pt">                        
                            <div class="video-modal-product-tile-ssv" onclick="openListProductssv(event, this, ${pi});">
                                <div class="product-info-pt-ssv" style="${
                                  ssv_brandCustomizations.product_tile_cta_btn ==
                                    "0" ||
                                  ssv_brandCustomizations.cta_on_tile == "1"
                                    ? "width: 100% !important;"
                                    : ""
                                }">
                                    <div class="product-image-pt-ssv" style="${
                                      ssv_brandCustomizations.mobile_product_tile_img ==
                                      "1"
                                        ? "display: block !important;"
                                        : ""
                                    }">
                                        <img alt="Product Image" src="${
                                          product.image.includes("imagekit")
                                            ? imagekitDynamicssv(
                                                product.image,
                                                80
                                              )
                                    ...
                                    <div class="product-detail-pt-ssv" style="${
                                      ssv_brandCustomizations.mobile_product_tile_img ==
                                      "1"
                                        ...
                                        ${desc}
                                        <label style="${
                                          ssv_brandCustomizations.product_price_status ==
                                          "0"
                                            ? "display: none !important;"
                                            : ""
                                        } color:${
              ssv_brandCustomizations.dis_fk_color
            } !important;">${product.currencysymbols}${
              ...
              parseFloat(product.price) > parseFloat(product.discount_price)
                ...
                : ""
                                ...
                                <div class="product-buy-btn-pt-ssv" style="${
                                  ssv_brandCustomizations.product_tile_cta_btn ==
                                    "0" ||
                                  ssv_brandCustomizations.cta_on_tile == "1"
                                    ? "display: none !important;"
            ...
            pb += `
              ...
              parseFloat(product.price) > parseFloat(product.discount_price)
                ...
                : ""
                                ...
                                <div class="product-quantity-pb-ssv" style="${
                                  ssv_storeType == "0" ||
                                  ssv_brandCustomizations.product_qty == "0"
                                    ...
                                    <div class="product-quantity-inp-pb-ssv">
                                        ...
                                        <input id="quantity_val-${
                                          product.brand_product_id
                                        }-${index}" class="quantity-inp-ssv" value="1" readonly></input>
                                        <button style="background: #eaeaea !important; color: #323232 !important;" onclick="changeQtyssv('plus', this);">+</button>
                                    </div>
                                </div>
                                <div class="product-rating-ssv prssv-${
                                  video.video_id
                                }-${product.product_id}"></div>
                                <div class="product-rating-all-ssv prassv-${
                                  video.video_id
                                }-${product.product_id}"></div>
                            </div>
                            <div class="product-bottom-pb-ssv">
                                <button class="product-addtocart-pb-ssv" onclick="addTocartForOdr(${
                                  video.product[0].brand_product_id
                                },this,${
              video.product[0].product_id
            },${index});CTAClicksssv(${product.product_id}, '', '${
              product.image
            }', '${product.url}', ${video.designer_id}, ${
              video.video_id
            }, '2');" style="background: none !important; border: 2px solid ${
              ...
              ssv_brandCustomizations.cust_cart_redirection_link
            }">
                                        <button class="product-cart-pb-ssv" style="background: none !important; border: 2px solid ${
                                          ssv_brandCustomizations.front_color_add_to_cart_btn
                                        } !important; color: ${
              ssv_brandCustomizations.front_color_add_to_cart_btn
            } !important; ${
              ...
              ssv_brandCustomizations.view_cart == "0"
                ...
                : ""
                                            ...
                                            <label class="cart-count-ssv" style="line-height: 20px !important; background: ${
                                              ssv_brandCustomizations.bk_color_buy_btn
                                            } !important; color: ${
              ssv_brandCustomizations.front_color_buy_btn
            } !important;">0</label>
              ...
              ? `
                  ...
                  ssv_brandCustomizations.buy_now == "0"
                    ...
                    : ""
                        ...
                        <a class="ontile-selectSize-ssv" onclick="jQuery(this).parents('.video-modal-product-tile-ssv').click();" style="display : ${
                          ssv_brandCustomizations.brand_id === "236633565"
                            ? ""
                            : "none"
                        } ">Select Size</a>
          ...
          productTile = `
                    <div class="video-modal-product-tile-ssv" onclick="openProductssv(event, this);">
                        <div class="product-info-pt-ssv" style="${
                          ssv_brandCustomizations.product_tile_cta_btn == "0" ||
                          ssv_brandCustomizations.cta_on_tile == "1"
                            ? "width: 100% !important;"
                            : ""
                        }">
                            <div class="product-image-pt-ssv" style="${
                              ...
                              "1"
                                ...
                                <img alt="Product Image" src="${
                                  video.product[0].image.includes("imagekit")
                                    ? imagekitDynamicssv(
                                        video.product[0].image,
                                        80
                                      )
            ...
            ssv_brandCustomizations.dis_fk_color
          } !important;">${video.product[0].currencysymbols}${
            video.product[0].discount_price
          } <strike style="${
            parseFloat(video.product[0].price) >
            parseFloat(video.product[0].discount_price)
              ? ""
              : "display: none;"
          } color:${ssv_brandCustomizations.mrp_fk_color} !important;">${
            video.product[0].currencysymbols
          ...
          productList = `
                    <div class="video-modal-product-block-product-ssv">
                        ...
                        <div class="product-middle-pb-ssv">                            
                            <div class="product-detail-pb-ssv">
                                <div class="product-image-pb-ssv">
                                    <img alt="Product Image" src="${
                                      video.product[0].image.includes(
                                        "imagekit"
                                      )
                                        ? imagekitDynamicssv(
                                            video.product[0].image,
            ...
            parseFloat(video.product[0].discount_price)
              ...
              : ""
                            ...
                            <div class="product-desc-pb-ssv">
                                <p class="product-desc-title-pb-ssv">Product Details</p>
                                <section style="font-size:14px !important">${
                                  video.product[0].desription
                                }</section>                                
                            </div>                            
                            <div class="selectSizeAndColorWrapper" style="${
                              video.product[0].brand_product_id == 6040740593834
                                ? "display:block;"
                                : "display:none;"
                            }">
                                <select id="select-size1-${
                                  video.product[0].product_id
                                }-${index}" onchange="handleSizeChange1(${
            video.product[0].product_id
          },${index})" style="padding: 10px; width:100%;">
                            ...
                            <div class="selectSizeAndColorWrapper" style="${
                              video.product[0].brand_product_id == 4733388685366
                                ? "display:flex"
                                : "display:none;"
                            }">
                                <select id="select-size2-${
                                  video.product[0].product_id
                                }-${index}" onchange="handleSizeChange2(${
            video.product[0].product_id
          },${index})" style="padding: 10px; width:100%;">
                                    <option value="" disabled selected>--Select Size--</option>       
                            ...
                            <div class="sizeGenderFitWrapper" style="border: ${
                              ssv_brandCustomizations.brand_id == "236633565"
                                ? "1px solid #eee"
                                : "none"
                            }">
                            <div style="border: 1px solid #eaeaea;padding:10px;background-color:#eaeaea; border-radius:10px;display: ${
                              ssv_brandCustomizations.brand_id == "236633565"
                                ? "block"
                                : "none"
                            };" >
                                <p style="cursor:pointer;text-decoration:underline; margin-bottom:5px" onclick="handleFindFitWrapper(${
                                  video.product[0].product_id
                                },${index})">Find Your Fit</p>
                                <div class="shoe-gender-wrapper" id="shoeGenderWrapper-${
                                  video.product[0].product_id
                                    ...
                                    <div class="shoe-gender-wrapper-row1">
                                        <div class="shoe-size-input-wrapper">
                                            <label for="shoeSize-${
                                              video.product[0].product_id
                                            }-${index}">What's your gym shoe size?</label>
                                            <input class="shoe-size-input" type="number" oninput="validateInput(this,${
                                              video.product[0].product_id
                                            },${index})" id="shoeSize-${
            video.product[0].product_id
          }-${index}" product-id="${
            video.product[0].brand_product_id
                                            ...
                                            <span class="size-input-error-msg-${
                                              video.product[0].product_id
                                            }-${index}" style="color:red;font-size:10px;font-weight:normal;"></span>
                                        </div>
                                        <div class="gender-wrapper">
                                            <label>Gender</label>
                                            <select id="genderForSize-${
                                              video.product[0].product_id
                                            }-${index}">
                                    ...
                                    <div class="shoe-gender-wrapper-row2">
                                        <div class="ssv_find_fit" onclick="handleFindFit(${
                                          video.product[0].product_id
                                        },${index})">Find Fit</div>
                            ...
                            <div class="product-rating-ssv prssv-${
                              video.video_id
                            }-${video.product[0].product_id}"></div>
                            <div class="product-rating-all-ssv prassv-${
                              video.video_id
                            }-${video.product[0].product_id}"></div>
                        </div>
                        <div class="product-bottom-pb-ssv">
                            <button class="product-addtocart-pb-ssv" disabled id="add-to-cart-btn-${
                              video.product[0].product_id
                            }-${index}" onclick="addTocartForOdr(${
            video.product[0].brand_product_id
          },this,${video.product[0].product_id},${index});CTAClicksssv(${
            video.product[0].product_id
          }, '', '${video.product[0].image}', '${video.product[0].url}', ${
            video.designer_id
          }, ${
            video.video_id
          }, '2');" style="background: none !important;opacity: 0.5; border: 2px solid ${
            ssv_brandCustomizations.front_color_add_to_cart_btn
          } !important; color: ${
            ssv_brandCustomizations.front_color_add_to_cart_btn
          } !important; ${
            ...
            ssv_brandCustomizations.buy_now == "0"
              ...
              : ""
                            ...
                            <button class="product-addtocart-pb-ssv"  id="add-to-cart-btn-${
                              video.product[0].product_id
                            }-${index}" onclick="addtocartssv(${
            video.video_id
          }, ${video.product[0].product_id}, '${
            video.product[0].sku_code
          }', this, 1);CTAClicksssv(${video.product[0].product_id}, '', '${
            video.product[0].image
          }', '${video.product[0].url}', ${video.designer_id}, ${
            video.video_id
          }, '2');" style="background: none !important; border: 2px solid ${
            ssv_brandCustomizations.front_color_add_to_cart_btn
          } !important; color: ${
        ...
        buynowBtn =
          video.product_link && ssv_brandCustomizations.buy_now == "1"
            ? `
                    <a href="${
                      video.product_link
                    }" onclick="CTAClicksssv(0, '', '', '', ${
                video.designer_id
              }, ${video.video_id}, '1');">
                        <button class="video-redirect-btn-ssv" style="background: ${
                          ssv_brandCustomizations.bk_color_buy_btn
                        } !important; color: ${
                ssv_brandCustomizations.front_color_buy_btn
              } !important; ${
      ...
      modalSlides += `
            <div class="swiper-slide ${
              typeof video.product[0] == "undefined" ? "no-product-ssv" : ""
                ...
                <div class="video-modal-video-container-ssv" >
                    ...
                    <video id="modalVideossv-${
                      video.video_id
                    }" loop playsinline="" data-setup="{}" onplay="jQuery(this).next().hide()"  title="${
        video.video_title
      }"
                    product-title="${
                      video.product.length > 0 && video.product[0]
                        ? video.product[0].title
                        : "No product"
                    } " onmouseover="showControlsssv(this);" onclick="showPhoneControlsssv(this);"
                    ontimeupdate="updateProgressbarssv(this)"
                    style="width:${
                      video.is_landscape == 0 && window.innerWidth > 775
                        ? "450px"
                        : "100%"
                    } !important; object-fit: ${
        video.is_landscape == "0" ? "fill" : "cover"
      } !important"
                    poster="${video.cover_image}" src="${
        video.video_url
      }" preload="none">
                    <source src="${video.video_url}" data-src="${
        video.video_url
      }" type="video/mp4">
                    </video>
                    ...
                    <button class="video-modal-pip-ssv" title="PIP Mode" onclick="playpipssv('','',${index});" style="${
        ssv_brandCustomizations.pip_mode == "0"
          ? "display: none !important;"
          : ""
                    ...
                    <div class="video-modal-actions-ssv">                                           
                        ...
                        <button class="video-modal-like-ssv" title="Like" onclick="likeVideossv(this, ${
                          video.video_id
                        }, ${
        video.designer_id
      });" style="background: rgb(0, 0, 0, .6) !important; display: none !important;">
                            <img src="${
                              getCookie(`ssv_vl_${video.video_id}`)
                        ...
                        <button class="video-modal-download-ssv" title="Download Video" onclick="downloadVideossv(this, '${
                          video.video_url
                        }', ${video.video_id}, ${video.designer_id})" style="${
        ssv_brandCustomizations.download_icon == "0"
          ? "display: none !important;"
          : ""
      } background: rgb(0, 0, 0, .6) !important;">
                        ...
                        <button class="video-modal-askque-ssv" title="Ask Question" onclick="openAskquessv(this);" style="${
                          ssv_brandCustomizations.ask_question == "0"
                            ? "display: none !important;"
                            : ""
                        } background: rgb(0, 0, 0, .6) !important;">
        ...
        video.video_id
                    ...
                    <div class="video-modal-askque-modal-ssv">
                        ...
                        <div class="askque-modal-form-ssv askque-modal-form1-ssv">
                            <form onsubmit="return askQuestionssv(this, ${
                              video.designer_id
                            }, ${video.video_id});">
                                ...
                                <button class="askque-modal-btn-ssv askque-modal-btn1-ssv" style="background: ${
                                  ssv_brandCustomizations.bk_color_buy_btn
                                } !important; color: ${
        ssv_brandCustomizations.front_color_buy_btn
                        ...
                        <div class="askque-modal-form-ssv askque-modal-form2-ssv">
                            <form onsubmit="return registerUserssv(this, ${
                              video.video_id
                            });">
                                ...
                                <button class="askque-modal-btn-ssv askque-modal-btn2-ssv" style="background: ${
                                  ssv_brandCustomizations.bk_color_buy_btn
                                } !important; color: ${
        ssv_brandCustomizations.front_color_buy_btn
                    ...
                    <div class="user-registration-popup-ssv">
                        <div class="user-registration-popup-inner-ssv">
                            ...
                            <div class="urp-form-ssv">
                                <form onsubmit="return userRegistrationssv(this, ${
                                  video.video_id
                                });" autocomplete="off">
                                    ...
                                    <button class="urp-btn-ssv" style="background: ${
                                      ssv_brandCustomizations.bk_color_buy_btn
                                    } !important; color: ${
        ssv_brandCustomizations.front_color_buy_btn
      } !important;">Register</button>                      
                    ...
                    <div class="chat-input-ssv" style="${
                    ...
                    <div class="chat-history-ssv chatssv-${video.video_id}">
                        <div class="chat-messages-block-ssv">
                            <p><b>${
                              ssv_brandCustomizations.designer_brand_name
                            ...
                            <div class="chat-history-input-ssv">
                                <form onsubmit="return sendChatMessagessv(this, ${
                                  video.video_id
                                });">
                        ...
                        <div class="cart-popup-flex-ssv">
                            <div class="cart-popup-wraper-ssv">
                                ...
                                <div class="video-modal-cart-popup-action-ssv">
                                    <a href="${window.location.origin}/${
        ssv_brandCustomizations.cust_cart_redirection_link
      }" class="popup-cart-btn-ssv" style="background: none !important; border: 2px solid ${
        ssv_brandCustomizations.front_color_add_to_cart_btn
      } !important; color: ${
        ssv_brandCustomizations.front_color_add_to_cart_btn
      } !important;">${ssv_brandCustomizations.view_cart_txt}</a>
                                    <a onclick="closeCartPopupssv(this);" class="popup-shopping-btn-ssv" style="background: ${
                                      ssv_brandCustomizations.bk_color_buy_btn
                                    } !important; color: ${
        ssv_brandCustomizations.front_color_buy_btn
        ...
        <div class="filter_main_div" id="main_div_filter" style="display: none">
                ...
                <div class="right_section_first_row">
                    ...
                    <div class="search-clear-wrapper">
                        ...
                        <div class="custom-search-container">
                            ...
                            <input type="text" class="custom-search-input" value="${
                              localStorage.getItem("searchQuery")
                                ? localStorage.getItem("searchQuery")
                                : ""
                            }" placeholder="Search..." onchange="handleSearchVideos(this)">
                        </div>
                    </div>
                </div>
            </div>
            <div class="second_row">
                <div class="search-container" style="display: ${
                  ssv_brandCustomizations.location == "1" ? "block" : "none"
                };max-width:400px">
                    <select id="select_location"  class="search-input" onChange="handleLocationChange()">
                        ...
                        <option ${
                          selecteOrNotCheck({
                            ...
                            ? "selected"
                            : ""
                        } value="3" data-object='{"city": "Brighton","state": "Utah","country": "USA"}'>Brighton,Utah, USA</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="4" data-object='{"city": "","state": "","country": "Canada"}'>Canada</option>
                        <option  ${
                          selecteOrNotCheck({
                            ...
                            ? "selected"
                            : ""
                        } value="5" data-object='{"city": "Mammoth Mountain","state": "California","country": ""}'>Mammoth Mountain, California</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="6" data-object='{"city": "","state": "","country": "France"}'>France</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="8" data-object='{"city": "","state": "","country": "Europe"}' >Europe</option>
                        <option  ${
                          selecteOrNotCheck({
                            ...
                            ? "selected"
                            : ""
                        } value="9" data-object='{"city": "Arizona Snowbowl","state": "Arizona","country": ""}'>Arizona Snowbowl, Arizona</option>
                        <option  ${
                          selecteOrNotCheck({
                            ...
                            ? "selected"
                            : ""
                        } value = "9"  data - object='{"city": "Wildcat Mountain","state": "NH","country": ""}' >Wildcat Mountain, NH</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "Lee Canyon",
                            state: "Nevada",
                            ...
                            ? "selected"
                            : ""
                        } value="10" data-object='{"city": "Lee Canyon","state": "Nevada","country": ""}'>Lee Canyon, Nevada</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "Whistler",
                            state: "BC",
                            ...
                            ? "selected"
                            : ""
                        } value="11" data-object='{"city": "Whistler","state": "BC","country": ""}'>Whistler, BC</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "Calgary",
                            state: "BC",
                            ...
                            ? "selected"
                            : ""
                        } value="12" data-object='{"city": "Calgary","state": "BC","country": ""}'>Calgary, BC</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "Mt. Holly",
                            state: "MI",
                            ...
                            ? "selected"
                            : ""
                        } value="13" data-object='{"city": "Mt. Holly","state": "MI","country": ""}'>Mt. Holly, MI</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "Vermont",
                            ...
                            ? "selected"
                            : ""
                        } value="14" data-object='{"city": "","state": "Vermont","country": ""}'>Vermont</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "Ontario",
                            ...
                            ? "selected"
                            : ""
                        } value="15" data-object='{"city": "","state": "Ontario","country": ""}'>Ontario</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            ...
                            ? "selected"
                            : ""
                        } value="16" data-object='{"city": "","state": "Crystal Mountain","country": "WA"}'>Crystal Mountain, WA</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="17" data-object='{"city": "","state": "","country": "South Korea"}'>South Korea</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "Keystone",
                            state: "CO",
                            ...
                            ? "selected"
                            : ""
                        } value="18" data-object='{"city": "Keystone","state": "CO","country": ""}'>Keystone, CO</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="19" data-object='{"city": "","state": "","country": "Russia"}'>Russia</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="20" data-object='{"city": "","state": "","country": "Iceland"}'>Iceland</option>
                        <option  ${
                          selecteOrNotCheck({
                            city: "",
                            state: "",
                            ...
                            ? "selected"
                            : ""
                        } value="21" data-object='{"city": "","state": "","country": "Lithuania"}'>Lithuania</option>
                    </select>
                </div>
                <div class="mobile-view-container" style="flex-grow: ${
                  ssv_brandCustomizations.search == "1" &&
                  ssv_brandCustomizations.location == "1"
                    ? "2"
                    : "1"
                }">
                    ...
                    <div class="custom-search-container" style="display: ${
                      ssv_brandCustomizations.search == "1" ? "flex" : "none"
                    }">
                        <i class="custom-search-icon">
                        ...
                        <i class="clear-search-icon" onclick="clearFilterForSearch()"  style="display: ${
              ...
              if (Object.keys(reviews.data).length > 0) {
                ...
                let rappend =
                  Object.keys(reviews.data).length > 2
                    ? `
                                    <p class="product-rating-title-ssv">${totalRating} <i class="starssv"></i> (${
  ...
  const filtered_length_is_null = document.getElementById(
    "filtered_length_is_null"
  );
  const videosFiltered = JSON.parse(
    localStorage.getItem("_ssv_storeResponseData")
  ).swilrs.video;

  filtered_length_is_null.style.display =
    videosFiltered.length === 0 ? "block" : "none";

  // Code to display/hide clear filter button
  const clear_filter_btn_wrapper = document.getElementById(
    "clear-filter-btn-wrapper"
  );
  const clear_filter_btn_wrapper_status = JSON.parse(
    localStorage.getItem("filtered")
  );

  clear_filter_btn_wrapper.style.display = clear_filter_btn_wrapper_status
    ? "block"
    : "none";
    ...
    success: function (response) {
      ...
      if (multiselectData.length > 0) {
        ...
        function updateSelectedValues(filterKey, selectedValues) {
          ...
          if (
            Object.values(globalData).flat().length > 0 &&
...
function openProductssv(e, btn) {
  if (
    ...
    e.target.classList == "ontile-selectSize-ssv"
  )
    return;
  jQuery(btn)
    .closest(".video-modal-video-container-ssv")
...
function sendChatMessagessv(frm, videoId) {
  ...
  jQuery(`.chatssv-${videoId} .chat-messages-ssv`).append(
    `<div class="chat-message-R-ssv"><section>${formData[0]["value"]}</section></div>`
  );
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      ...
      // </div>
      jQuery(`.chatssv-${videoId} .chat-messages-ssv`).append(
        `<div class="chat-message-L-ssv"><section>${response.reply}</section></div>`
      );
      chatDiv.scrollTop(chatDiv.prop("scrollHeight"));
    },
    error: function (request, error) {},
...
const selecteOrNotCheck = (data) => {
  ...
  if (storedAddredd) {
    if (
      storedAddredd.city == data.city &&
      storedAddredd.state == data.state &&
      storedAddredd.country == data.country
    ) {
      return true;
    } else {
      return false;
...
function handleFindFitWrapper(id, index) {
  var shoeGenderWrapper = document.getElementById(
    `shoeGenderWrapper-${id}-${index}`
  );
  shoeGenderWrapper.style.display =
    shoeGenderWrapper.style.display === "none" ||
    shoeGenderWrapper.style.display === ""
      ? "flex"
      : "none";
}
const handleFindFit = (id, index) => {
  ...
  const productId = document
    .getElementById(`shoeSize-${id}-${index}`)
...
const addTocartForOdr = async (productId, btn, clr, index) => {
  ...
  jQuery(btn).html("Adding...");
  const colorSelected = document.getElementById(
    `selectedColor-${clr}-${index}`
  ).value;
  let id;
  if (productId == 6040740593834) {
  ...
  const obj = {
    form_type: "product",
    quantity:
      parseInt(ele?.value) > 1 && parseInt(ele?.value) !== "undefined"
        ? ele?.value
        : "1",
...
const handleLocationChange = async () => {
  ...
  const selectedObjectString =
    select.options[selectedIndex].getAttribute("data-object");
  ...
  if (
    ...
    if (selectedObjectVal == "7") {
      ...
      let addressString = `${
        globalData.address.city !== "" ? globalData.address.city + ", " : ""
      }${
...
const handleSizeChange1 = (productsId, index) => {
  ...
  selectedVarient = document.getElementById(
    `select-size1-${productsId}-${index}`
  ).value;
...
window.addEventListener("unload", () => {
  ...
  if (analyticsDataString) {
    try {
      ...
      console.log("Original analyticsData:", analyticsData);
      localStorage.removeItem("_all_video_data");

      const updatedData = analyticsData.map((i) => {
        i.video_id = i.id.replace(substring_to_remove, ""); // Modify `i` directly
        return i; // Return the updated object
      });
      ...
      try {
        ...
        if (success) {
          console.log("Data successfully sent to Server 1");
        } else {
          console.error("Beacon transmission failed to Server 1");
        }
      } catch (error) {
        console.error("Error sending data to Server 1:", error);
      }
      ...
      try {
        ...
        if (success) {
          console.log("Data successfully sent to Server 2");
        } else {
          console.error("Beacon transmission failed to Server 2");
        }
      } catch (error) {
    ...
    console.log("No analytics data found in local storage.");
  }
});
