...
(() => {
    ...
    jqTag.href = 'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/reviews/v13.5.1/style-v1.min.css';
...
function initssv13() {
    ...
    if (localStorage.getItem('ssv13_pip') && document.querySelectorAll('.swirl-short-videos-c-ssv13').length == 0) {
        ...
        cDiv.innerHTML = `
            <div class="swirl-short-videos-c-ssv13" data-code="${localStorage.getItem('ssv13_storeCode')}" data-playlist="${localStorage.getItem('ssv13_storePlaylist')}"
                data-pdp="${localStorage.getItem('ssv13_storePdp')}" data-wt="${localStorage.getItem('ssv13_storeType')}" data-reviewid="${localStorage.getItem('ssv13_reviewId')}" ></div>
        `;
    ...
    apiCall = ssv13_pdppip ? 'https://pdp-reviews-prod-api.goswirl.in/api/product/reviews/playlist' : 'https://api.goswirl.live/index.php/ShortVideo/getVideosv13';
    ...
    if (ssv13_pipMode && !ssv13_storePlaylist && localStorage.getItem('ssv13_pdppip')) {
        ...
        apiCall = 'https://pdp-reviews-prod-api.goswirl.in/api/product/reviews/playlist';
    }
    ...
    if (ssv13_storePlaylist || ssv13_pdppip || ssv13_pipMode) {
        fetch(`${apiCall}?sCode=${ssv13_storeCode}&pCodes=${ssv13_storePlaylist}&url=${cUrl}&reviewId=${ssv13_reviewId}`, requestOptions)
...
function searchVideos(frm) {
    ...
    let apiCall = 'https://pdp-reviews-prod-api.goswirl.in/api/product/reviews/playlist';
    let cUrl = window.location.href;
    cUrl = cUrl.split('?')[0];
    cUrl = cUrl.split('#')[0];
    let myHeaders = new Headers();
    ...
    fetch(`${apiCall}?sCode=${ssv13_storeCode}&pCodes=${ssv13_storePlaylist}&url=${cUrl}&reviewId=${ssv13_reviewId}&q=${searchVal}`, requestOptions)
        .then(response => response.text())
        .then(result => {
            let data = JSON.parse(result);
            // console.log(data);
...
function generatessv13(videos, target, no, reinit = false) {
    ...
    videos.forEach(video => {
        ...
        let cReview = ssv13_pdppip && ssv13_reviewId ? `
        ...
        video.product.forEach(prd => {
            ...
            let reviewSummary = ssv13_pdppip && ssv13_reviewId ? `
        ...
        let mReviewSummary = ssv13_pdppip && ssv13_reviewId ? `
    ...
    if (ssv13_pdppip && ssv13_reviewId && !reinit && ssv13_brandCustomizations.aiVideoSearch && ssv13_brandCustomizations.pdppip_hidecarousel == '0' && ssv13_brandCustomizations.brand_id != '236634103') {
        let aiSuggestions = getAISuggestionssv13();
