var ssv13_videoDataArray = [], ssv13_currentVideoTimer = null; function generateUUIDssv13() { var e = (new Date).getTime(), t = "undefined" != typeof performance && performance.now && 1e3 * performance.now() || 0; return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (function (i) { var s = 16 * Math.random(); return e > 0 ? (s = (e + s) % 16 | 0, e = Math.floor(e / 16)) : (s = (t + s) % 16 | 0, t = Math.floor(t / 16)), ("x" === i ? s : 3 & s | 8).toString(16) })) } function getDeviceTypessv13() { const e = window.innerWidth; return e <= 767 ? "Mobile" : e >= 768 && e <= 1024 ? "Tablet" : "Desktop" } async function initializeVideoDatassv13(e, t, i) { let s = !1; const o = ssv13_videoDataArray.find((t => t.id === e)), n = localStorage.getItem("ssv13_responseData"), a = JSON.parse(n), r = document.getElementById(e), d = o || { id: e, url: t, unique_views: 0, watch_time: 0, brand_id: a.swirls.customization.brand_id, total_views: 0, duration: a?.swirls.video?.find((e => e.server_url == t))?.video_len, video_title: a?.swirls.video?.find((e => e.server_url == t))?.video_title, module: "reviews", drop_of_point: [], skip_points: [], segments: [], location_details: {}, system_detail: { swirl_machine_id: generateUUIDssv13(), device_type: getDeviceTypessv13() } }; initializeSegmentssv13(d), ssv13_currentVideoTimer && clearInterval(ssv13_currentVideoTimer), o && (d.watch_time = parseInt(o.watch_time || 0, 10)), r.addEventListener("timeupdate", (() => { const e = Math.floor(r.currentTime); d.segments.find((t => e >= t.start && e <= t.end)); 0 == e || s || (d.unique_views = 1, d.total_views += 1, s = !0, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), ssv13_currentVideoTimer = setInterval((() => { r.paused || (tenPercent = Math.floor(.1 * d.duration), d.watch_time += 1, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }), 1e3), r.addEventListener("pause", (() => { const e = Math.floor(r.currentTime), t = d.segments.find((t => e >= t.start && e <= t.end)); t && (d.drop_of_point.push({ segment_id: t.segment_id, timestamp: e }), localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), r.addEventListener("ended", (() => { const e = Math.floor(r.currentTime), t = d.segments.find((t => e >= t.start && e <= t.end)); t && (d.drop_of_point.push({ segment_id: t.segment_id, timestamp: e }), localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), r.addEventListener("seeked", (() => { const e = Math.floor(r.currentTime), t = d.segments.find((t => e >= t.start && e <= t.end)); t && (0 == d?.skip_points.length || d.skip_points[d.skip_points.length - 1].to ? d.skip_points.push({ from: { segmentId: t.segmentId, timeStamp: e }, to: null }) : d.skip_points[d.skip_points.length - 1].to = { segmentId: t?.segmentId, timeStamp: e }, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), o ? Object.assign(o, d) : ssv13_videoDataArray.push(d) } function initializeSegmentssv13(e) { for (let t = 0; t < Math.ceil(e.duration / 3); t++)e.segments.push({ segment_id: t + 1, start: 3 * t, end: Math.min(3 * (t + 1), e.duration) }) } function pushAnalyticsdatassv13() { substring_to_remove = "ssv13-video-m-"; const e = JSON.parse(localStorage.getItem("_all_video_data")), t = e?.map((async e => (e.video_id = e.id.replace(substring_to_remove, ""), e))); t.length > 0 && Promise.all(t).then((async e => { await fetch("https://analytics-api.goswirl.live/engagement", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payloads: e }) }).then((e => { if (ssv13_videoDataArray = [], !e.ok) throw new Error("Network response was not ok"); localStorage.removeItem("_all_video_data") })).catch((e => { console.error("Error sending data:", e) })) })).catch((e => console.log(e))) } async function impressionCountssv13(e, t, i) { let s = window.location.href.split("?")[0]; const o = localStorage.getItem("uuid"), n = localStorage.getItem("timestamp"); if (o && n) { const e = 144e5; if ((new Date).getTime() - parseInt(n, 10) < e) return void console.log("Using existing UUID:", o) } const a = generateUUIDssv13(); localStorage.setItem("uuid", a), localStorage.setItem("timestamp", (new Date).getTime().toString()), e = parseInt(e); let r = await getUserGeossv13(), d = { id: a, playlistCode: t || "", url: s, brandId: e, videoIds: i, fingerprint: a, browserName: r.browserName, osName: r.osName, deviceType: r.deviceType, userAgent: r.userAgent, ipAddress: r.ipAddress, browserVersion: r.browserVersion, city: "", region: "", country: "" }; await fetch("https://analytics-api.goswirl.live/impression", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payload: d }) }).then((e => { })).catch((e => { console.error("Error sending data:", e) })) } async function getUserGeossv13() { const e = {}; try { const t = await fetch("https://api.ipify.org?format=json"), i = await t.json(); e.ipAddress = i.ip } catch (t) { e.ipAddress = "Unable to fetch IP" } e.userAgent = navigator.userAgent; let t = "Unknown", i = "Unknown", s = "Unknown", o = "Desktop"; const n = e.userAgent; return n.includes("Chrome") ? (t = "Chrome", i = n.match(/Chrome\/([\d.]+)/)?.[1] || "Unknown") : n.includes("Firefox") ? (t = "Firefox", i = n.match(/Firefox\/([\d.]+)/)?.[1] || "Unknown") : n.includes("Safari") && !n.includes("Chrome") && (t = "Safari", i = n.match(/Version\/([\d.]+)/)?.[1] || "Unknown"), n.includes("Windows") ? s = "Windows" : /Android/i.test(n) ? (s = "Android", o = "Mobile") : /iPhone|iPad|iPod/i.test(n) ? (s = "iOS", o = "Mobile") : n.includes("Mac") ? s = "macOS" : n.includes("Linux") && (s = "Linux"), e.browserName = t, e.browserVersion = i, e.osName = s, e.deviceType = o, e } window.addEventListener("beforeunload", (() => { const e = localStorage.getItem("_all_video_data"); if (e) try { const t = JSON.parse(e); localStorage.removeItem("_all_video_data"); const i = t?.map((async e => (e.video_id = e.id.replace("ssv13-video-m-", ""), e))); if (i) try { navigator.sendBeacon("https://analytics-api.goswirl.live/engagement/onclose", JSON.stringify(t)) || console.log("Beacon transmission failed"), localStorage.removeItem("_all_video_data") } catch (e) { console.error("Error sending data:", e) } } catch (e) { localStorage.removeItem("_all_video_data"), console.error("Error parsing analytics data:", e) } }));
...
(() => {
    ...
    jqTag.href = 'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/reviews/v13.5.1/style-v2.min.css';
...
function initssv13() {
    ...
    if (localStorage.getItem('ssv13_pip') && document.querySelectorAll('.swirl-short-videos-c-ssv13').length == 0) {
        ...
        cDiv.innerHTML = `
            <div class="swirl-short-videos-c-ssv13" data-code="${localStorage.getItem('ssv13_storeCode')}" data-playlist="${localStorage.getItem('ssv13_storePlaylist')}"
                data-pdp="${localStorage.getItem('ssv13_storePdp')}" data-wt="${localStorage.getItem('ssv13_storeType')}" data-reviewid="${localStorage.getItem('ssv13_reviewId')}" data-ugcplaylist="${localStorage.getItem('ssv13_ugcPlaylist')}" ></div>
        `;
        ...
        let reviewId = document.querySelectorAll('.swirl-short-videos-c-ssv13')[0].dataset.reviewid;
        let ugcPlaylist = document.querySelectorAll('.swirl-short-videos-c-ssv13')[0].dataset.ugcplaylist;
        if ((reviewId != '' && typeof reviewId != 'undefined') || (ugcPlaylist != '' && typeof ugcPlaylist != 'undefined')) {
            ssv13_pdppip = true;
        } else ssv13_pdppip = false;
    }
    ...
    let apiCall = ssv13_pdppip ? 'https://api.goswirl.live/index.php/ShortVideo/getPDPVideosv13' : 'https://api.goswirl.live/index.php/ShortVideo/getVideosv13';
    apiCall = ssv13_pdppip ? 'https://pdp-reviews-prod-api.goswirl.in/api/product/reviews/playlist/ugcplaylist' : 'https://api.goswirl.live/index.php/ShortVideo/getVideosv13';
    ...
    document.querySelectorAll('.swirl-short-videos-c-ssv13').forEach(elm => {
        ...
        if (typeof elm.dataset.reviewid != 'undefined' && elm.dataset.reviewid != '') ssv13_reviewId.push(elm.dataset.reviewid);
        if (typeof elm.dataset.ugcplaylist != 'undefined' && elm.dataset.ugcplaylist != '') ssv13_ugcPlaylist.push(elm.dataset.ugcplaylist);
        ssv13_storeType = elm.dataset.wt;
        ssv13_storePdp = elm.dataset.pdp;
        // ssv13_reviewId = typeof elm.dataset.reviewid != 'undefined' ? elm.dataset.reviewid : '';
    });
    ssv13_storeURL = window.location.origin;
    ssv13_storePlaylist = ssv13_storePlaylist.join(',');
    ssv13_reviewId = ssv13_reviewId.join(',');
    ssv13_ugcPlaylist = ssv13_ugcPlaylist.join(',');
    ...
    if (ssv13_pipMode && !ssv13_storePlaylist && localStorage.getItem('ssv13_pdppip')) {
        ...
        ssv13_reviewId = localStorage.getItem('ssv13_reviewId');
        ssv13_ugcPlaylist = localStorage.getItem('ssv13_ugcPlaylist');
        apiCall = 'https://api.goswirl.live/index.php/ShortVideo/getPDPVideosv13';
        apiCall = 'https://pdp-reviews-prod-api.goswirl.in/api/product/reviews/playlist/ugcplaylist';
    }
    ...
    localStorage.setItem('ssv13_reviewId', ssv13_reviewId);
    localStorage.setItem('ssv13_ugcPlaylist', ssv13_ugcPlaylist);
    ...
    if (ssv13_storePlaylist || ssv13_pdppip || ssv13_pipMode) {
        fetch(`${apiCall}?sCode=${ssv13_storeCode}&pCodes=${ssv13_storePlaylist}&url=${cUrl}&reviewId=${ssv13_reviewId}&ugcPlaylistCode=${ssv13_ugcPlaylist}`, requestOptions)
            .then(response => response.text())
            .then(result => {
                let tmpRes = JSON.parse(result);
                if (ssv13_pdppip) tmpRes = tmpRes.data, result = JSON.stringify(tmpRes);
                ...
                // testing
                ssv13_brandCustomizations.video_like = 1;
                ...
                if (ssv13_brandCustomizations.comment_mode == '1' || ssv13_brandCustomizations.video_like == '1') {
                    jqTag = document.createElement('script');
                    jqTag.rel = 'text/javascript';
                ...
                let i = 0;
                let c = document.querySelectorAll('.swirl-short-videos-c-ssv13').length;
                document.querySelectorAll('.swirl-short-videos-c-ssv13').forEach(elm => {
                    if (!ssv13_pdppip) {
                        ...
                        if (c == 1) {
                            ...
                            generatessv13(data.swirls.video, elm, i);
                            impressionCountssv13(data.swirls.customization.brand_id, "", data.swirls.video.map((item) => item.video_id));
                        } else {
                            ssv13_responseData['video'] = data.swirls[elm.dataset.reviewid];
                            generatessv13(data.swirls[elm.dataset.reviewid], elm, i);
                            impressionCountssv13(data.swirls.customization.brand_id, "", data.swirls[elm.dataset.reviewid].map((item) => item.video_id));
                        }
                    }
                    i++;
                });
...
function searchVideos(frm) {
    event.preventDefault();
    let searchInp = frm.querySelector('input[name="aiSearch"]');
    let searchVal = searchInp.value;
    let searchedTag = frm.querySelector('.searched-tag-ssv13');
    let cDiv = document.querySelector('.swirl-short-videos-c-ssv13');
    let aiMsg = document.querySelector('.aisearch-result-msg-ssv13');

    if (searchedTag.querySelector('input[name="clearAIForm"]').value == '1' && searchVal == '') {
        ...
        generatessv13(ssv13_responseData.video, cDiv, 0, true);

        // Analytics code
        impressionCountssv13(ssv13_brandCustomizations.brand_id, "", ssv13_responseData.video.map((item) => item.video_id));

        return;
    }

    if (! /^[ A-Za-z0-9_@./#&+-?]*$/.test(searchVal.trim())) {
        aiMsg.innerHTML = 'Enter valid search query';
    ...
    if (searchVal.length < 3) {
        aiMsg.innerHTML = 'Enter atleast 3 letter to search';
        aiMsg.style.opacity = 1;
        setTimeout(() => {
            aiMsg.style.opacity = 0;
        }, 4000);
    ...
    searchedTag.querySelector('input[name="clearAIForm"]').value = 0;
    searchedTag.querySelector('span').innerHTML = searchVal;
    searchedTag.style.display = 'block';

    showHidePageLoaderssv13(true);

    let apiCall = 'https://pdp-reviews-prod-api.goswirl.in/api/product/reviews/playlist/ugcplaylist';
    let cUrl = window.location.href;
    cUrl = cUrl.split('?')[0];
    cUrl = cUrl.split('#')[0];
    let myHeaders = new Headers();
    ...
    fetch(`${apiCall}?sCode=${ssv13_storeCode}&pCodes=${ssv13_storePlaylist}&url=${cUrl}&reviewId=${ssv13_reviewId}&ugcPlaylistCode=${ssv13_ugcPlaylist}&q=${searchVal}`, requestOptions)
        .then(response => response.text())
        .then(result => {
            let data = JSON.parse(result);
            // console.log(data);
            ...
            // Suggestion to LS
            let aisuggest = localStorage.getItem(`_ssv13_aiSearchSuggestions_${ssv13_brandCustomizations.brand_id}`) ? JSON.parse(localStorage.getItem(`_ssv13_aiSearchSuggestions_${ssv13_brandCustomizations.brand_id}`)) : [];
            if (!aisuggest.includes(searchVal)) {
                aisuggest.push(searchVal);
                localStorage.setItem(`_ssv13_aiSearchSuggestions_${ssv13_brandCustomizations.brand_id}`, JSON.stringify(aisuggest));

                let aiSuggestions = getAISuggestionssv13();
                let suggestions = document.querySelector('.JS-ai-search-suggestions-ssv13')
                suggestions.innerHTML = aiSuggestions;
            }

            if (Object.keys(data.data.swirls.video).length) {
                // console.log(data.data.swirls.video)
                ssv13_swiper = [];
                ssv13_swiper_modal = [];
                ssv13_swiper_product = [];
                ssv13_swiper_review = [];

                generatessv13(data.data.swirls.video, cDiv, 0, true);

                // Analytics code
                impressionCountssv13(ssv13_brandCustomizations.brand_id, "", data.data.swirls.video.map((item) => item.video_id));
            } else {
                aiMsg.innerHTML = 'No matching videos for applied filters';
                aiMsg.style.opacity = 1;
                setTimeout(() => {
                    aiMsg.style.opacity = 0;
                }, 4000);
        ...
        .catch(error => {
            aiMsg.innerHTML = `Couldn't search`;
            aiMsg.style.opacity = 1;
            setTimeout(() => {
                aiMsg.style.opacity = 0;
            }, 4000);
...
function getAISuggestionssv13() {
    // From LS
    let aiSuggests = localStorage.getItem(`_ssv13_aiSearchSuggestions_${ssv13_brandCustomizations.brand_id}`) ? JSON.parse(localStorage.getItem(`_ssv13_aiSearchSuggestions_${ssv13_brandCustomizations.brand_id}`)) : [];
    let aiSuggestions = '';
    aiSuggests.forEach(ais => {
        if (!aiSuggestions.includes(ais)) {
            aiSuggestions += `
                <div onclick="document.querySelector('input[name=aiSearch]').value=this.innerHTML; document.querySelector('.ai-search-frm-btn-ssv13').click();" class="ai-search-suggestions-tiles-ssv13">${ais}</div>
            `;
        }
    });

    // From API
    let resData = JSON.parse(localStorage.getItem('ssv13_responseData'));
    let dbSuggestions = typeof resData.swirls.suggestions != 'undefined' ? resData.swirls.suggestions : [];
    dbSuggestions.forEach(dbs => {
        if (!aiSuggestions.includes(dbs)) {
            aiSuggestions += `
                <div onclick="document.querySelector('input[name=aiSearch]').value=this.innerHTML; document.querySelector('.ai-search-frm-btn-ssv13').click();" class="ai-search-suggestions-tiles-ssv13">${dbs}</div>
            `;
        }
    });

    aiSuggestions = aiSuggestions ? aiSuggestions : `
        <p style="text-align: center; margin: 20px 0; padding: 0; font-size: 14px;">No suggestions</p>
    `;

    return aiSuggestions;
}

function generatessv13(videos, target, no, reinit = false) {
    // testing    
    ssv13_brandCustomizations.chat_bot = typeof ssv13_brandCustomizations.chat_bot == 'undefined' ? '0' : ssv13_brandCustomizations.chat_bot;
    ssv13_brandCustomizations.cta_on_tile = 0;
    ssv13_subtitle = target.dataset.st == '18581' ? true : false;
    ssv13_brandCustomizations.marque_txt = ssv13_subtitle ? '0' : ssv13_brandCustomizations.marque_txt;
    ...
    reviewStarsC[0] = ``;
    reviewStarsC[1] = `
        <svg class="video-c-bottom-reviews-svg-ssv13" width="14"
    ...
    videos.forEach(video => {
        ...
        let cReview = ssv13_pdppip && (ssv13_reviewId || ssv13_ugcPlaylist) ? `
            <div class="video-c-bottom-reviews-ssv13">
                <div style="${(!video.c_name || video.c_name == 'anonymous') && ssv13_brandCustomizations.brand_id == '236634166' ? 'display: none;' : ''}">${reviewStarsC[video.rating]}</div>
                <div class="video-c-bottom-review-text-ssv13" style="justify-content: end;">
                    <p style="${(!video.c_name || video.c_name == 'anonymous') && ssv13_brandCustomizations.brand_id == '236634166' ? 'display: none;' : ''}">${video.c_name ? video.c_name : 'Anonymous'}</p>
                    <label>
                        <svg width="4" height="4" viewBox="0 0 4 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect width="4" height="4" rx="2" fill="#E8E8E8"/>
                        </svg>
                        ${getTimeAgossv13((video.created_at_time * 1000))}
                    </label>
                </div>
            </div>
        ` : '';
        cSlides += `
            <div class="swiper-slide" onclick="openVideoModalssv13(${i}, ${no});">
                <video style="aspect-ratio: 9 / 16; background: #000000; ${video.is_landscape == '1' ? 'object-fit: cover;' : ''}" data-src="${video.cover_video}" data-srcf="${video.server_url}" data-id="${video.video_id}" class="carousel-video-ssv13 ${video.is_landscape == '0' ? 'JS-cvideo-L-ssv13' : 'JS-cvideo-P-ssv13'} ${videoConflictClass}" onplay="this.closest('.swiper-slide').querySelector('.carousel-video-play-ssv13').style.display='none';"
                ...
                <div class="video-views-count-top-ssv13" style="${ssv13_brandCustomizations.views == '1' ? '' : 'display: none;'}">
                    <p>
                        ...
                        ${numberToShorthandssv13(video.total_views ? video.total_views : 0)}
                    </p>
                </div>
                ...
                ${cReview ? cReview : cProduct + cTitle}                
            </div>
        `;
        ...
        let mProductList = '';
        let mProducts = '';
        let mReviewSliderPhone = '';
        let mShareURL = window.location.href.split('?')[0] + '?ssv=' + window.btoa(video.video_id);

        // Temp
        if (ssv13_brandCustomizations.brand_id == '236634103' || ssv13_brandCustomizations.brand_id == '236634113') {
            video.cta_customization = video.overlayCtaButton ? video.overlayCtaButton : video.cta_customization;
        }
        ...
        video.product.forEach(prd => {
                        ...
                        <div class="video-modal-ps-detail-ssv13" style="${ssv13_brandCustomizations.product_reviews == '0' ? 'padding-top: 10px;' : ''} ${ssv13_brandCustomizations.product_price_status == '0' || ssv13_brandCustomizations.cta_on_tile == '0' ? 'height: 80px; width: calc(100% - 80px);' : ''} ${ssv13_brandCustomizations.product_price_status == '0' && ssv13_brandCustomizations.cta_on_tile == '0' ? 'height: 75px; width: calc(100% - 75px);' : ''}">                            
                            ...
                            <div style="text-align: right;">
                                <div class="video-modal-ps-ratting-ssv13" style="${prd.avgRating == 0 ? 'display: none !important;' : ''} ${ssv13_brandCustomizations.product_reviews == '0' ? 'display: none !important;' : ''}">
                                    ...
                                    ${prd.avgRating}
                                    <svg style="margin: 0 5px;" width="2" height="9" viewBox="0 0 2 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1.22561 8.184H0.569609V0.72H1.22561V8.184Z" fill="white"/>
                                    </svg>
            ...
            if (window.innerWidth > 768) {
                ...
                pbhCut += prd.avgRating > 0 && ssv13_brandCustomizations.product_reviews == '1' ? 47 : 0;
                pbhCut += ssv13_brandCustomizations.buy_now == '0' && ssv13_brandCustomizations.add_to_cart == '0' ? 0 : 60;
            } else {
                pbhCut += prd.avgRating > 0 && ssv13_brandCustomizations.product_reviews == '1' ? 77 : 30;
                pbhCut += ssv13_brandCustomizations.buy_now == '0' && ssv13_brandCustomizations.add_to_cart == '0' ? 0 : 60;
            }
            let reviewSummary = ssv13_pdppip && (ssv13_reviewId || ssv13_ugcPlaylist) ? `
                <div style="margin-top: 15px; margin-bottom: 10px;">
                    <h4 class="video-modal-sr-pb-pi-rc-user-ssv13" style="margin-bottom: 6px !important;">Review summary</h4>                                
                    <p class="video-modal-sr-pb-pi-rc-review-ssv13">${video.summary}</p>
                </div>
            ` : '';
            mProducts += `
                <div class="video-modal-sr-pb-product-info-ssv13" style="transform: none; ${Object.keys(video.product).length > 1 ? 'display: none;' : ''}">
                    <div class="video-modal-sr-pb-pi-header-ssv13">
                        ...
                        <div class="video-modal-sr-pb-pi-review-btn-ssv13" onclick="this.querySelector('.CSS-review-toogler-D-ssv13').dispatchEvent(new Event('click'));" style="${prd.avgRating == 0 ? 'display: none !important;' : ''} ${ssv13_brandCustomizations.product_reviews == '0' ? 'display: none !important;' : ''}">
                            <div class="video-modal-sr-pb-pi-review-btnL-ssv13">
                                <div class="video-modal-sr-pb-pi-review-rating-ssv13">
                                    <div class="video-modal-sr-pb-pi-review-ratingL-ssv13">
                                        <p>
                                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                    ...
                                    <div class="video-modal-sr-pb-pi-review-ratingR-ssv13">
                                        <p>${prd.title}</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="video-modal-sr-pb-pi-middle-ssv13" style="height: calc(100% - ${pbhCut}px);">
                        <div class="video-modal-sr-pb-pi-reviews-ssv13">
                            ${reviewSummary}
                            <div class="video-modal-p-review-desktop-ssv13">
                                ${mReviews}
                            </div>
                            <div class="video-modal-p-review-phone-ssv13">
                                <div class="swiper swiper-review-phone-ssv13 swiper-review-phone-ssv13-${video.video_id}-${no}">
        ...
        let mReviewSummary = ssv13_pdppip && (ssv13_reviewId || ssv13_ugcPlaylist) ? `
            <div style="margin-top: 15px; margin-bottom: 10px; text-align: left!important;">
                <h4 class="video-modal-pr-bottom-rc-user-ssv13">Review summary</h4>                                
                <p class="video-modal-pr-bottom-rc-review-ssv13">${video.summary}</p>
            </div>
        ` : '';
        mSlides += `
            <div class="swiper-slide swiper-slide-m">
                <div class="video-modal-sl-video-container-ssv ${!Object.keys(video.product).length ? 'no-products-slide-ssv13' : ''}">
                    <video style="aspect-ratio: 9/16; background: ${ssv13_brandCustomizations.bk_color_buy_btn}; object-fit: contain;" id="ssv13-video-m-${video.video_id}" data-src="${video.server_url}" data-st="${typeof video.subtitles.arabic != 'undefined' ? video.subtitles.arabic : ''}" data-products="${Object.keys(video.product).length}" data-id="${video.video_id}" class="video-modal-sl-video-ssv13 ${video.is_landscape == '0' ? 'JS-mvideo-L-ssv13' : 'JS-mvideo-P-ssv13'} ${videoConflictClass}"
                    ...
                    <div class="video-modal-sl-bottom-controls-ssv13 ${ssv13_brandCustomizations.buy_now == '1' && videoCTA ? 'video-CTA-on' : ''}">
                        <svg class="video-modal-sl-video-control-ssv13 video-modal-sl-vc-like-ssv13" onclick="likeVideossv13('${video.video_id}')" width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M13.6 8.00039H20C20.5304 8.00039 21.0391 8.21111 21.4142 8.58618C21.7893 8.96125 22 9.46996 22 10.0004V12.1044C22.0003 12.3658 21.9493 12.6246 21.85 12.8664L18.755 20.3814C18.6795 20.5646 18.5513 20.7213 18.3866 20.8315C18.2219 20.9417 18.0282 21.0005 17.83 21.0004H1C0.734784 21.0004 0.48043 20.895 0.292893 20.7075C0.105357 20.52 0 20.2656 0 20.0004V10.0004C0 9.73518 0.105357 9.48082 0.292893 9.29329C0.48043 9.10575 0.734784 9.00039 1 9.00039H4.482C4.6421 9.00043 4.79986 8.96203 4.94203 8.88843C5.0842 8.81482 5.20662 8.70815 5.299 8.57739L10.752 0.850392C10.8209 0.752705 10.9226 0.682953 11.0385 0.653782C11.1545 0.624612 11.277 0.637955 11.384 0.691392L13.198 1.59839C13.7085 1.85355 14.1163 2.27571 14.3537 2.79471C14.5911 3.31371 14.6438 3.89831 14.503 4.45139L13.6 8.00039ZM6 10.5884V19.0004H17.16L20 12.1044V10.0004H13.6C13.2954 10.0003 12.9948 9.93073 12.7212 9.79685C12.4476 9.66297 12.2082 9.46836 12.0213 9.22788C11.8343 8.98739 11.7048 8.70739 11.6425 8.40922C11.5803 8.11105 11.5869 7.80259 11.662 7.50739L12.565 3.95939C12.5933 3.84872 12.5828 3.73171 12.5353 3.62782C12.4878 3.52393 12.4062 3.43944 12.304 3.38839L11.643 3.05839L6.933 9.73039C6.683 10.0844 6.363 10.3744 6 10.5884ZM4 11.0004H2V19.0004H4V11.0004Z" fill="white"/>
                        </svg>
                        <p class="video-modal-sl-vc-likes-count-ssv13">0</p>

                        <svg class="video-modal-sl-video-control-ssv13 video-modal-sl-vc-dislike-ssv13" onclick="dislikeVideossv13('${video.video_id}')" width="22" height="21" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M8.4 13H2C1.46957 13 0.96086 12.7893 0.585788 12.4142C0.210715 12.0391 1.0643e-06 11.5304 1.0643e-06 11V8.896C-0.000268556 8.63464 0.050692 8.37576 0.150001 8.134L3.246 0.62C3.32128 0.436753 3.4493 0.279995 3.6138 0.169612C3.77831 0.0592292 3.97189 0.00019547 4.17 0H21C21.2652 0 21.5196 0.105357 21.7071 0.292893C21.8946 0.48043 22 0.734784 22 1V11C22 11.2652 21.8946 11.5196 21.7071 11.7071C21.5196 11.8946 21.2652 12 21 12H17.518C17.3579 12 17.2001 12.0384 17.058 12.112C16.9158 12.1856 16.7934 12.2922 16.701 12.423L11.248 20.149C11.1791 20.2467 11.0774 20.3164 10.9615 20.3456C10.8455 20.3748 10.723 20.3614 10.616 20.308L8.802 19.4C8.2915 19.1448 7.88367 18.7227 7.64627 18.2037C7.40887 17.6847 7.3562 17.1001 7.497 16.547L8.4 13ZM16 10.412V2H4.84L2 8.896V11H8.4C8.7046 11 9.00517 11.0697 9.27877 11.2035C9.55237 11.3374 9.79178 11.532 9.97873 11.7725C10.1657 12.013 10.2952 12.293 10.3575 12.5912C10.4197 12.8893 10.4131 13.1978 10.338 13.493L9.435 17.041C9.40674 17.1517 9.41722 17.2687 9.46471 17.3726C9.51219 17.4765 9.59382 17.561 9.696 17.612L10.357 17.942L15.067 11.27C15.317 10.916 15.637 10.626 16 10.412ZM18 10H20V2H18V10Z" fill="white"/>
                        </svg>
                        <p class="video-modal-sl-vc-dislikes-count-ssv13">0</p>
                        ...
                        <svg class="video-modal-sl-video-control-ssv13 video-modal-sl-vc-reviews-ssv13"
                            ...
                            viewBox="0 0 25 22" fill="none" xmlns="http://www.w3.org/2000/svg" style="${ssv13_brandCustomizations.product_reviews == '0' ? 'display: none !important;' : ''} display: none !important;">
                            <path
                        ...
                        <p class="video-modal-sl-vc-reviews-count-ssv13" style="${ssv13_brandCustomizations.product_reviews == '0' ? 'display: none !important;' : ''} display: none !important;">${Object.keys(video.product).length ? Object.keys(video.product[0].pReviews).length : 0}</p>
                    ...
                    <div class="video-modal-product-reviews-ssv13" style="display: none;">
    ...
    if (ssv13_pdppip && (ssv13_reviewId || ssv13_ugcPlaylist) && !reinit && ssv13_brandCustomizations.aiVideoSearch && ssv13_brandCustomizations.pdppip_hidecarousel == '0' && ssv13_brandCustomizations.brand_id != '236634103') {
        let aiSuggestions = getAISuggestionssv13();
        let aiDiv = document.createElement('div');
        let aiSearch = `
            <div class="ai-search-ssv13" style="${ssv13_brandCustomizations.brand_id == '236634166' ? 'display: none;' : ''}">
                <div class="ai-search-container-ssv13">
                    <button class="ai-search-init-btn-ssv13" onclick="this.style.display = 'none'; document.querySelector('.ai-search-ask-form-ssv13').style.display = 'block';">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10 2.01758C11.0096 2.01758 11.8281 1.1991 11.8281 0.189453H12.6719C12.6719 1.1991 13.4903 2.01758 14.5 2.01758V2.86133C13.4903 2.86133 12.6719 3.67981 12.6719 4.68945H11.8281C11.8281 3.67981 11.0096 2.86133 10 2.86133V2.01758ZM0.25 6.93945C2.73528 6.93945 4.75 4.92474 4.75 2.43945H6.25C6.25 4.92474 8.26472 6.93945 10.75 6.93945V8.43945C8.26472 8.43945 6.25 10.4542 6.25 12.9395H4.75C4.75 10.4542 2.73528 8.43945 0.25 8.43945V6.93945ZM3.15701 7.68945C4.14038 8.23515 4.95432 9.04905 5.5 10.0325C6.04568 9.04905 6.85962 8.23515 7.843 7.68945C6.85962 7.14375 6.04568 6.32985 5.5 5.34646C4.95432 6.32985 4.14038 7.14375 3.15701 7.68945ZM12.4375 9.18945C12.4375 10.5356 11.3462 11.627 10 11.627V12.752C11.3462 12.752 12.4375 13.8433 12.4375 15.1895H13.5625C13.5625 13.8433 14.6538 12.752 16 12.752V11.627C14.6538 11.627 13.5625 10.5356 13.5625 9.18945H12.4375Z" fill="white"/>
                        </svg>
                        AI Search
                    </button>
                    <div class="ai-search-ask-form-ssv13">
                        <form onsubmit="searchVideos(this); document.querySelector('.ai-search-suggestions-ssv13').style.display = 'none';">
                        <input onfocus="document.querySelector('.ai-search-suggestions-ssv13').style.display = 'block';" onfocusout="" type="text" name="aiSearch" placeholder="Type here and press enter" required tabindex="-1" autocomplete="off" />
                        <p class="searched-tag-ssv13">
                            <input type="hidden" name="clearAIForm" value="0" />
                            <svg onclick="document.querySelector('input[name=aiSearch]').value=''; document.querySelector('input[name=clearAIForm]').value = '1'; document.querySelector('.ai-search-frm-btn-ssv13').click(); document.querySelector('.searched-tag-ssv13').style.display = 'none'; document.querySelector('input[name=aiSearch]').disabled = false;" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                        ...
                        <button type="submit" class="ai-search-frm-btn-ssv13">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10 2.01758C11.0096 2.01758 11.8281 1.1991 11.8281 0.189453H12.6719C12.6719 1.1991 13.4903 2.01758 14.5 2.01758V2.86133C13.4903 2.86133 12.6719 3.67981 12.6719 4.68945H11.8281C11.8281 3.67981 11.0096 2.86133 10 2.86133V2.01758ZM0.25 6.93945C2.73528 6.93945 4.75 4.92474 4.75 2.43945H6.25C6.25 4.92474 8.26472 6.93945 10.75 6.93945V8.43945C8.26472 8.43945 6.25 10.4542 6.25 12.9395H4.75C4.75 10.4542 2.73528 8.43945 0.25 8.43945V6.93945ZM3.15701 7.68945C4.14038 8.23515 4.95432 9.04905 5.5 10.0325C6.04568 9.04905 6.85962 8.23515 7.843 7.68945C6.85962 7.14375 6.04568 6.32985 5.5 5.34646C4.95432 6.32985 4.14038 7.14375 3.15701 7.68945ZM12.4375 9.18945C12.4375 10.5356 11.3462 11.627 10 11.627V12.752C11.3462 12.752 12.4375 13.8433 12.4375 15.1895H13.5625C13.5625 13.8433 14.6538 12.752 16 12.752V11.627C14.6538 11.627 13.5625 10.5356 13.5625 9.18945H12.4375Z" fill="white"/>
                            </svg>
                            Ask
                ...
                <div class="ai-search-suggestions-ssv13">
                    <p class="ai-search-suggestions-title-ssv13">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10 2.01758C11.0096 2.01758 11.8281 1.1991 11.8281 0.189453H12.6719C12.6719 1.1991 13.4903 2.01758 14.5 2.01758V2.86133C13.4903 2.86133 12.6719 3.67981 12.6719 4.68945H11.8281C11.8281 3.67981 11.0096 2.86133 10 2.86133V2.01758ZM0.25 6.93945C2.73528 6.93945 4.75 4.92474 4.75 2.43945H6.25C6.25 4.92474 8.26472 6.93945 10.75 6.93945V8.43945C8.26472 8.43945 6.25 10.4542 6.25 12.9395H4.75C4.75 10.4542 2.73528 8.43945 0.25 8.43945V6.93945ZM3.15701 7.68945C4.14038 8.23515 4.95432 9.04905 5.5 10.0325C6.04568 9.04905 6.85962 8.23515 7.843 7.68945C6.85962 7.14375 6.04568 6.32985 5.5 5.34646C4.95432 6.32985 4.14038 7.14375 3.15701 7.68945ZM12.4375 9.18945C12.4375 10.5356 11.3462 11.627 10 11.627V12.752C11.3462 12.752 12.4375 13.8433 12.4375 15.1895H13.5625C13.5625 13.8433 14.6538 12.752 16 12.752V11.627C14.6538 11.627 13.5625 10.5356 13.5625 9.18945H12.4375Z" fill="#8B8B8E"/>
                        </svg>
                        Suggestions
                        <svg onclick="document.querySelector('.ai-search-suggestions-ssv13').style.display = 'none';" style="float: right; cursor: pointer;" width="12"
                    ...
                    <p>
                    <div class="JS-ai-search-suggestions-ssv13">
                        ${aiSuggestions}
                    </div>
                </div>           
            </div>
            <p class="aisearch-result-msg-ssv13"></p>
            <div class="aisearch-loader-ssv13">
                <img src="https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn@latest/assets/images/spiner.svg" alt="Spinner">
            </div>
        `;
        aiDiv.innerHTML = aiSearch;
        target.insertAdjacentElement('beforebegin', aiDiv);
    }
    ...
    let fontCSS = `
        @font-face {
            ...
            font-style: normal;
            src: url(https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/live_stream/live_stream_theme/fonts/Greycliff/GreycliffCF-Medium.ttf);
        }
    `;
    fontCSS = '';
    mDiv.innerHTML = `
        <style>
            ...
            .video-modal-sl-video-ssv13.JS-mvideo-L-ssv13 {
                background: ${ssv13_brandCustomizations.bk_color_buy_btn};
            }
        </style>
    ...
    if (no == 0 && ssv13_brandCustomizations.pip_mode == '1') {
        ...
        pDiv.innerHTML = `
            <div class="swirl-short-video-pip-ssv13" onclick="fsPIPssv13();">
                <video style="aspect-ratio: 9/16; background: ${ssv13_brandCustomizations.bk_color_buy_btn}; object-fit: contain;" class="ssv-pip-video-ssv13 ${videoConflictClass}" poster="" loop playsinline muted>                    
                </video>
    ...
    videos.forEach(video => {
        ...
        if (ssv13_brandCustomizations.product_reviews == '1') {
            ssv13_swiper_review[`sw${video.video_id}-${no}`] = new Swiper(`.swiper-review-phone-ssv13-${video.video_id}-${no}`, {
                slidesPerView: 1,
                direction: 'horizontal',
                spaceBetween: 0,
                centeredSlidesBounds: true,
                ...
                init: function () {
                    if (ssv13_brandCustomizations.auto_play == '1') {
                        ...
                        document.querySelectorAll(`.swiper-ssv13-c${no} .carousel-video-ssv13`).forEach(elm => {
                            if (pff > 0) {
                                if (!elm.dataset.src.includes('.m3u8')) {
                                    elm.src = elm.dataset.src;
                                    elm.load();
                                    elm.play();
                                } else {
                                    var hls = new Hls();
                                    hls.loadSource(elm.dataset.src);
                                    hls.attachMedia(elm);
                ...
                slideChange: function () {
                    if (ssv13_brandCustomizations.auto_play == '1') {
                        this.slides.forEach(s => {
                            ...
                            if (vdo.src == '') {
                                if (!vdo.dataset.src.includes('.m3u8')) {
                                    vdo.src = vdo.dataset.src;
                                    vdo.load();
                                    vdo.play();
                                } else {
                                    var hls = new Hls();
                                    hls.loadSource(vdo.dataset.src);
                                    hls.attachMedia(vdo);
                                    hls.on(Hls.Events.MANIFEST_PARSED, function () {
                                        vdo.play();
                                    });
...
function showHidePageLoaderssv13(flag) {
    if (flag) {
        document.querySelector('.aisearch-loader-ssv13').style.display = 'flex';
    } else {
        document.querySelector('.aisearch-loader-ssv13').style.display = 'none';
    }
}
...
function openVideoModalssv13(slideNo, mNo) {
    ...
    //     if (element.src == '') {
    //         if (!element.dataset.src.includes('.m3u8')) {
    //             element.src = element.dataset.src;
    //             element.load();
    //         } else {
    //             var hls = new Hls();
    //             hls.loadSource(element.dataset.src);
...
function playModalVideossv13(slideNo) {
    ...
    // New HLS logic
    let videosToLoad = [];
    videosToLoad.push(vdo);
    // if (document.querySelectorAll(`.swirl-short-videos-m-ssv13-${ssv13_currentModal} .video-modal-sl-video-ssv13`)[slideNo - 1] != null) videosToLoad.push(document.querySelectorAll(`.swirl-short-videos-m-ssv13-${ssv13_currentModal} .video-modal-sl-video-ssv13`)[slideNo - 1]);
    // if (document.querySelectorAll(`.swirl-short-videos-m-ssv13-${ssv13_currentModal} .video-modal-sl-video-ssv13`)[slideNo + 1] != null) videosToLoad.push(document.querySelectorAll(`.swirl-short-videos-m-ssv13-${ssv13_currentModal} .video-modal-sl-video-ssv13`)[slideNo + 1]);
    videosToLoad.forEach(vtl => {
        if (vtl.src == '') {
            if (!vtl.dataset.src.includes('.m3u8')) {
                vtl.src = vtl.dataset.src;
                vtl.load();
            } else {
                var hls = new Hls({
                    testBandwidth: true,
                ...
                hls.loadSource(vtl.dataset.src);
                hls.attachMedia(vtl);
                hls.on(Hls.Events.MANIFEST_PARSED, function () {
                    // vtl.play();
                });

                if (ssv13_subtitle && vtl.dataset.st) {
                    fetch(vtl.dataset.st)
                        .then(response => {
                        ...
                        .then(vttBlob => {
                            ...
                            track.srclang = 'ar';
                            track.src = vttUrl;
                            track.default = true; // Set to true if you want it to be on by default

                            vtl.appendChild(track);
                        })
                        .catch(error => {
    ...
    playPromise.then(_ => {
        ...
        // Analytics code
        initializeVideoDatassv13(vdo.id, vdo.dataset.src, document.querySelectorAll(`.swirl-short-videos-c-ssv13`)[ssv13_currentModal].dataset.playlist);
    }).catch(error => { });
    ...
    if (ssv13_brandCustomizations.pip_mode == '1') {
        ...
        let cPlaylist = document.querySelector(`video[data-srcf="${vdo.dataset.src}"]`).closest('.swirl-short-videos-c-ssv13').dataset.playlist;
    ...
    // Like/dislike
    if (ssv13_brandCustomizations.video_like == '1') {
        let videoId = vdo.dataset.id;
        if (!videosPlayedssv13.includes(videoId)) {
            // Like code            
            ssv13_fsdb.collection("ssv13_like_dislike").doc(videoId).onSnapshot((doc) => {
                if (doc.exists) {
                    let likes = numberToShorthandssv13(doc.data().likes)
                    let dislikes = numberToShorthandssv13(doc.data().dislikes)
                    vdo.closest('.video-modal-sl-video-container-ssv').querySelector('.video-modal-sl-vc-likes-count-ssv13').innerHTML = likes;
                    vdo.closest('.video-modal-sl-video-container-ssv').querySelector('.video-modal-sl-vc-dislikes-count-ssv13').innerHTML = dislikes;

                    let disliked = localStorage.getItem(`ssv13_vdl_${videoId}`) ? localStorage.getItem(`ssv13_vdl_${videoId}`) : null;
                    let liked = localStorage.getItem(`ssv13_vl_${videoId}`) ? localStorage.getItem(`ssv13_vl_${videoId}`) : null;
                    vdo.closest('.video-modal-sl-video-container-ssv').querySelector('.video-modal-sl-vc-like-ssv13 path').setAttribute('fill', liked ? '#77BBFF' : 'white');
                    vdo.closest('.video-modal-sl-video-container-ssv').querySelector('.video-modal-sl-vc-dislike-ssv13 path').setAttribute('fill', disliked ? '#77BBFF' : 'white');
                }
            });
            videosPlayedssv13.push(videoId);
        }
    }

    // review open
    if (ssv13_brandCustomizations.product_reviews == '1') {
        document.querySelectorAll(`.swirl-short-videos-m-ssv13-${ssv13_currentModal} .video-modal-sr-pb-pi-review-btn-ssv13`).forEach(elm => {
            if (!elm.classList.contains('open-ssv13')) {
...
function openPIPssv13(mode = false) {
    ...
    vdo.setAttribute('data-src', pipData.src);

    if (!pipData.src.includes('.m3u8')) {
        vdo.src = pipData.src;
        vdo.load();
    } else {
        var hls = new Hls();
        hls.loadSource(pipData.src);
        hls.attachMedia(vdo);
        hls.on(Hls.Events.MANIFEST_PARSED, function () {
            // vdo.play();
        });
...
function fsPIPssv13() {
    ...
    let src = document.querySelector('.ssv-pip-video-ssv13').dataset.src;
...
function likeVideossv13(vId) {
    let likedVideoId = null;
    if (localStorage.getItem(`ssv13_vl_${vId}`)) {
        likedVideoId = localStorage.getItem(`ssv13_vl_${vId}`);
    } else {
        localStorage.setItem(`ssv13_vl_${vId}`, vId);
    }

    let disliked = localStorage.getItem(`ssv13_vdl_${vId}`) ? localStorage.getItem(`ssv13_vdl_${vId}`) : null;
    if (disliked) {
        localStorage.removeItem(`ssv13_vdl_${vId}`, vId);
    }

    // Add to firestore
    if (!likedVideoId) {
        ssv13_fsdb.collection('ssv13_like_dislike').doc(`${vId}`).get().then((docSnapshot) => {
            if (docSnapshot.exists) {
                var videoCount = ssv13_fsdb.collection('ssv13_like_dislike').doc(`${vId}`);
                videoCount.update({
                ...
                ssv13_fsdb.collection('ssv13_like_dislike').doc(`${vId}`).set({
                    likes: 1,
                    dislikes: 0
                });
            }
...
function dislikeVideossv13(vId) {
    let dislikedVideoId = null;
    if (localStorage.getItem(`ssv13_vdl_${vId}`)) {
        dislikedVideoId = localStorage.getItem(`ssv13_vdl_${vId}`);
    } else {
        localStorage.setItem(`ssv13_vdl_${vId}`, vId);
    }

    let liked = localStorage.getItem(`ssv13_vl_${vId}`) ? localStorage.getItem(`ssv13_vl_${vId}`) : null;
    if (liked) {
        localStorage.removeItem(`ssv13_vl_${vId}`, vId);
    }
        ...
        if (ssv13_brandCustomizations.ajax_cart_class == '') {
            ...
            fetch(wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'add_to_cart'), {
                ...
                if (response.error && response.product_url) {
                    ...
                    // document.body.dispatchEvent(new CustomEvent('added_to_cart', { detail: [response.fragments, response.cart_hash] }));
                }
            }).catch(error => {
