//v10.10 - New design with new swiping up-down + Multiple Product + Multiple Page + PDP PIP (v10 Upgrade) [Reels] feature
...
let ssv_pdpapiURL = 'https://bigleap.live/index.php/ShortVideo/getmultiple_productvideo'; // bigleap.live  ,  api.goswirl.live
        ...
        if (ssv_pdppip || ssv_storePlaylist) {
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    } else if (Object.keys(data.swilrs).length > 0 && Object.keys(data.swilrs.video).length > 0) {
...
function generatessv(videos) {
    ...
    $(".video-modal-product-tile-carousel-ssv").on('touchmove', function (e) { e.preventDefault(); });
}
...
function nextPrevTilessv(btn, act) {
    ...
    let scrollW = (elm[0].scrollWidth / $(elm).find('.video-modal-product-tile-ssv').length) + 2;

    if (act == 'next') {
        $(elm).animate({ scrollLeft: (scrollW + elm[0].scrollLeft) }, 500);
    } else {
        $(elm).animate({ scrollLeft: (elm[0].scrollLeft - scrollW) }, 500);
    }

    setTimeout(() => {
        if ((elm[0].scrollWidth - $(elm).find('.video-modal-product-tile-ssv').width()) - (elm[0].scrollLeft) > 100) jQuery(btn).closest('.video-modal-video-container-ssv').find('.video-modal-product-tile-carousel-ssv .video-modal-product-tile-next-ssv').show();
