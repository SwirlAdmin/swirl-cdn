//v10.3.1 - New design with new swiping up-down + Multiple products + Multiple pagey (v10 Upgrade) [Reels] feature
...
let ssv_store = '';
let ssv_storeType = '0';
let ssv_storeCode = ''; let ssv_storePlaylist = '';
let ssv_baseURL = ssv_mode === 'Live' ? 'https://apigoswirl.com/short_video/v10/multiple_page_pip/style.min.css' : '';
let ssv_responseData = [];
let ssv_productIds = [];
let ssv_apiURL = 'https://bigleap.live/index.php/shopify/videolistingV4'; // bigleap.live  ,  api.goswirl.live
let ssv_swiper = [];
...
function executessv() {
    ...
    if (document.querySelector('#swirl-short-videos') != null || localStorage.getItem('_ssv_pip')) {
        ...
        $.getScript("https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.13/js/intlTelInput-jquery.min.js", function () {
            JSL1 = true;
        });

        $.getScript("https://cdn.gumlytics.com/insights/1.1/gumlet-insights.min.js", function () {
            JSL2 = true;
        });

        $.getScript("https://www.gstatic.com/firebasejs/9.19.1/firebase-app-compat.js", function () {
            JSL3 = true;
        });

        $.getScript("https://www.gstatic.com/firebasejs/9.19.1/firebase-auth-compat.js", function () {
            JSL4 = true;
        });

        $.getScript("https://www.gstatic.com/firebasejs/9.19.1/firebase-firestore-compat.js", function () {
            JSL5 = true;
        });
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                if (Object.keys(data.swilrs).length > 0 && ssv_storePlaylist) {
                    ...
                    let SSVSL = setInterval(() => {
                        if (JSL1 && JSL2 && JSL3 && JSL4 && JSL5) {
                            ...
                            generatessv(ssv_responseData.swilrs, ssv_storePlaylist.split(','));
                            clearInterval(SSVSL);
                        }
                    }, 300);
...
function generatessv(swirls, playlistCodes) {
    if (Object.keys(playlistCodes).length) {
        appendPIPssv();

        playlistCodes.forEach(pCode => {
            let cPlaylist = jQuery(`.swirl-short-videos-playlist[data-playlist="${pCode}"]`).length ? jQuery(`.swirl-short-videos-playlist[data-playlist="${pCode}"]`) : '';
            let cPlaylistCode = pCode;
            ...
            swipeupTooltip = '';
            let i = 0;
            let pipPlaylist = false;
            let pipVideo = localStorage.getItem('_ssv_pip') ? atob(localStorage.getItem('_ssv_pip')) : '';

            videos.forEach(video => {
                // testing
                // video.video_url = video.cover_video = 'https://livevideoshopping.in/test/pre.mp4';

                if (typeof video.product[0] != 'undefined') {
                    if (pipVideo == video.video_url) pipPlaylist = true;
                        ...
                        onpageSlides += `
                            <div class="swiper-slide ms-${video.video_id}" onclick="playssv(${i}, '${cPlaylistCode}')">
                                ...
                                <div class="product-on-carousel-ssv" ${ssv_brandCustomizations.product_blog_img == '0' ? 'style="display: none !important;"' : ''}>                                    
                                    ...
                                    <h6 style="${video.product.length > 1 ? '' : 'display: none;'} background: ${ssv_brandCustomizations.bk_color_buy_btn} !important; color: ${ssv_brandCustomizations.front_color_buy_btn} !important;">${video.product.length}</h6>
                                    <p>${video.product[0].title}</p>
                                    <label ${ssv_brandCustomizations.product_price_status == '0' ? 'style="display: none !important;"' : ''}>₹${video.product[0].discount_price} <strike ${parseFloat(video.product[0].price) > parseFloat(video.product[0].discount_price) ? '' : 'style="display: none;"'}>₹${video.product[0].price}</strike>
                    ...
                    if (Object.keys(video.product).length > 1) {
                        ...
                        productTile += `
                            <button style="display: none;" onclick="nextPrevTilessv(this, 'prev');" class="video-modal-product-tile-prev-ssv">
                                <img src="https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn@latest/assets/images/back-btn.svg" alt="Previous icon">
                            </button>
                            <button onclick="nextPrevTilessv(this, 'next');" class="video-modal-product-tile-next-ssv">
                                <img src="https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn@latest/assets/images/next-btn.svg" alt="Next icon">
                            </button>
                        `;
                                ...
                                <div class="video-modal-product-tile-ssv" onclick="openListProductssv(this, ${pi});">
                                    <div class="product-info-pt-ssv" style="${ssv_brandCustomizations.product_tile_cta_btn == '0' ? 'style="width: 100% !important;"' : ''}">
                                        ...
                                        <div class="product-detail-pt-ssv" style="${ssv_brandCustomizations.mobile_product_tile_img == '1' ? 'width: calc(100% - 75px) !important;' : ''}">
                                            ...
                                            <label ${ssv_brandCustomizations.product_price_status == '0' ? 'style="display: none !important;"' : ''}>₹${product.discount_price} <strike ${parseFloat(product.price) > parseFloat(product.discount_price) ? '' : 'style="display: none;"'}>₹${product.price}</strike>
                                                <br><span ${parseFloat(product.price) > parseFloat(product.discount_price) ? '' : 'style="display: none;"'}>${parseFloat(product.price) > parseFloat(product.discount_price) ? Math.round(((product.price - product.discount_price) * 100) / product.price) : ''}% OFF</span></label>
                                        </div>
                                    </div>
                        ...
                        productList = `
                            <div class="video-modal-product-block-product-ssv">
                                ...
                                <div class="product-bottom-pb-ssv">
                                    <button class="product-addtocart-pb-ssv" onclick="addtocartssv(${video.video_id}, ${video.product[0].product_id}, this); CTAClicksssv(${video.product[0].product_id}, '${video.product[0].title}', '${video.product[0].image}', '${video.product[0].url}', ${video.designer_id}, ${video.video_id}, '2');" style="background: none !important; border: 2px solid ${ssv_brandCustomizations.front_color_add_to_cart_btn} !important; color: ${ssv_brandCustomizations.front_color_add_to_cart_btn} !important; ${ssv_brandCustomizations.add_to_cart == '0' ? 'display: none !important;' : ''} ${ssv_brandCustomizations.buy_now == '0' ? 'width: calc(100% - 65px) !important;' : ''}">${ssv_brandCustomizations.add_to_cart_btn}</button>
            ...
            jQuery('body').append(`
                <div class="swirl-short-videos" data-playlist="${cPlaylistCode}">
                    ...
                    <div class="video-modal-ssv video-modal-ssv-${cPlaylistCode}" data-playlist="${cPlaylistCode}" style="display: none;">
            ...
            if (!ssv_pip) {
                ...
                ssv_swiper[cPlaylistCode] = new Swiper(`.swiper-ssv-c-${cPlaylistCode}`, {
                    ...
                    on: {
                        init: function () {
                            updateSwiperNavigationssv(`.swiper-ssv-c-${cPlaylistCode}`);
                        }
                    }
                });
            }
                ...
                on: {
                    init: function () {
                        ...
                        if (ssv_brandCustomizations.next_previous_preview == '1') {
                            updatePreviewThumbsssv(`.swiper-ssv-m-${cPlaylistCode}`);
                        }
                    },
                }
            });
        ...
        // Tile touch scroll disable
        jQuery(".video-modal-product-tile-carousel-ssv").on('touchmove', function (e) { e.preventDefault(); });

        if (getMobileOperatingSystemssv() == 'iOS') {
            document.querySelectorAll('.video-modal-askque-modal-ssv textarea, .video-modal-askque-modal-ssv input').onfocus = function () {
                window.scrollTo(0, 0);
                document.body.scrollTop = 0;
            }
        }
    } else {
        console.log('SSV Playlist absent.');
    }
}

function updatePreviewThumbsssv(swpr) {
    jQuery(swpr + ' .swiper-slide').each(function () {
        $(this).prepend($(this).prev().length ? '<img src="' + $(this).prev().find('video').attr('poster') + '" class="video-modal-np-img-ssv p-img-ssv" onclick="jQuery(`.video-modal-ssv:visible`).find(`.swiper-button-prev`).click();">' : '');
        $(this).prepend($(this).next().length ? '<img src="' + $(this).next().find('video').attr('poster') + '" class="video-modal-np-img-ssv n-img-ssv" onclick="jQuery(`.video-modal-ssv:visible`).find(`.swiper-button-next`).click();">' : '');
    })
}
...
function openProductssv(btn) {
    ...
    if (window.innerWidth <= 768) {
        let pl = jQuery(btn).parents('.video-modal-ssv').data('playlist');
        ssv_swiper_modal[pl].allowSlideNext = false;
        ssv_swiper_modal[pl].allowSlidePrev = false;
        ssv_swiper_modal[pl].allowTouchMove = false;
    }
}
...
function closeProductssv(btn) {
    ...
    if (window.innerWidth <= 768) {
        let pl = jQuery(btn).parents('.video-modal-ssv').data('playlist');
        ssv_swiper_modal[pl].allowSlideNext = true;
        ssv_swiper_modal[pl].allowSlidePrev = true;
        ssv_swiper_modal[pl].allowTouchMove = true;
    }
}
...
function setPopupHightssv() {
    ...
    // Navigations
    // setTimeout(() => {
    //     let activeModal = jQuery('.swiper-ssv-m:visible');
    //     let videoWidth = jQuery(activeModal).find('.swiper-slide-active .video-modal-video-container-ssv').width() + 80;
...
function fullscreenpipssv(btn) {
    ...
    closepipssv();
    playssv(slideNo, jQuery('.swiper-ssv-m video source[src="' + video + '"]').parents('.video-modal-ssv').data('playlist'));
}
...
function updateSwiperNavigationssv(swpr) {
    jQuery('.swiper-slide-c').each(function () {
