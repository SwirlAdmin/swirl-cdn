//v10 - New design with new swiping up-down [Reels] feature
...
let ssv_storeType = '0';
let ssv_storeCode = '';
let ssv_baseURL = ssv_mode === 'Live' ? 'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v10/swirl-short-videos-v1.min.css' : 'https://goswirl.shop/swirl-embed/short-videos-carousel/v10/style.css';
let ssv_responseData = [];
let ssv_productIds = [];
let ssv_apiURL = 'https://api.goswirl.live/index.php/ShortVideo';
let ssv_swiper = [];
...
function generatessv(videos) {
        ...
        if (!ssv_pip) {
            onpageSlides += `
                <div class="swiper-slide ms-${video.video_id}" onclick="playssv(${i})">
                    <video ${ssv_brandCustomizations.product_blog_img == '0' ? 'style="margin-bottom: 0 !important;"' : ''} class="carousel-video-ssv" poster="${video.cover_image}" ${ssv_brandCustomizations['auto_play'] === '1' && i < 5 ? 'autoplay' : ''} loop onplay="jQuery(this).next().hide();" onpause="jQuery(this).next().show();" ${ssv_brandCustomizations.time_sec === '1' ? 'onloadeddata="updateDurationssv(this);" ontimeupdate="updateProgressDurationssv(this);"' : ''} playsinline="" preload="metadata" data-setup="{}" muted>
        ...
        modalSlides += `
            <div class="swiper-slide">
                <div class="video-modal-video-container-ssv">
                    <video loop playsinline="" preload="none" data-setup="{}" onplay="jQuery(this).next().hide()"
                    ...
                    <div class="video-modal-video-controls-ssv" onmouseout="hideControlsssv(this);">
                    ...
                    <div class="video-modal-product-block-ssv">                                                             
                        ...
                        <div class="product-middle-pb-ssv">                            
                            ...
                            <div class="product-quantity-pb-ssv" style="${ssv_storeType == '0' ? 'display: none !important;' : ''}">
                        ...
                        <div class="product-bottom-pb-ssv">
                            <button class="product-addtocart-pb-ssv" onclick="addtocartssv(${video.video_id}, this);" style="background: none !important; border: 2px solid ${ssv_brandCustomizations.front_color_add_to_cart_btn} !important; color: ${ssv_brandCustomizations.front_color_add_to_cart_btn} !important; ${ssv_storeType == '0' ? 'display: none !important;' : ''}">${ssv_brandCustomizations.add_to_cart_btn}</button>
                            <a href="${video.product[0].url}">
                            <button class="product-buynow-pb-ssv" style="background: ${ssv_brandCustomizations.bk_color_buy_btn} !important; color: ${ssv_brandCustomizations.front_color_buy_btn} !important; ${ssv_storeType == '0' ? 'width: 100% !important; margin: 0 !important' : ''}">${video.cta_customization ? video.cta_customization : ssv_brandCustomizations.buy_btn}</button>
                            </a>
                            <a href="https://${ssv_store}/cart">
                            <button class="product-cart-pb-ssv" style="background: none !important; border: 2px solid ${ssv_brandCustomizations.front_color_add_to_cart_btn} !important; color: ${ssv_brandCustomizations.front_color_add_to_cart_btn} !important; ${ssv_storeType == '0' ? 'display: none !important;' : ''}">
                                ...
                                <label class="cart-count-ssv" style="background: ${ssv_brandCustomizations.bk_color_buy_btn} !important; color: ${ssv_brandCustomizations.front_color_buy_btn} !important;"></label>
                            </button>
                            </a>
                        </div>
                    </div>                    
                    ...
                    <div class="video-modal-askque-modal-ssv">
                        ...
                        <div class="askque-modal-form-ssv askque-modal-form2-ssv">
                            <form onsubmit="return registerUserssv(this, ${video.video_id});">
                                <input name="fullname" type="text" placeholder="Enter your name" style="margin-bottom: 8px !important;" required />
                                <input name="code" id="code${video.video_id}" type="hidden" value="91" />
                                <input name="phone" id="phone${video.video_id}" type="text" style="margin: 0px !important;" placeholder="Enter your phone" onkeypress='return event.charCode >= 48 && event.charCode <= 57' required />                                
    ...
    // Slider on page
    jQuery('#swirl-short-videos').html(!ssv_pip ? `
        ...
        <div class="swirl-short-videos">
            ...
            <div class="video-pip-ssv">
                <video loop playsinline="" preload="none" data-setup="{}" onplay="jQuery(this).next().hide()" onmouseover="showPipControls();"                   
                    ...
                    <source src="https://video.gumlet.io/621770ca1c8b821b05d7035a/63ad92c3edd12e42882a6216/main.mp4" type="video/mp4">
                </video>
    ...
    if (ssv_storeType == '1') {  // Shopify        
        ...
        if (ssv_brandCustomizations.ajax_cart_class == '') {
            ...
            setTimeout(() => {
                ...
                if (jQuery('.xoo-wsc-items-count').length > 0) {
                    jQuery('.cart-count-ssv').html((parseInt(jQuery('.xoo-wsc-items-count').html()) + quantity));
                }
            }, 1000);
        }
    }
...
function askQuestionssv(form, designerId, videoId) {
    ...
    if (ssv_userData) {
        let userId = ssv_userData.user_id;

        jQuery.ajax({
            type: "POST",
            dataType: "json",
            url: "https://api.goswirl.live/index.php/Shopify/askque",
            data: "user_id=" + encodeURIComponent(userId) + "&designer_id=" + encodeURIComponent(designerId) + "&msg=" + encodeURIComponent(question) + "&swirls_id=" + encodeURIComponent(videoId),
            beforeSend: function () {
                videoloaderssv(true);
            },
...
function registerUserssv(form) {
    ...
    jQuery.ajax({
        ...
        data: "name=" + encodeURIComponent(formData[0]['value']) + "&code=" + encodeURIComponent(formData[1]['value']) + "&mobile=" + encodeURIComponent(formData[2]['value']),
        beforeSend: function () {
            videoloaderssv(true);
        },
        success: function (response) {
            if (response.success == true) {
                localStorage.setItem('_ssv_user', JSON.stringify(response.data));
                ssv_userData = response.data;
