...
let ssv_storeType = '0';
let ssv_storeCode = '';
let ssv_baseURL = ssv_mode === 'Live' ? 'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v10/swirl-short-video-v9.min.css' : '';
let ssv_responseData = [];
let ssv_productIds = [];
let ssv_apiURL = 'https://api.goswirl.live/index.php/ShortVideo'; // bigleap.live  ,  api.goswirl.live
let ssv_swiper = [];
...
function generatessv(videos) {
        ...
        modalSlides += `
            <div class="swiper-slide">
                <div class="video-modal-video-container-ssv">
                    ...
                    <button class="video-modal-like-ssv" title="Like" onclick="likeVideossv(this, ${video.video_id});" style="background: rgb(0, 0, 0, .6) !important;">
    ...
    videos.forEach(video => {
        ...
        ssv_fsdb.collection("swirlshortvideo_likes").doc(video.video_id).onSnapshot((doc) => {
            if (doc.exists) {
                let likes = nFormatterssv(doc.data().unique_likes)
                $(`.videoLikes-${video.video_id}`).html(likes);
            }
        });
...
function askQuestionssv(form, designerId, videoId) {
    ...
    if (ssv_userData) {
        videoloaderssv(true);
        ssv_fsdb.collection("swirlshortvideo_askquestion").doc(`${videoId}`).collection('queries').add({
...
function CTAClicksssv(pId, pTitle, pImage, pURL, dId, vId, cType) {
    ssv_fsdb.collection("swirlshortvideo_cta").doc(`${vId}`).collection('clicks').add({
...
function likeVideossv(btn, videoId) {
    ...
    // Add to firestore
    ssv_fsdb.collection('swirlshortvideo_likes').doc(`${videoId}`).get().then((docSnapshot) => {
        if (docSnapshot.exists) {
            var videoCount = ssv_fsdb.collection('swirlshortvideo_likes').doc(`${videoId}`);
            videoCount.update({
            ...
            ssv_fsdb.collection('swirlshortvideo_likes').doc(`${videoId}`).set({
                total_likes: 1,
                unique_likes: 1
            });
        }
...
function SWIRLHeartFlawsssv(elm) {
    ...
    $('<div class="SWIRLheart part-' + b + " " + a[Math.floor((Math.random() * 6))] + '" style="font-size:' + Math.floor(Math.random() * (50 - 22) + 22) + 'px;"><span></span></div>').appendTo(elm).css({
