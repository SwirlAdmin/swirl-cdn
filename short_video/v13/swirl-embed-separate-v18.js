var ssv13_videoDataArray = [], ssv13_currentVideoTimer = null; function generateUUIDssv13() { var e = new Date().getTime(), t = "undefined" != typeof performance && performance.now && 1e3 * performance.now() || 0; return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function (i) { var s = 16 * Math.random(); return e > 0 ? (s = (e + s) % 16 | 0, e = Math.floor(e / 16)) : (s = (t + s) % 16 | 0, t = Math.floor(t / 16)), ("x" === i ? s : 3 & s | 8).toString(16) }) } function getDeviceTypessv13() { let e = window.innerWidth; return e <= 767 ? "Mobile" : e >= 768 && e <= 1024 ? "Tablet" : "Desktop" } async function initializeVideoDatassv13(e, t, i) { let s = !1, a = ssv13_videoDataArray.find(t => t.id === e), n = localStorage.getItem("ssv13_responseData"), r = JSON.parse(n), o = document.getElementById(e), d = a || { id: e, url: t, unique_views: 0, watch_time: 0, brand_id: r.swirls.customization.brand_id, total_views: 0, duration: r?.swirls[i]?.find(e => e.server_url == t)?.video_len, video_title: r?.swirls[i]?.find(e => e.server_url == t)?.video_title, drop_of_point: [], skip_points: [], segments: [], location_details: {}, system_detail: { swirl_machine_id: generateUUIDssv13(), device_type: getDeviceTypessv13() } }; initializeSegmentssv13(d), ssv13_currentVideoTimer && clearInterval(ssv13_currentVideoTimer), a && (d.watch_time = parseInt(a.watch_time || 0, 10)), o.addEventListener("timeupdate", () => { let e = Math.floor(o.currentTime); d.segments.find(t => e >= t.start && e <= t.end), 0 == e || s || (d.unique_views = 1, d.total_views += 1, s = !0, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }), ssv13_currentVideoTimer = setInterval(() => { o.paused || (tenPercent = Math.floor(.1 * d.duration), d.watch_time += 1, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }, 1e3), o.addEventListener("pause", () => { let e = Math.floor(o.currentTime), t = d.segments.find(t => e >= t.start && e <= t.end); t && (d.drop_of_point.push({ segment_id: t.segment_id, timestamp: e }), localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }), o.addEventListener("ended", () => { let e = Math.floor(o.currentTime), t = d.segments.find(t => e >= t.start && e <= t.end); t && (d.drop_of_point.push({ segment_id: t.segment_id, timestamp: e }), localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }), o.addEventListener("seeked", () => { let e = Math.floor(o.currentTime), t = d.segments.find(t => e >= t.start && e <= t.end); t && (0 == d?.skip_points.length || d.skip_points[d.skip_points.length - 1].to ? d.skip_points.push({ from: { segmentId: t.segmentId, timeStamp: e }, to: null }) : d.skip_points[d.skip_points.length - 1].to = { segmentId: t?.segmentId, timeStamp: e }, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }), a ? Object.assign(a, d) : ssv13_videoDataArray.push(d) } function initializeSegmentssv13(e) { for (let t = 0; t < Math.ceil(e.duration / 3); t++)e.segments.push({ segment_id: t + 1, start: 3 * t, end: Math.min((t + 1) * 3, e.duration) }) } function pushAnalyticsdatassv13() { substring_to_remove = "ssv13-video-m-"; let e = JSON.parse(localStorage.getItem("_all_video_data")), t = e?.map(async e => (e.video_id = e.id.replace(substring_to_remove, ""), e)); t.length > 0 && Promise.all(t).then(async e => { await fetch("https://goswirl.world:3001/insertVideoData", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payloads: e }) }).then(e => { if (ssv13_videoDataArray = [], !e.ok) throw Error("Network response was not ok"); localStorage.removeItem("_all_video_data") }).catch(e => { console.error("Error sending data:", e) }), await fetch("https://analytics-api.goswirl.live/engagement", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payloads: e }) }).then(e => { if (ssv13_videoDataArray = [], !e.ok) throw Error("Network response was not ok"); localStorage.removeItem("_all_video_data") }).catch(e => { console.error("Error sending data:", e) }) }).catch(e => console.log(e)) } async function impressionCountssv13(e, t, i) { let s = window.location.href.split("?")[0], a = localStorage.getItem("uuid"), n = localStorage.getItem("timestamp"); if (a && n) { let r = new Date().getTime(); if (r - parseInt(n, 10) < 144e5) { console.log("Using existing UUID:", a); return } } let o = generateUUIDssv13(); localStorage.setItem("uuid", o), localStorage.setItem("timestamp", new Date().getTime().toString()), e = parseInt(e); await fetch("https://goswirl.world:3001/insertImpression", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ impressionData: { id: o, playlist: t || "", current_url: s, brand_id: e, video_ids: i } }) }).then(e => { }).catch(e => { console.error("Error sending data:", e) }); await fetch("https://analytics-api.goswirl.live/impression", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payload: { id: o, playlistCode: t || "", url: s, brandId: e, videoIds: i } }) }).then(e => { }).catch(e => { console.error("Error sending data:", e) }) } window.addEventListener("unload", () => { let e = localStorage.getItem("_all_video_data"); if (e) try { let t = JSON.parse(e); localStorage.removeItem("_all_video_data"); let i = t?.map(async e => (e.video_id = e.id.replace("ssv13-video-m-", ""), e)); if (i) { try { let s = navigator.sendBeacon("https://goswirl.world:3001/insertVideoDataOnWindowclose", JSON.stringify(t)); if (!s) throw Error("Beacon transmission failed"); localStorage.removeItem("_all_video_data") } catch (a) { console.error("Error sending data:", a) } try { let n = navigator.sendBeacon("https://analytics-api.goswirl.live/engagement/onclose", JSON.stringify(t)); if (!n) throw Error("Beacon transmission failed"); localStorage.removeItem("_all_video_data") } catch (r) { console.error("Error sending data:", r) } } } catch (o) { localStorage.removeItem("_all_video_data"), console.error("Error parsing analytics data:", o) } });
...
(() => {
    ...
    jqTag.href = window.location.href.includes('short-videos-carousel/v13') ?
        'style.css' :
        document.querySelector('script[src="https://goswirlreact1.in/swirl-embed/short-videos-carousel/v13/swirl-embed.js"]') ?
            'https://goswirlreact1.in/swirl-embed/short-videos-carousel/v13/style.css' :
            'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/v13/style-v5.min.css';
