let ssv13_videoDataArray = [];
let ssv13_currentVideoTimer = null;
...
async function initializeVideoDatassv13(videoId, videoUrl, playlist) {
    ...
    const videoData = existingVideoData || {
        ...
        unique_views: 0,
        watch_time: 0,
        brand_id: allDataofSwirls.swirls.data.brand_id,
        total_views: 0,
        duration: allDataofSwirls?.swirls?.swirls[playlist]?.find((el) => {
            return el.server_url == videoUrl;
        })?.video_len,
        video_title: allDataofSwirls?.swirls?.swirls[playlist]?.find((el) => {
            return el.server_url == videoUrl;
        })?.video_title,
        drop_of_point: [],
        skip_points: [],
