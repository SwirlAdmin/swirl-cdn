...
var ssv13_videoDataArray = [], ssv13_currentVideoTimer = null; function generateUUIDssv13() { var e = (new Date).getTime(), t = "undefined" != typeof performance && performance.now && 1e3 * performance.now() || 0; return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (function (a) { var i = 16 * Math.random(); return e > 0 ? (i = (e + i) % 16 | 0, e = Math.floor(e / 16)) : (i = (t + i) % 16 | 0, t = Math.floor(t / 16)), ("x" === a ? i : 3 & i | 8).toString(16) })) } function getDeviceTypessv13() { const e = window.innerWidth; return e <= 767 ? "Mobile" : e >= 768 && e <= 1024 ? "Tablet" : "Desktop" } async function initializeVideoDatassv13(e, t) { let a = !1; const i = ssv13_videoDataArray.find((t => t.id === e)), o = localStorage.getItem("ssv13_responseData"), s = JSON.parse(o), n = document.getElementById(e), r = i || { id: e, url: t, unique_views: 0, watch_time: 0, brand_id: s.data.customization.brand_id, total_views: 0, duration: s?.data.video?.find((e => e.server_url == t))?.video_len, video_title: s?.data.video?.find((e => e.server_url == t))?.video_title, drop_of_point: [], skip_points: [], segments: [], location_details: {}, system_detail: { swirl_machine_id: generateUUIDssv13(), device_type: getDeviceTypessv13() } }; initializeSegmentssv13(r), ssv13_currentVideoTimer && clearInterval(ssv13_currentVideoTimer), i && (r.watch_time = parseInt(i.watch_time || 0, 10)), n.addEventListener("timeupdate", (() => { const e = Math.floor(n.currentTime); r.segments.find((t => e >= t.start && e <= t.end)); 0 == e || a || (r.unique_views = 1, r.total_views += 1, a = !0, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), ssv13_currentVideoTimer = setInterval((() => { n.paused || (tenPercent = Math.floor(.1 * r.duration), r.watch_time += 1, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) }), 1e3), n.addEventListener("pause", (() => { const e = Math.floor(n.currentTime), t = r.segments.find((t => e >= t.start && e <= t.end)); t && (r.drop_of_point.push({ segment_id: t.segment_id, timestamp: e }), localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), n.addEventListener("ended", (() => { const e = Math.floor(n.currentTime), t = r.segments.find((t => e >= t.start && e <= t.end)); t && (r.drop_of_point.push({ segment_id: t.segment_id, timestamp: e }), localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), n.addEventListener("seeked", (() => { const e = Math.floor(n.currentTime), t = r.segments.find((t => e >= t.start && e <= t.end)); t && (0 == r?.skip_points.length || r.skip_points[r.skip_points.length - 1].to ? r.skip_points.push({ from: { segmentId: t.segmentId, timeStamp: e }, to: null }) : r.skip_points[r.skip_points.length - 1].to = { segmentId: t?.segmentId, timeStamp: e }, localStorage.setItem("_all_video_data", JSON.stringify(ssv13_videoDataArray))) })), i ? Object.assign(i, r) : ssv13_videoDataArray.push(r) } function initializeSegmentssv13(e) { for (let t = 0; t < Math.ceil(e.duration / 3); t++)e.segments.push({ segment_id: t + 1, start: 3 * t, end: Math.min(3 * (t + 1), e.duration) }) } function pushAnalyticsdatassv13() { substring_to_remove = "ssv13-video-m-"; const e = JSON.parse(localStorage.getItem("_all_video_data")), t = e?.map((async e => (e.video_id = e.id.replace(substring_to_remove, ""), e))); t.length > 0 && Promise.all(t).then((async e => { await fetch("https://analytics-api.goswirl.live/engagement", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payloads: e }) }).then((e => { if (ssv13_videoDataArray = [], !e.ok) throw new Error("Network response was not ok"); localStorage.removeItem("_all_video_data") })).catch((e => { console.error("Error sending data:", e) })) })).catch((e => console.log(e))) } async function impressionCountssv13(e, t, a) { let i = window.location.href.split("?")[0]; const o = localStorage.getItem("uuid"), s = localStorage.getItem("timestamp"); if (o && s) { const e = 144e5; if ((new Date).getTime() - parseInt(s, 10) < e) return void console.log("Using existing UUID:", o) } const n = generateUUIDssv13(); localStorage.setItem("uuid", n), localStorage.setItem("timestamp", (new Date).getTime().toString()); let r = { id: n, playlistCode: t || "", url: i, brandId: e, videoIds: a }; await fetch("https://analytics-api.goswirl.live/impression", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ payload: r }) }).then((e => { })).catch((e => { console.error("Error sending data:", e) })) } window.addEventListener("unload", (() => { const e = localStorage.getItem("_all_video_data"); if (e) try { const t = JSON.parse(e); localStorage.removeItem("_all_video_data"); const a = t?.map((async e => (e.video_id = e.id.replace("ssv13-video-m-", ""), e))); if (a) try { navigator.sendBeacon("https://analytics-api.goswirl.live/engagement/onclose", JSON.stringify(t)) || console.log("Beacon transmission failed"), localStorage.removeItem("_all_video_data") } catch (e) { console.error("Error sending data:", e) } } catch (e) { localStorage.removeItem("_all_video_data"), console.error("Error parsing analytics data:", e) } }));
