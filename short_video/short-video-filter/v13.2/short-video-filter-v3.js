...
(() => {
    ...
    jqTag.href = window.location.href.includes('short-videos-carousel/v13') ?
        'style.css' :
        document.querySelector('script[src="https://goswirl.shop/swirl-embed/short-videos-carousel/v13.2/swirl-embed.js"]') ?
            'https://goswirl.shop/swirl-embed/short-videos-carousel/v13.2/style.css' :
            'https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn/short_video/short-video-filter/v13.2/style-v3.min.css';
...
function initssv13() {
    ...
    if (ssv13_storeCode || ssv13_pipMode) {
        fetch(`https://bigleap.live/index.php/ShortVideo/getFiltersv13_2?sCode=${ssv13_storeCode}&limit=${ssv13_videoPerPage}`, requestOptions)
...
function generateFilterssv13(filter, product, target, no) {
    ...
    filterHTML += `<div class="filters-left-ssv13">        
        <svg class="filters-filter-btn-ssv13" style="display: none;" onclick="expandFilterSearchssv13();" width="20" height="20" viewBox="0 0 256 256">
                ...
                filterHTML += `
                    <div class="dropdown-ssv13">
                        <button type="button" onclick="" class="dropbtn-ssv13">
                            <input name="menuFilter[]" onchange="document.querySelector('#filters-master-form-submit-ssv13').click();" style="position: absolute; opacity: 0; cursor: pointer; left: 0; width: 100%; height: 100%; margin: 0;" type="checkbox" class="option" value="${f.video_id}">
                            ${f.f_name}
                        </button>                        
                    </div>                        
                `;
    ...
    filterHTML += `<div class="filters-right-ssv13">
        <svg class="filters-search-btn-ssv13" style="display: block;" onclick="expandFilterSearchssv13();" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    filterHTML += ['236633366', '18581'].some(el => ssv13_brandCustomizations.brand_id == el) ? `
        <style>
...
function filterVideossv13() {
    ...
    if (vIds || pIds) {
        ...
        fetch(`https://bigleap.live/index.php/ShortVideo/getFilteredVideosv13_2?brandId=${ssv13_brandCustomizations.brand_id}&videoIds=${vIds}&productIds=${pIds}&limit=${ssv13_videoPerPage}&page=${pageNo}`, requestOptions).then(response => response.text()).then(result => {
            let data = JSON.parse(result);
...
function pageChangessv13(btn, page, vType) {
    ...
    if (vType == 'D') {
        if (page == '0') {
            ...
            fetch(`https://bigleap.live/index.php/ShortVideo/getNextDefaultVideosv13_2?brandId=${ssv13_brandCustomizations.brand_id}&limit=${ssv13_videoPerPage}&page=${ssv13_videoCurrentPage}`, requestOptions).then(response => response.text()).then(result => {
                let data = JSON.parse(result);
                if (Object.keys(data.data.video).length) {
                    ssv13_swiper = [];
                    ssv13_swiper_modal = [];
...
function generatessv13(videos, videoTotal, videoType, target, no) {
    ...
    videos.forEach(video => {
        ...
        video.product.forEach(prd => {
            ...
            let dpcHeight = 162;
            dpcHeight = ssv13_brandCustomizations.product_reviews == '0' && ssv13_brandCustomizations.product_price_status == '0' ? 142 : dpcHeight;
            mProductList += `
