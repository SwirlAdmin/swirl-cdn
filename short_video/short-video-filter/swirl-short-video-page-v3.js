...
function initssv13() {
    ...
    if (ssv13_storeCode || ssv13_pipMode) {
        fetch(`https://bigleap.live/index.php/ShortVideo/getFiltersv13?sCode=${ssv13_storeCode}&limit=${ssv13_videoPerPage}`, requestOptions)
            ...
            .then(result => {
                ...
                ssv13_brandCustomizations.comment_mode = 0;
                // ssv13_brandCustomizations.auto_play_mute_un = '0';
                // ssv13_brandCustomizations.product_price_status = '1';
                // ssv13_brandCustomizations.buy_now = 0;
                // ssv13_brandCustomizations.pdppip_hidecarousel = 1;
                // ssv13_brandCustomizations.pdppip = 1;
                // ssv13_brandCustomizations.cta_on_tile = 0;
                // ssv13_brandCustomizations.next_previous_preview = 0;
                // ssv13_brandCustomizations.show_add_to_cart_popup = 1;
...
function filterVideossv13() {
    ...
    if (fValues || pIds) {
        ...
        fetch(`https://bigleap.live/index.php/ShortVideo/getFilteredVideosv13?brandId=${ssv13_brandCustomizations.brand_id}&filter=${fValues}&productIds=${pIds}&limit=${ssv13_videoPerPage}&page=${pageNo}`, requestOptions).then(response => response.text()).then(result => {
            let data = JSON.parse(result);
...
function pageChangessv13(btn, page, vType) {
    ...
    if (vType == 'D') {
        if (page == '0') {
            ...
            fetch(`https://bigleap.live/index.php/ShortVideo/getNextDefaultVideosv13?brandId=${ssv13_brandCustomizations.brand_id}&limit=${ssv13_videoPerPage}&page=${ssv13_videoCurrentPage}`, requestOptions).then(response => response.text()).then(result => {
                let data = JSON.parse(result);
                if (Object.keys(data.data.video).length) {
                    ssv13_swiper = [];
                    ssv13_swiper_modal = [];
        ...
        mSlides += `
            <div class="swiper-slide swiper-slide-m">
                <div class="video-modal-sl-video-container-ssv ${!Object.keys(video.product).length ? 'no-products-slide-ssv13' : ''}">
                    <video id="ssv13-video-m-${video.video_id}" data-src="${video.server_url}" data-products="${Object.keys(video.product).length}" data-id="${video.video_id}" class="video-modal-sl-video-ssv13"
