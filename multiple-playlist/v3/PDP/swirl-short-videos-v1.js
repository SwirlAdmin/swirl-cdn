...
(function () {
    if (localStorage.getItem('SSVallURL') && localStorage.getItem('SSVlastPlayed') && document.querySelector('#swirl-short-videos') == null) {
        ...
        jqTag.href = 'https://apigoswirl.com/short_video/v3/pdp/swir-short-videos-v1.min.css';
            ...
            `);
                ...
                if (typeof SSVvideosAll['swilrs']['video'] != 'undefined') {
                    ...
                    SSVvideosAll['swilrs']['video'].forEach(swirl => {
                        ...
                        <video poster="${coverPoster}" ontimeupdate="updateSSVProgressBar(this);" onloadeddata="updateSSVDurations(this);" playsinline preload="metadata" data-setup="{}" muted ${firstFivePlay} width="100%" height="100%" data-product="${product_link}" data-title="${swirl['video_title']}" data-cover="${swirl['image']}" data-link="${swirl['link']}" data-vid="${swirl['video_id']}" data-did="${swirl['designer_id']}" data-productid="${product_id}" data-productlink="${product_link}" data-gumletassetid="${swirl['gumletAssetId']}" data-totalviews="${swirl['total_views']}" data-buynow="${swirl['cta_customization']}" data-videourl="${swirl['video_url']}" data-landscape="${swirl['is_landscape']}">
        ...
        function executeNow() {
            ...
            jQuery(document).ready(function () {
                ...
                <div id="SSV-short-videos">
                    <div class="swiper" ontouchmove="alert(''); aupdateSSVSwiperNavigation();">
                        <div class="swiper-wrapper">
                            <p style="margin: 50px 0; padding: 10px; text-align: center; font-size: 20px; color: #999;display: block;width: 100%;">Loading Videos...</p>
                        </div>
                        <div class="swiper-button-next">
            ...
            `);
                ...
                let playlist = jQuery('#swirl-short-videos').data('playlist');
                let domain = (new URL(url));
                store = domain.hostname;
                store = shopify ? store : 'satyamgandhidev.com';
                storeType = shopify;
                ...
                let urlPass = '';
                urlPass = "https://api.goswirl.live/index.php/ShortVideo/videolistingV4";

                if (typeof jQuery('#swirl-short-videos').data('onpdp') != 'undefined') {
                    urlPass = 'https://api.goswirl.live/index.php/ShortVideo/getmultiple_productvideo';
                }
                ...
                jQuery.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (localStorage.getItem('SSVallURL') && typeof jQuery('#swirl-short-videos').data('onpdp') != 'undefined' && typeof data['swilrs']['video'] == 'undefined') {
                            data = JSON.parse(localStorage.getItem('SSVallURL'));
                            jQuery('#SSV-short-videos .swiper').css('display', 'none');
                        }
...
function updateSSVModalInfos(player) {
    ...
    if (Object.keys(products).length > 0) {
        ...
        let rLink = jQuery(`.swiper-wrapper source[src="${video}"]`).length ? jQuery(`.swiper-wrapper source[src="${video}"]`).parent('video').attr('data-productlink') : PIPvideoData['productlink'];
        if (rLink) {
    ...
    // hide show addtocart buynow and buynow btn
...
function updateSSVSwiperNavigation() {
    if (jQuery(window).width() >= 640 && jQuery(".swiper .swiper-slide").length < 6) {
        ...
        jQuery('.swiper-button-prev, .swiper-button-next').css("opacity", '0');
