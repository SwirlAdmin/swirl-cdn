...
var showcase_bar = (productsid != '') ? 70 : 0;
var end_time = $('#end_time').val();
if ((end_time != '0000-00-00 00:00:00')) {
  var recordingcontrol_size = 36;
}
else{
  $('.swiper-button-next').css('bottom','80px');
  $('.swiper-button-prev').css('bottom','80px');
  $('#bottom-showcase-products').css('bottom','56px');
...
const isValidUrl = string => {
  ...
  if ($(window).width() < 768) {
    let pinheight = $("#bottom-showcase-products").height() + actionbar_size + recordingcontrol_size + 5;
    $('.messages-pin-mobile').css('bottom', pinheight);
    

   /*  let arrow = pinheight + 30;
...
$('body').on('click', '.SLS-product-buynow-btn-new', function() {
  var product_id = $(this).attr("data-proid");
  var designer_id = $(this).attr("data-desid");
  var pro_title = $(this).attr("data-protitle");
  var pro_img = $(this).attr("data-proimg");
  ...
  var mobileOS = navigator.userAgent || navigator.vendor || window.opera;
  if (/iPad|iPhone|iPod/.test(mobileOS) && !window.MSStream) {
    $('.SLS-product-buynow-btn-new').click(function () {
      var redirectWindow = window.open(pro_redirection, '_blank');
      $.ajax({
        type: 'POST',
        url: '/echo/json/',
  ...
  if (doc.exists) {
    ...
    if (pin != '' && pin != undefined) {
      ...
      docRef.get().then((doc) => {
        ...
        let pinheight = $("#bottom-showcase-products").height() + actionbar_size + recordingcontrol_size + 10;
        $('.messages-pin-mobile').css('bottom', pinheight);
  ...
  if (msg == '') {
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        $('.err_ask_success').html(json.message).fadeIn().delay(3000).fadeOut();
        //alert(json.message);
        //ask_que_count();
      }
    });
