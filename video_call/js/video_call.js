  ...
  if (document.querySelector('#swirl-video-call') != null) {
    ...
    function SVCexecuteNow() {
      ...
      jQuery.ajax({
        ...
        ` : `<input type="hidden" id="SVCRCountryCode" value="91">`;

        let defaultIndian = brandCustonization['country_dropdown'] == 0 ? `<p class="default-indian">+91</p>` : ``;
        ...
        div.innerHTML = `
            <div class="swirl-video-call-floating">
              <img src="https://cdn.jsdelivr.net/gh/SwirlAdmin/swirl-cdn@latest/assets/images/video-call.png" alt="Video call icon" onclick="openVCPopup();">              
              <div class="swirl-video-call-popup">
                <div class="swirl-video-call-popup-head">${brandCustonization['video_header_txt']} <span style="font-family:arial; " onclick="closeVCPopup();">&times;</span></div>
                <div class="swirl-video-call-popup-container">                              
                  <div class="swirl-video-call-instant swirl-video-call-all-blocks">
                    <form id="SWIRLstartVC" onsubmit="return SWIRLstartVC();" autocomplete="off">
                      ...
                      <div class="SVC-phone-number" ${brandCustonization['country_dropdown'] == 0 ? 'style="width: 100%; padding-left: 0;"' : ''}>
                        ${defaultIndian}
                        <input type="text" id="SVCRPhone" class="swirl-video-call-popup-input" placeholder="Phone Number" required autocomplete="off" pattern=".{10,10}" onkeypress='return event.charCode >= 48 && event.charCode <= 57' title="Minimum 10 and maximim 10 digits." ${brandCustonization['country_dropdown'] == 0 ? 'style="padding-left: 45px;"' : ''}>
                      </div>
                      </div>
                      ${emailDynamic}
                      ${dropdown}
                      <p class="SVC-wa-note"><span>*</span>The Video Call link will be sent to you on WhatsApp</p>
                      <p class="SVC-alert-message"></p>                                            
                      <button class="svc-action-btns swirl-video-call-popup-next-btn">${brandCustonization['schedule_btn_text']}</button>
                    </form>
                  </div>
                  <div class="swirl-video-call-schedule swirl-video-call-all-blocks">
                    <form id="SWIRLscheduleVC" onsubmit="return SWIRLscheduleVC();">
                      ...
                      <p class="SVC-alert-message"></p>
                      <button type="button" class="svc-action-btns" style="width: 49% !important; display: inline-block;" onclick="SVCCallSteps('.swirl-video-call-instant');">Back</button>
                      <button class="svc-action-btns swirl-video-call-popup-schedule-btn" style="width: 49% !important; display: inline-block; padding: 8px 0 !important;">${brandCustonization['book_now_txt']} <p class="SWIRL-VC-loader" style="margin-right: 8px;"></p></button>
                    </form>
                  </div>                  
