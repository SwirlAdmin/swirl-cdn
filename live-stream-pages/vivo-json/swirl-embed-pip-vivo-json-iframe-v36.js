            ...
            if (Object.keys(data).length && (data['scheduled'].length || data['completed'].length)) {
                ...
                if (data['scheduled'].length) {
                    data['scheduled'].forEach(scheduled => {
                        scheduled.streamURL = scheduled.streamURL.replace('https://live.goswirl.in/', 'https://jawa.goswirl.in/');
                ...
                if (data['completed'].length) {
                    data['completed'].forEach(completed => {
                        completed.streamURL = completed.streamURL.replace('https://live.goswirl.in/', 'https://jawa.goswirl.in/');
...
eventerSL(messageEventSL, function (e) {
    ...
    if (action == 'streamLiveStarted') {
        let lLabel = document.querySelector('.SL-stream-tag-full');
        if (lLabel) lLabel.innerHTML = 'Currently <span>Live</span>';

        // update LS
        if (localStorage.getItem('_pipSL')) {
            let pipStreamData = JSON.parse(localStorage.getItem('_pipSL'));
            pipStreamData.is_live = '1';
            localStorage.setItem('_pipSL', JSON.stringify(pipStreamData));
        }
        // Update local array
        allStreamsSL[btoa(currentStreamSL)].is_live = '1';
    }
    if (action == 'streamLiveEnded') {
        let lLabel = document.querySelector('.SL-stream-tag-full');
        if (lLabel) lLabel.innerHTML = 'Watch Again';
    }
...
window.addEventListener('resize', () => {
    if (document.querySelector('.SL-iframe-live')) SLFrameHeight();
});
