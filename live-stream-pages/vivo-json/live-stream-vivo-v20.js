...
jqTag.href = 'https://apigoswirl.com/live_stream_pages/vivo-json/style.min.css';
headTag.insertBefore(jqTag, headTag.lastChild);
...
function resetPIPSL() {
    document.querySelector('.SL-pip-live').innerHTML = `
       ...
       <img class="SL-pip-close-btn JS-ignore-click-SL" onclick="closePIPSL();" src="https://apigoswirl.com/assets/vivo/close-black.webp">  
        <img class="SL-pip-fullscreen-btn" src="https://apigoswirl.com/assets/vivo/arrow-up-s-line-black.webp">            
        <img class="SL-pip-playpause-btn JS-ignore-click-SL" onclick="playPausePIPSL(this);" src="https://apigoswirl.com/assets/vivo/pause-black.webp">
        <img class="SL-pip-muteunmute-btn JS-ignore-click-SL" onclick="muteUnmutePIPSL(this);" src="https://apigoswirl.com/assets/vivo/mute-black.webp">
        <div class="SL-pip-loader" style="display: none;">
            <img src="https://apigoswirl.com/assets/vivo/spiner.svg" alt="Spinner">
        </div>
    `;
}

function playPausePIPSL(btn) {
    let vdo = document.querySelector('.SL-pip-live video');
    if (vdo.paused) {
        vdo.play();
        btn.src = 'https://apigoswirl.com/assets/vivo/pause-black.webp';

    } else {
        vdo.pause();
        btn.src = 'https://apigoswirl.com/assets/vivo/play-icon-black.webp';

    }

}

function muteUnmutePIPSL(btn) {

    let vdo = document.querySelector('.SL-pip-live video');
    if (vdo.muted) {
        vdo.muted = false;
        btn.src = 'https://apigoswirl.com/assets/vivo/unmute-black.webp';
    } else {
        vdo.muted = true;
        btn.src = 'https://apigoswirl.com/assets/vivo/mute-black.webp';
    }
...
function executeSLiveNow() {
    $('body').append(`
        <div class="SL-iframe-live" style="display:none;">
            <iframe id="SL-iframe-tag" style="width: 100%;height: 100vh;border: none;position: fixed;top: 0;left: 0;z-index: 1211;" src="" allow="clipboard-read; clipboard-write; fullscreen; autoplay; encrypted-media; picture-in-picture"></iframe>
            <img class="SL-popup-close-btn SL-popup-close-btn-P" onclick="closeLiveStreamPop(true);" src="https://apigoswirl.com/assets/vivo/arrow-down-s-line-black.webp">          
            <img class="SL-popup-close-btn SL-popup-close-btn-L" style="display: none;" onclick="closeLiveStreamPop(true);" src="https://apigoswirl.com/assets/vivo/close.webp">       
        </div> 

        <div class="SL-pip-live" onclick="fullScreenPIPSL();" style="display: none;">
            <video class="" onplaying="hideLoaderSL();" id="id-video-pip-SL" width="320" height="180" style="width: 320px; height: 180px;" playsinline="playsinline" preload="auto" data-setup="{}" autoplay muted loop>
                <source src="" type=""></source>
            </video> 
            <img class="SL-pip-close-btn JS-ignore-click-SL" onclick="closePIPSL();" src="https://apigoswirl.com/assets/vivo/close-black.webp">  
    ...
    $.ajax({
        type: "GET",
        dataType: "json",
        // url: "https://bigleap.live/index.php/api/LiveAPI/streamlisting?user=" + code,
        ...
        success: function (data) {
            ...
            if (Object.keys(data).length && (data['scheduled'].length || data['completed'].length)) {
                ...
                setTimeout(() => {
                    ...
                    if (typeof params['time'] != 'undefined') {  // pass shareable time 
                ...
                document.querySelector('.SL-pip-live').addEventListener('touchend', function (event) {
                    fullScreenPIPSL(event);
                });
            }
            else {
...
eventerSL(messageEventSL, function (e) {
    ...
    if (action == 'updateStreamVolume') {
        localStorage.setItem('_pipVolumeSL', data);
    }
...
function watchStream(stream, fromPIP = false) {
    localStorage.setItem('_pipVolumeSL', true);
    ...
    let startTimeQ = '';
    if (fromPIP) {
        startTimeQ = '&time=' + Math.floor($('.SL-pip-live video')[0].currentTime);
    }
    ...
    slsFrame.contentWindow.location.replace(stream + '&close=false' + startFrom + startTimeQ);
...
function openPIPSL() {
    ...
    let pipVolume = localStorage.getItem('_pipVolumeSL') == 'true' ? true : false;
    if (pipVolume) {
        $('.SL-pip-muteunmute-btn')[0].src = 'https://apigoswirl.com/assets/vivo/mute-black.webp';
    } else {
        $('.SL-pip-muteunmute-btn')[0].src = 'https://apigoswirl.com/assets/vivo/unmute-black.webp';
    }
    ...
    if (pipStreamData.video_orientation == '1') $('.SL-pip-live video').attr('height', pH), $('.SL-pip-live video').attr('width', pW);
    if (pipStreamData.video_orientation == '1') $('.SL-pip-live video').css('height', `${pH}px`), $('.SL-pip-live video').css('width', `${pW}px`);

    if (pipStreamData.video_orientation == '1') $('.SL-pip-live').addClass('SL-pip-live-P');
    else $('.SL-pip-live').removeClass('SL-pip-live-P');
    ...
    if (pipStreamData.recording_preview && pipStreamData.is_live == '0') {
        ...
        $('.SL-pip-live video')[0].muted = pipVolume;
        ...
        jqTag.onload = function () {
            ...
            player.on('error', function () {
                ...
                player.play().catch((playError) => {
                    console.log("Failed to start playback after reload - SL");
                });
            });
        };
    }
...
function fullScreenPIPSL() {
    event.stopPropagation();
    if (event.target.matches('.JS-ignore-click-SL')) return;
...
function SLtoggleFullScreen(fs) {
    return;
}
