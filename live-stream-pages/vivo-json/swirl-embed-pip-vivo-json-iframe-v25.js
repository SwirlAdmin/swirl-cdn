            ...
            if (Object.keys(data).length && (data['scheduled'].length || data['completed'].length)) {
                ...
                if (data['completed'].length) {
                    data['completed'].forEach(completed => {
                        cStreamIds.push(completed.streamId);
                                        ...
                                        <div class="col-12 col-md-6 col-lg-4 px-sm-3 pt-4 pt-sm-0 ${customization.scheduled_grid == '0' ? 'order-md-first' : ''}"">
                                            ...
                                            <div class="SL-views-and-like">
                                                <div>
                                                    ...
                                                    <label id="view_${completed.streamId}">${completed.total_views ? completed.total_views : '0'}</label>
                                                </div>
                                                <div>
                                                    <img src="https://apigoswirl.com/assets/vivo/heart-fill.svg" alt="Heart icon">
                                                    <label id="like_${completed.streamId}">${completed.total_like ? completed.total_like : '0'}</label>
                                                </div>                                            
                                            </div>
                ...
                // GET LATEST VIEWS / LIKE
                const requestOptions = {
                    method: "GET",
                    redirect: "follow"
                };

                fetch(`https://livestream-analytics.goswirl.live/livestream/recoding/data/brand/all?streamIds=${cStreamIds.join(',')}`, requestOptions)
                    .then((response) => response.json())
                    .then((result) => {
                        if (result.success) {
                            result.data.forEach(sData => {
                                $(`#view_${sData.streamId}`).html(numberToShorthandSL(sData.views));
                                $(`#like_${sData.streamId}`).html(numberToShorthandSL(sData.likes));
                            });
                        }
                    })
...
// Utility
function numberToShorthandSL(n, precision = 1) {
    n = Number(n) || 0;
    let nFormat;
    let suffix;
    if (n < 900) {
        // 0 - 900
        nFormat = n.toFixed(precision);
        suffix = '';
    } else if (n < 900000) {
        // 0.9k-850k
    ...
    if (precision > 0) {
        ...
        nFormat = nFormat.replace(dotzero, '');
    }
